...
async function sendToken(keypair, client, contracts, args, options) {
  const [symbol, destinationChain, destinationAddress, feeAmount, amount] =
    args;

  const { Example, GasService, AxelarGateway, InterchainTokenService } =
    contracts;
  const ItsToken = contracts[symbol.toUpperCase()];

  if (!ItsToken) {
    throw new Error(
      `Token ${symbol} not found. Deploy it first with 'ts-node sui/its-example.js deploy-token' command`,
    );
  }
  ...
  await broadcastFromTxBuilder(
    txBuilder,
    keypair,
    `${amount} ${symbol} Token Sent`,
    options,
  );
}

async function sendDeployment(keypair, client, contracts, args, options) {
  const { AxelarGateway, GasService, InterchainTokenService, Example } =
    contracts;
  ...
  await broadcastFromTxBuilder(
    txBuilder,
    keypair,
    `Sent ${symbol} Deployment on ${destinationChain}`,
    options,
  );
}
...
async function handleReceivedMessage(
  ...
  await broadcastExecuteApprovedMessage(
    client,
    keypair,
    discoveryInfo,
    gatewayInfo,
...
async function receiveDeployment(keypair, client, contracts, args, options) {
  ...
  await handleReceivedMessage(
    ...
    args,
    options,
    `Received ${symbol} Token Deployment`,
  );
}
...
async function deployToken(keypair, client, contracts, args, options) {
  ...
  const publishTxn = await broadcastFromTxBuilder(
    txBuilder,
    keypair,
    `Published ${symbol}`,
    options,
  ...
  const [TreasuryCap, Metadata] = getObjectIdsByObjectTypes(publishTxn, [
    `TreasuryCap<${tokenType}>`,
    `Metadata<${tokenType}>`,
  ]);
...
async function printReceiveDeploymentInfo(contracts, args, options) {
  ...
  const hubMessage = defaultAbiCoder.encode(
    ["uint256", "string", "bytes"],
    [ITSMessageType.ReceiveFromItsHub, sourceChain, itsMessage],
  );
...
async function printReceiveTransferInfo(contracts, args, options) {
  ...
  const itsMessage = defaultAbiCoder.encode(
    ["uint256", "uint256", "bytes", "bytes", "uint256", "bytes"],
    [
      ITSMessageType.InterchainTokenTransfer,
      tokenId,
      sourceAddress,
      channelId,
      unitAmount,
...
if (require.main === module) {
  ...
  program
    .name("InterchainTokenService Example")
    .description("SUI InterchainTokenService Example scripts");

  const sendTokenTransferProgram = new Command()
    .name("send-token")
    .description("Send token from Sui to other chain.")
    .command(
      "send-token <symbol> <dest-chain> <dest-contract-address> <fee> <amount>",
    )
    .action(
      (symbol, destChain, destContractAddress, feeAmount, amount, options) => {
        mainProcessor(
          sendToken,
          options,
          [symbol, destChain, destContractAddress, feeAmount, amount],
          processCommand,
        );
      },
    );
  ...
  const receiveTokenTransferProgram = new Command()
    ...
    .command(
      "receive-token <source-chain> <message-id> <source-address> <token-symbol> <payload>",
    )
    .addOption(
      new Option("--data <data>", "Data").default(ethers.constants.HashZero),
    )
    .action(
      (
        sourceChain,
        ...
        mainProcessor(
          receiveToken,
          options,
          [sourceChain, messageId, sourceAddress, tokenSymbol, payload],
          processCommand,
        );
      },
    );
  ...
  const deployTokenProgram = new Command()
    .name("deploy-token")
    .description(
      "Deploy token on Sui. The supported token manager modes are lock_unlock (default) and mint_burn.",
    )
    ...
    .addOption(
      new Option(
        "--origin",
        "Deploy as a origin token or receive deployment from another chain",
        false,
      ),
    )
  ...
  const receiveTokenDeploymentProgram = new Command()
    ...
    .command(
      "receive-deployment <source-chain> <message-id> <source-address> <token-symbol> <payload>",
    )
    .action(
      (symbol, sourceChain, messageId, sourceAddress, payload, options) => {
        mainProcessor(
          receiveDeployment,
          options,
          [symbol, sourceChain, messageId, sourceAddress, payload],
          processCommand,
        );
      },
    );
  ...
  const mintTokenProgram = new Command()
    .name("mint-token")
    .description(
      "Mint token for the given symbol on Sui. The token must be deployed on Sui first.",
    )
    .command("mint-token <symbol>")
    .addOption(new Option("--recipient <recipient>", "Recipient address"))
    .addOption(
      new Option("--amount <amount>", "Amount to mint").default("1000"),
    )
  ...
  const printDeploymentInfoProgram = new Command()
    ...
    .description(
      "Print deployment info. This script will be useful for testing receive deployment flow.",
    )
    .command(
      "print-receive-deployment <sourceChain> <name> <symbol> <decimals>",
    )
    .addOption(
      new Option("--distributor <distributor>", "Distributor address").default(
        ethers.constants.HashZero,
      ),
    )
    .addOption(
      new Option("--tokenId <tokenId>", "Token ID").default(
    ...
    .action((sourceChain, name, symbol, decimals, options) => {
      const config = loadConfig(options.env);
      const chain = getChainConfig(config, options.chainName);
      printReceiveDeploymentInfo(
        chain.contracts,
        [sourceChain, name, symbol, decimals],
        options,
      );
    });
  ...
  const printReceiveTransferInfoProgram = new Command()
    ...
    .description(
      "Print receive token info. This script will be useful for testing receive token flow.",
    )
    .command(
      "print-receive-transfer <sourceChain> <symbol> <source-address> <amount>",
    )
    .addOption(
      new Option(
        "--itsBytes <itsBytes>",
        "InterchainTokenService Bytes",
      ).default(ethers.constants.HashZero),
    )
    .action((sourceChain, symbol, sourceAddress, amount, options) => {
      const config = loadConfig(options.env);
      const chain = getChainConfig(config, options.chainName);
      printReceiveTransferInfo(
        chain.contracts,
        [sourceChain, symbol, sourceAddress, amount],
        options,
      );
    });
