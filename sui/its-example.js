...
async function sendToken(keypair, client, contracts, args, options) {
    ...
    if (!ItsToken) {
        throw new Error(`Token ${symbol} not found. Deploy it first with 'npm run script sui/its-example.js deploy-token' command`);
    }
...
if (require.main === module) {
    ...
    // To deploy the token, use the command `npm run script sui/its-example.js deploy-token <symbol> <name> <decimals>`
