...
class CoinManager {
  static async getAllCoins(client, account) {
    ...
    try {
      ...
      const coins = await paginateAll(client, "getAllCoins", {
        owner: account,
      });
  ...
  static async splitCoins(
    ...
    const objectToSplit = isGasToken(coinType)
      ? tx.gas
      : coinTypeToCoins[coinType].data.find(
          (coinObject) => BigInt(coinObject.balance) >= splitAmount,
        )?.coinObjectId;
    ...
    printInfo(
      "Split Amount",
      `${formatUnits(splitAmount, metadata.decimals).toString()}`,
    );
  ...
  static async mergeCoins(tx, coinTypeToCoins, options) {
    const coinTypes = options.coinType
      ? [options.coinType]
      : Object.keys(coinTypeToCoins);
...
async function processSplitCommand(keypair, client, args, options) {
  ...
  const coinTypeToCoins = await CoinManager.getAllCoins(
    client,
...
if (require.main === module) {
  // Main program
  const program = new Command("tokens").description(
    "Merge, split, and list coins.",
  );

  // Sub-programs
  const mergeProgram = new Command("merge").description(
  ...
  const listProgram = new Command("list").description(
    "List all coins and balances",
  );

  // Define options, arguments, and actions for each sub-program
  mergeProgram
    .option("--coin-type <coinType>", "Coin type to merge")
    .action((options) => {
  ...
  splitProgram
    .argument(
      "<amount>",
      "Amount should be in the full coin unit (e.g. 1.5 for 1_500_000_000 coins)",
      parseSuiUnitAmount,
    )
    .option(
      "--transfer <recipientAddress>",
      "Used with split command to transfer the split coins to the recipient address",
    )
