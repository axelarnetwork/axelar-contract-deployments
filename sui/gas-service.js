...
async function collectGas(keypair, client, gasServiceConfig, args, options) {
  ...
  const gasServiceBalance = bcs.U64.parse(
  ...
  if (gasServiceBalance < unitAmount) {
    printError(
      "Insufficient gas service balance",
      `${getFormattedAmount(gasServiceBalance)} < ${getFormattedAmount(unitAmount)}`,
    );
    return;
  }
...
if (require.main === module) {
  ...
  program
    .name("gas-service")
    .description("Interact with the gas service contract.");
  ...
  const payGasCmd = new Command()
    ...
    .option(
      ...
      "Refund address. Default is the sender address.",
    )
    .requiredOption(
      "--amount <amount>",
      "Amount to pay gas",
  ...
  const collectGasCmd = new Command()
    ...
    .option(
      "--receiver <receiver>",
      "Receiver address. Default is the sender address.",
    )
    .requiredOption(
      "--amount <amount>",
      "Amount to collect gas",
      parseSuiUnitAmount,
    )
    .action((options) => {
