...
async function setFlowLimits(
  ...
  if (tokenIds.length !== flowLimits.length)
    throw new Error(
      "<token-ids> and <flow-limits> have to have the same length.",
    );
  ...
  for (const i in tokenIds) {
    ...
    const coinType = bcs.String.parse(
      new Uint8Array(resp.results[1].returnValues[0][0]),
    );
  ...
  if (options.offline) {
    ...
    await saveGeneratedTx(
      tx,
      `Set flow limits for ${tokenIds} to ${flowLimits}`,
      client,
      options,
    );
  } else {
...
if (require.main === module) {
  ...
  program
    ...
    .description("SUI InterchainTokenService scripts");
  ...
  const setFlowLimitsProgram = new Command()
    ...
    .description(
      `Set flow limits for multiple tokens. <token-ids> and <flow-limits> can both be comma separated lists`,
    )
