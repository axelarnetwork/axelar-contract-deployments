...
function getSignatures(keypair, messageToSign, options) {
  if (options.proof === "wallet") {
    ...
    const { signature, recid } = secp256k1.ecdsaSign(
      arrayify(keccak256(messageToSign)),
      getRawPrivateKey(keypair),
    );
...
async function approve(
  ...
  const packageId = contractConfig.address;
  const [sourceChain, messageId, sourceAddress, destinationId, payloadHash] =
    args;
  ...
  const encodedProof = getProof(
    keypair,
    COMMAND_TYPE_APPROVE_MESSAGES,
    encodedMessages,
    contractConfig,
    options,
  );
...
async function submitProof(
  ...
  const { payload, status } = await getMultisigProof(
    config,
    chain.axelarId,
    multisigSessionId,
  );
  ...
  const executeData = bcsStructs.gateway.ExecuteData.parse(
    arrayify("0x" + status.completed.execute_data),
  );
    ...
    tx.moveCall({
      ...
      arguments: [
        ...
        tx.pure(
          bcs
            .vector(bcs.u8())
            .serialize(new Uint8Array(executeData.payload))
            .toBytes(),
        ),
        tx.pure(
          bcs
            .vector(bcs.u8())
            .serialize(new Uint8Array(executeData.proof))
            .toBytes(),
        ),
      ],
    });
...
async function rotate(
  ...
  const newNonce = options.newNonce
    ? keccak256(toUtf8Bytes(options.newNonce))
    : signers.nonce;
...
async function checkVersionControl(version, options) {
  ...
  if (options.allowedFunctions) {
    const allowedFunctions =
      options.allowedFunctions === "all"
        ? Object.keys(functions)
        : options.allowedFunctions.split(",");
    ...
    for (const allowedFunction of allowedFunctions) {
      ...
      console.log(
        `${allowedFunction} is ${color(allowed ? "allowed" : "dissalowed")}`,
      );
    }
  }
  ...
  if (options.disallowedFunctions) {
    const disallowedFunctions =
      options.allowedFunctions === "all"
        ? Object.keys(functions)
        : options.disallowedFunctions.split(",");
    ...
    for (const disallowedFunction of disallowedFunctions) {
      const allowed = await isAllowed(
        ...
        keypair,
        chain,
        functions[disallowedFunction],
        options,
      );
      const color = allowed ? chalk.red : chalk.green;
      console.log(
        `${disallowedFunction} is ${color(allowed ? "allowed" : "dissalowed")}`,
      );
    }
  }
}
...
async function testNewField(value, options) {
  ...
  const returnedValue = bcs.U64.parse(
...
async function mainProcessor(processor, args, options) {
  ...
  const { tx, message } = await processor(
    keypair,
    client,
    config,
    chain,
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        ...
        "nonce of the existing signers",
      ),
    )
    .addOption(
      new Option(
        "--newNonce <newNonce>",
        "nonce of the new signers (useful for test rotations)",
      ),
    )
    .action((options) => {
  ...
  program
    .command(
      "approve <sourceChain> <messageId> <sourceAddress> <destinationId> <payloadHash>",
    )
    ...
    .action(
      (
        ...
        mainProcessor(
          approve,
          [sourceChain, messageId, sourceAddress, destinationId, payloadHash],
          options,
        );
      },
    );
  ...
  program
    ...
    .addOption(
      new Option(
        ...
        "bcs encoded data to pass to the migrate function",
      ),
    )
    .action((options) => {
  ...
  program
    ...
    .addOption(
      new Option(
        "--channel <channel>",
        "Existing channel ID to initiate a cross-chain message over",
      ),
    )
    .action((destinationChain, destinationAddress, payload, options) => {
      mainProcessor(
        callContract,
        [destinationChain, destinationAddress, payload],
        options,
      );
    });
  ...
  program
    ...
    .addOption(
      new Option(
        ...
        "Functions that should be allowed on this version",
      ),
    )
    .addOption(
      new Option(
        "--disallowed-functions <disallowed-functions>",
        "Functions that should be disallowed on this version",
      ),
    )
