...
function getVariablesForPackage(chain, packageName) {
  ...
  const version = Math.max(
    ...Object.keys(contractConfig.versions).map((version) => Number(version)),
  );
...
async function disallowFunctions(
  keypair,
  client,
  packageId,
  moduleName,
...
async function pause(keypair, client, chain, args, options) {
  ...
  const {
    ...
    defaultFunctions,
    contract,
  } = getVariablesForPackage(chain, packageName);

  let versionsArg = [];
  let allowedFunctionsArg = [];

  if (functions === SPECIAL_PAUSE_FUNCTION_TAGS.ALL) {
    const allowedFunctionsArray = await getAllowedFunctions(
      client,
      versionedId,
    );
    ...
    for (const version in allowedFunctionsArray) {
      ...
      if (Number(version) === allowedFunctionsArray.length - 1) {
        allowedFunctions = allowedFunctions.filter(
          (allowedFunction) =>
            allowedFunction !== "allow_function" &&
            allowedFunction !== "disallow_function",
        );
      }

      printInfo(
        `Functions that will be disallowed for version ${version}`,
        allowedFunctions,
      );

      versionsArg = versionsArg.concat(
        new Array(allowedFunctions.length).fill(Number(version)),
      );
    ...
    throw new Error(
      "Need to specify a version if providing specific functions.",
    );
  }
  ...
  contract.disallowedFunctions.versions =
    contract.disallowedFunctions.versions.concat(versionsArg);
  contract.disallowedFunctions.functionNames =
    contract.disallowedFunctions.functionNames.concat(allowedFunctionsArg);
...
async function unpause(keypair, client, chain, args, options) {
  ...
  if (contract.disallowedFunctions) {
    for (
      let i = contract.disallowedFunctions.versions.length - 1;
      i >= 0;
      i--
    ) {
      ...
      for (let j = 0; j < versionsArg.length; j++) {
        if (
          version === versionsArg[j] &&
          functionName === allowedFunctionsArg[j]
        ) {
