...
function getWallet(chain, options) {
  ...
  switch (options.signatureScheme) {
    ...
    default: {
      throw new Error(
        `Unsupported signature scheme: ${options.signatureScheme}`,
      );
    }
  }
...
async function printWalletInfo(wallet, client, chain, options = {}) {
  ...
  if (options.privateKey !== "ledger") {
    owner =
      wallet instanceof Ed25519Keypair ||
      wallet instanceof Secp256k1Keypair ||
      wallet instanceof Secp256r1Keypair
        ? wallet.toSuiAddress()
        : wallet;
  } else {
    ...
    printInfo(
      "PublicKey",
      (await wallet.getPublicKey()).address.toString("base64"),
    );
  }
  ...
  if (!options.offline) {
    ...
    printInfo(
      "Wallet balance",
      `${coins.totalBalance / 1e9} ${chain.tokenSymbol || coins.coinType}`,
    );
  }
}
...
async function askForConfirmation(actionName, commandOptions = {}) {
  ...
  if (!yes) {
    const promptTitle = actionName
      ? `Confirm ${actionName} Tx?`
      : "Confirm Tx?";
...
async function broadcastFromTxBuilder(
  txBuilder,
  keypair,
  actionName,
  commandOptions = {},
...
async function broadcastSignature(
  client,
  txBytes,
  signature,
  actionName,
...
async function signTransactionBlockBytes(keypair, client, txBytes, options) {
  const serializedSignature = (await keypair.signTransaction(txBytes))
    .signature;
  ...
  if (publicKey.toSuiAddress() !== (await keypair.toSuiAddress())) {
    throw new Error(
      `Verification failed for address ${keypair.toSuiAddress()}`,
    );
  }
...
async function signTransaction(chain, txDetails, options) {
  ...
  const result = await signTransactionBlockBytes(
    keypair,
    client,
    txBytes,
    options,
...
async function getMultisig(config, multisigKey) {
  ...
  if (multisigKey) {
    ...
    for (const signer of signers) {
      ...
      publicKeys.push({
        publicKey: await getWrappedPublicKey(
          signer.publicKey,
          signer.schemeType,
        ),
