...
const deployPackage = async (packageName, client, keypair, options = {}) => {
  ...
  await builder.publishPackageAndTransferCap(
    packageName,
...
const checkSuiVersionMatch = () => {
  ...
  if (installedVersion !== definedVersion) {
    ...
    printWarn(
      `Please download the correct version (${definedVersion}) from https://github.com/MystenLabs/sui/releases`,
    );
  }
};
...
const getSigners = async (keypair, config, chain, options) => {
  if (options.signers === "wallet") {
    ...
    printInfo(
      "Using wallet pubkey as the signer for the gateway",
      hexlify(pubKey),
    );
...
const findOwnedObjectIdByType = async (client, ownerAddress, objectType) => {
  ...
  if (ownedObjects.data.length !== 1) {
    throw new Error(
      `Expecting exactly one object of type ${objectType} owned by ${ownerAddress}`,
    );
  }
...
const getBagContentId = async (client, objectType, bagId, bagName) => {
  ...
  const objectId = result.data.find(
    (cap) => cap.objectType === objectType,
  )?.objectId;
...
const getTransactionList = async (client, discoveryObjectId) => {
  ...
  const data =
    bcsStructs.relayerDiscovery.RelayerDiscovery.parse(tableBcsBytes);
...
const getStructs = async (client, packageId) => {
  ...
  for (const type of packageData.data.bcs.typeOriginTable) {
    structs[type.datatype_name] =
      `${type.package}::${type.module_name}::${type.datatype_name}`;
  }
...
const isAllowed = async (client, keypair, chain, exec, options) => {
  ...
  try {
    ...
    if (
      packageId === chain.contracts.VersionControl.address &&
    ...
    while (suiPackageAddress.length < 66) {
      suiPackageAddress =
        suiPackageAddress.substring(0, suiPackageAddress.length - 1) + "02";
    }

    if (
      packageId === suiPackageAddress &&
      module === "dynamic_field" &&
      (functionName === "borrow_child_object_mut" ||
        functionName === "borrow_child_object")
    ) {
      regexp = /Some\(".*?"\) \}, (.*?)\)/;
...
const getAllowedFunctions = async (client, versionedObjectId) => {
  ...
  const allowedFunctionsArray =
    response.data.content.fields.value.fields.version_control.fields
      .allowed_functions;
  return allowedFunctionsArray.map(
    (allowedFunctions) => allowedFunctions.fields.contents,
  );
};
