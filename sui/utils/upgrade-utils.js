...
function getUpgradePolicyId(policy) {
  switch (policy) {
    ...
    default:
      throw new Error(
        `Unknown upgrade policy: ${policy}. Supported policies: any_upgrade, code_upgrade, dep_upgrade`,
      );
  }
}
...
function broadcastRestrictedUpgradePolicy(
  client,
  keypair,
  upgradeCap,
  options,
...
async function upgradePackage(
  ...
  const { modules, dependencies, digest } = await builder.getContractBuild(
    packageDir,
    moveDir,
  );
  ...
  const ticket = tx.moveCall({
    ...
    arguments: [
      cap,
      tx.pure.u8(policy),
      tx.pure(bcs.vector(bcs.u8()).serialize(digestHash).toBytes()),
    ],
  });
  ...
  if (offline) {
    await saveGeneratedTx(
      tx,
      `Transaction to upgrade ${packageDir}`,
      client,
      options,
    );
  } else {
    ...
    const [upgradeCap] = getObjectIdsByObjectTypes(result, [
