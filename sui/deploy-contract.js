...
async function postDeployRelayerDiscovery(
  published,
  keypair,
  client,
  config,
  ...
  const [
    relayerDiscoveryObjectId,
    relayerDiscoveryObjectIdv0,
    ownerCap,
    upgradeCap,
  ] = getObjectIdsByObjectTypes(published.publishTxn, [
...
async function postDeployUtils(
  published,
  keypair,
  client,
  config,
...
async function postDeployVersionControl(
  published,
  keypair,
  client,
  config,
...
async function postDeployGasService(
  published,
  keypair,
  client,
  config,
...
async function postDeployExample(
  published,
  keypair,
  client,
  config,
  ...
  const relayerDiscovery =
    chain.contracts.RelayerDiscovery?.objects?.RelayerDiscovery;
  const { policy } = options;

  // GMP Example Params
  const [gmpSingletonObjectId] = getObjectIdsByObjectTypes(
    published.publishTxn,
    [`${published.packageId}::gmp::Singleton`],
  );

  // InterchainTokenService Example Params
  const itsObjectId =
    chain.contracts.InterchainTokenService?.objects?.InterchainTokenService;
  const [itsSingletonObjectId] = getObjectIdsByObjectTypes(
    published.publishTxn,
    [`${published.packageId}::its::Singleton`],
  );
  ...
  tx.moveCall({
    ...
    arguments: [
      ...
      tx.object(itsSingletonObjectId),
  ...
  const gmpChannelId = await getSingletonChannelId(
    client,
    gmpSingletonObjectId,
  );
  const itsChannelId = await getSingletonChannelId(
    client,
    itsSingletonObjectId,
  );
...
async function postDeployOperators(
  published,
  keypair,
  client,
  config,
  ...
  const [operatorsObjectId, ownerCapObjectId, upgradeCap] =
    getObjectIdsByObjectTypes(published.publishTxn, [
...
async function postDeployAxelarGateway(
  published,
  keypair,
  client,
  config,
...
async function postDeployIts(
  ...
  tx.moveCall({
    ...
    arguments: [
      tx.object(creatorCapObjectId),
      tx.pure.string(chainName),
      tx.pure.string(itsHubAddress),
    ],
  });

  const setupReceipt = await broadcast(client, keypair, tx, "Setup", options);

  const [InterchainTokenServiceObjectId, InterchainTokenServiceV0ObjectId] =
    getObjectIdsByObjectTypes(setupReceipt, [
  ...
  const channelId = await getItsChannelId(
    client,
    InterchainTokenServiceV0ObjectId,
  );
  ...
  tx.moveCall({
    ...
    arguments: [
      tx.object(InterchainTokenServiceObjectId),
      tx.object(relayerDiscovery),
    ],
  });
...
async function postDeploySquid(
  published,
  keypair,
  client,
  config,
  ...
  const [squidObjectId, ownerCapObjectId, upgradeCap] =
    getObjectIdsByObjectTypes(published.publishTxn, [
...
async function deploy(
  ...
  const dependencies = getLocalDependencies(
    packageDir,
    `${__dirname}/../node_modules/@axelar-network/axelar-cgp-sui/move`,
  );

  for (const { name } of dependencies) {
    if (!chain.contracts[name]) {
      throw new Error(
        `Contract ${name} needed to be deployed before deploying ${packageName}`,
      );
    }
  }
  ...
  chain.contracts[packageName].structs = await getStructs(
    client,
    published.packageId,
  );

  // Execute post-deployment function
  const executePostDeploymentFn =
    PACKAGE_CONFIGS.postDeployFunctions[packageName];

  if (executePostDeploymentFn) {
    await executePostDeploymentFn(
      published,
      keypair,
      client,
      config,
  ...
  printInfo(
    `${packageName} Configuration Updated`,
    JSON.stringify(chain.contracts[packageName], null, 2),
  );
}
...
async function syncPackages(keypair, client, config, chain, options) {
  ...
  for (const packageDir of PACKAGE_DIRS) {
    ...
    if (!packageId) {
      printWarn(
        `Package ID for ${packageName} not found in config. Skipping...`,
      );
      continue;
    }
...
const DEPLOY_CMD_OPTIONS = [
  new Option(
    "--policy <policy>",
    'upgrade policy for upgrade cap: For example, use "any_upgrade" to allow all types of upgrades',
  )
...
const GATEWAY_CMD_OPTIONS = [
  new Option("--signers <signers>", "JSON with the initial signer set").env(
    "SIGNERS",
  ),
  new Option(
    "--operator <operator>",
    "operator for the gateway (defaults to the deployer address)",
  ).env("OPERATOR"),
  new Option(
    "--minimumRotationDelay <minimumRotationDelay>",
    "minium delay for signer rotations (in second)",
  )
  ...
  new Option(
    ...
    "number of previous signers to retain",
  ).default("15"),
];
...
if (require.main === module) {
  // 1st level command
  const program = new Command("deploy-contract").description(
    "Deploy/Upgrade packages",
  );

  // 2nd level commands
  const deployCmd = new Command("deploy").description("Deploy a Sui package");
  const upgradeCmd = new Command("upgrade").description(
    "Upgrade a Sui package",
  );
  ...
  const deployContractCmds = supportedPackages.map((supportedPackage) => {
    ...
    const command = new Command(packageName).description(
      `Deploy ${packageName} contract`,
    );
  ...
  const upgradeContractCmds = supportedPackages.map((supportedPackage) => {
    ...
    return new Command(packageName)
      ...
      .addOption(
        new Option(
          "--txFilePath <file>",
          "unsigned transaction will be stored",
        ),
      )
  ...
  const syncCmd = new Command("sync")
    .description("Sync local Move packages with deployed addresses")
    .action((options) => {
