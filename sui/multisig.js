...
async function initMultisigConfig(chain, options) {
  ...
  if (!base64PublicKeys) {
    throw new Error(
      "Please provide public keys with --base64PublicKeys option",
    );
  }
  ...
  const schemeTypes =
    options.schemeTypes || Array(base64PublicKeys.length).fill("secp256k1");
...
async function signTx(chain, options) {
  ...
  if (options.offline) {
    const { signature, publicKey } = await signTransactionBlockBytes(
      keypair,
      client,
      encodedTxBytes,
      options,
...
async function combineSignature(chain, options) {
  ...
  const combinedSignature =
    multiSigPublicKey.combinePartialSignatures(signatureArray);
...
if (require.main === module) {
  const program = new Command("multisig").description(
    "Script for multisig operators to sign, combine and execute data",
  );

  const initCmd = new Command("init").description(
    "Add a sui Multisig to config",
  );
  const signCmd = new Command("sign").description("Sign via Multisig signer");
  const combineCmd = new Command("combine").description(
    "Combine signatures for a Multisig",
  );
  const executeCmd = new Command("execute").description(
    "Execute signatures for a Multisig",
  );

  initCmd.addOption(
    new Option(
      "--base64PublicKeys [base64PublicKeys...]",
      "An array of public keys to use for init the multisig address",
    ),
  );
  initCmd.addOption(
    new Option(
      "--weights [weights...]",
      "An array of weight for each base64 public key. The default value is 1 for each",
    ),
  );
  initCmd.addOption(
    new Option(
  ...
  initCmd.addOption(
    new Option("--threshold <threshold>", "threshold for multisig"),
  );
  ...
  signCmd.addOption(
    new Option("--txBlockPath <file>", "path to unsigned tx block"),
  );
  signCmd.addOption(new Option("--offline", "run in offline mode"));
  signCmd.addOption(
    new Option("--signatureFilePath <file>", "signed signature will be stored"),
  );
  signCmd.action(async (options) => {
    await mainProcessor(options, signTx);
  });

  combineCmd.addOption(
    new Option("--txBlockPath <file>", "path to unsigned tx block"),
  );
  combineCmd.addOption(new Option("--offline", "run in offline mode"));
  combineCmd.addOption(
    new Option("--signatures [files...]", "array of signed transaction files"),
  );
  combineCmd.addOption(
    new Option("--executeResultPath <file>", "execute result will be stored"),
  );
  combineCmd.addOption(
    new Option("--multisigKey <multisigKey>", "multisig key").env(
      "MULTISIG_KEY",
    ),
  );
  combineCmd.addOption(
  ...
  executeCmd.addOption(
    new Option("--combinedSignPath <file>", "combined signature file path"),
  );
