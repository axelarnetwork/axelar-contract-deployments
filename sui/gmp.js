...
async function execute(keypair, client, chains, chain, args, options) {
  ...
  const channelId =
    options.channelId || chain.contracts.Example.objects.GmpChannelId;

  if (!channelId) {
    throw new Error(
      "Please provide either a channel id (--channelId) or deploy the Example contract",
    );
  }
...
if (require.main === module) {
  ...
  const sendCallProgram = new Command()
    ...
    .action((destChain, destContractAddress, feeAmount, payload, options) => {
      mainProcessor(
        sendCommand,
        options,
        [destChain, destContractAddress, feeAmount, payload],
        processCommand,
      );
    });
  ...
  const executeCommand = new Command()
    ...
    .option(
      "--channelId <channelId>",
      "Channel id for the destination contract",
    )
