...
async function submitOperation(
  ...
  const callOperation = Array.isArray(args)
    ? await contract.call(operation, ...args)
    : await contract.call(operation);
  ...
  const result = args.simulate
    ? returnValue.result.retval._value
    : returnValue.value();
...
async function extendInstance(
  ...
  const ledgersToExtend = !options.extendBy
    ? MAX_INSTANCE_TTL_EXTENSION
    : options.extendBy;
  ...
  program
    ...
    .action((contractName, options) => {
      mainProcessor(
        submitOperation,
        contractName,
        { operation: "paused", simulate: true },
        options,
      );
    });
  ...
  program
    ...
    .action((contractName, options) => {
      mainProcessor(
        submitOperation,
        contractName,
        { operation: "operator", simulate: true },
        options,
      );
    });
