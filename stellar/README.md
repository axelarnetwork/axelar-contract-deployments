...
Note: The `--wasm-path` flag is optional, so long as the `--new-version` flag is provided (and that version's wasm is present in R2 for download).
...
```bash
node stellar/deploy-contract.js upgrade <CONTRACT_NAME> --new-version <NEW_VERSION> --migration-data '["GAA...", "GAB..."]'
```

For migration data of type `u64`, omit the `--migration-data` flag and pass the number directly.

```bash
node stellar/deploy-contract.js upgrade <CONTRACT_NAME> --new-version <NEW_VERSION> --migration-data 1234567890
```

For migration data of type `bool`, omit the `--migration-data` flag and pass the boolean directly:

```bash
node stellar/deploy-contract.js upgrade <CONTRACT_NAME> --new-version <NEW_VERSION> --migration-data true
```
