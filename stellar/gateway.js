...
function getProof(dataHash, wallet, chain, options) {
  const nonce = options.currentNonce
    ? arrayify(id(options.currentNonce))
    : Array(32).fill(0);
  ...
  const domainSeparator =
    chain.contracts.AxelarGateway?.initializeArgs?.domainSeparator;
...
async function approve(wallet, _, chain, contractConfig, args, options) {
  const contract = new Contract(contractConfig.address);
  const [sourceChain, messageId, sourceAddress, destinationAddress, payload] =
    args;
...
async function validateMessage(
  ...
  const callArgs = [
    ...
    sourceChain,
    sourceAddress,
    Buffer.from(arrayify(keccak256(payload))),
  ].map(nativeToScVal);
...
async function rotate(wallet, config, chain, contractConfig, args, options) {
  ...
  const newSigners = weightedSignersToScVal(
    await getNewSigners(wallet, config, chain, options),
  );
...
async function execute(wallet, _, chain, contractConfig, args, options) {
  ...
  const messageApproved = await broadcast(
    isMessageApprovedOperation,
    wallet,
    chain,
    "is_message_approved called",
    options,
    true,
  );
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "--signers <signers>",
        "Either use `wallet` or provide a JSON with the new signer set",
      ),
    )
    .addOption(
      new Option(
  ...
  program
    .command(
      "approve <sourceChain> <messageId> <sourceAddress> <destinationAddress> <payload>",
    )
    ...
    .action(
      (
        ...
        mainProcessor(
          approve,
          [sourceChain, messageId, sourceAddress, destinationAddress, payload],
          options,
        );
      },
    );
  ...
  program
    .command(
      "validate-message <sourceChain> <messageId> <sourceAddress> <payload>",
    )
    .description(
      "Validate an approved message at the gateway contract. The signer will be treated as the destination address",
    )
  ...
  program
    .command(
      "execute <sourceChain> <messageId> <sourceAddress> <destinationAddress> <payload>",
    )
