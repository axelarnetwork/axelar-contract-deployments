...
function proofToScVal(proof) {
  return nativeToScVal(
    {
      signers: proof.signers.map(
        ({ signer: { signer, weight }, signature }) => {
          return {
            signer: {
              signer: Address.fromString(signer).toBuffer(),
              weight,
            ...
            signature: signature
              ? nativeToScVal([
                  nativeToScVal("Signed", { type: "symbol" }),
...
function itsCustomMigrationDataToScValV112(migrationData) {
  return nativeToScVal(
    {
      new_token_manager_wasm_hash: Buffer.from(
        migrationData.newTokenManagerWasmHash,
        "hex",
      ),
      new_interchain_token_wasm_hash: Buffer.from(
        migrationData.newInterchainTokenWasmHash,
        "hex",
      ),
    },
    {
