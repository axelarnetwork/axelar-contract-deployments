...
const deploy = async (options, config, chain, contractName) => {
  ...
  const wasmHash = await uploadWasm(
    wallet,
    chain,
    options.contractCodePath,
    contractName,
  );
  const initializeArgs = await getInitializeArgs(
    config,
    chain,
    contractName,
    wallet,
  ...
  const serializedArgs = Object.fromEntries(
    Object.entries(initializeArgs).map(([key, value]) => [
      key,
      serializeValue(scValToNative(value)),
    ]),
  );
  ...
  printInfo(
    ...
    JSON.stringify(serializedArgs, null, 2),
  );

  const deployResponse = await broadcast(
    operation,
    wallet,
    chain,
...
const upgrade = async (options, _, chain, contractName) => {
  ...
  const newWasmHash = await uploadWasm(
    wallet,
    chain,
    options.contractCodePath,
    contractName,
  ...
  printInfo("Contract upgraded successfully", {
    contractName,
    newWasmHash: serializeValue(newWasmHash),
  });
};

const upload = async (options, _, chain, contractName) => {
  const wallet = await getWallet(chain, options);
  const contractCodePath = await getUploadContractCodePath(
    options,
    contractName,
  );
  const newWasmHash = await uploadWasm(
  ...
  printInfo("Contract uploaded successfully", {
    contractName,
    wasmHash: serializeValue(newWasmHash),
  });
};

const getInitializeArgs = async (
  config,
  chain,
  contractName,
  wallet,
  ...
  const owner = nativeToScVal(Address.fromString(wallet.publicKey()), {
    type: "address",
  });
  const operator = nativeToScVal(Address.fromString(wallet.publicKey()), {
    type: "address",
  });
  ...
  switch (contractName) {
    case "AxelarGateway": {
      const domainSeparator = nativeToScVal(
        Buffer.from(arrayify(await getDomainSeparator(config, chain, options))),
      );
      const minimumRotationDelay = nativeToScVal(options.minimumRotationDelay);
      const previousSignersRetention = nativeToScVal(
        options.previousSignersRetention,
      );
      const nonce = options.nonce
        ? arrayify(id(options.nonce))
        : Array(32).fill(0);
    ...
    case "InterchainTokenService": {
      const gatewayAddress = nativeToScVal(
        Address.fromString(chain.contracts?.AxelarGateway?.address),
        { type: "address" },
      );
      const gasServiceAddress = nativeToScVal(
        Address.fromString(chain.contracts?.AxelarGasService?.address),
        { type: "address" },
      );
      const itsHubAddress = nativeToScVal(
        config.axelar?.contracts?.InterchainTokenService?.address,
        { type: "string" },
      );
      const chainName = nativeToScVal(chain.axelarId, { type: "string" });
      const nativeTokenAddress = nativeToScVal(
        Address.fromString(chain?.tokenAddress),
        { type: "address" },
      );
      const interchainTokenWasmHash = BytesToScVal(
        await uploadContract("InterchainToken", options, wallet, chain),
      );
      const tokenManagerWasmHash = BytesToScVal(
        await uploadContract("TokenManager", options, wallet, chain),
      );
    ...
    case "AxelarGasService": {
      ...
      const operator = operatorsAddress
        ? nativeToScVal(Address.fromString(operatorsAddress), {
            type: "address",
          })
        : owner;
    ...
    case "AxelarExample": {
      ...
      const itsAddress = options.useDummyItsAddress
        ? gatewayAddress
        : nativeToScVal(chain.contracts?.InterchainTokenService?.address, {
            type: "address",
          });
...
const uploadWasm = async (wallet, chain, filePath, contractName) => {
  ...
  const wasmResponse = await broadcast(
    operation,
    wallet,
    chain,
    `Uploaded ${contractName} wasm`,
    {},
    false,
  );
