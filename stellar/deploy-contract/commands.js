...
const CONTRACT_DEPLOY_OPTIONS = {
  AxelarGateway: () => [
    ...
    new Option(
      ...
      'domain separator (keccak256 hash or "offline")',
    ).default("offline"),
    new Option(
      ...
      .argParser(Number),
    new Option(
      "--minimum-rotation-delay <miniumRotationDelay>",
  ...
  AxelarExample: () => [
    new Option(
      "--use-dummy-its-address",
      "use dummy its address for AxelarExample contract to test a GMP call",
    ).default(false),
  ],
};
...
const CONTRACT_UPGRADE_OPTIONS = {
  AxelarGateway: () => [
    new Option("--migration-data <migrationData>", "migration data").default(
      null,
      "()",
    ),
  ],
...
const getDeployContractCommands = () => {
  return Array.from(SUPPORTED_CONTRACTS).map((contractName) => {
    const command = new Command(contractName).description(
      `Deploy ${contractName} contract`,
    );
...
`,
    ...
    command.hook("preAction", preActionHook(contractName));
    command.action((options) => {
      options.migrationData = sanitizeMigrationData(
