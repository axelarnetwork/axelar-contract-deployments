...
async function deployMultisig(_config, wallet, client, chain, options) {
  ...
  const multisigReserve = Math.ceil(
    baseReserve + (MAX_TICKET_COUNT + 1) * ownerReserve,
  );

  if (balance < Number(multisigReserve)) {
    printWarn(
      `Wallet XRP balance (${balance} XRP) is less than required multisig account reserve (${multisigReserve} XRP)`,
    );
    process.exit(0);
  }
  ...
  if (options.generateWallet) {
    ...
    printInfo(
      `Funding multisig account with ${multisigReserve} XRP from wallet`,
    );
    ...
    if (
      prompt(
        `Proceed with turning ${wallet.address} into a multisig account?`,
        options.yes,
      )
    ) {
      return;
  ...
  await client.sendSignerListSet(
    multisig,
    {
      ...
      signers: [
        { address: options.initialSigner, weight: INITIAL_SIGNER_WEIGHT },
      ],
    },
    options,
  );
  ...
  printInfo(
    "Successfully created and configured XRPL multisig account",
    multisig.address,
  );
}
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "--generateWallet",
        "convert a new wallet (instead of the active wallet) into an XRPL multisig account",
      ).default(false),
    )
    .addOption(
      new Option(
        "--transferRate <transferRate>",
        "account transfer rate",
      ).default(DEFAULTS.TRANSFER_RATE),
    )
    .addOption(
      new Option("--tickSize <tickSize>", "account tick size").default(
        DEFAULTS.TICK_SIZE,
      ),
    )
    .addOption(
      new Option("--domain <domain>", "account domain").default(
        DEFAULTS.DOMAIN,
      ),
    )
    .addOption(
      new Option(
        "--flags <flags...>",
        "extra account flags (beyond asfDisableMaster)",
