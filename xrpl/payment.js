...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "-m, --multisign",
        "active wallet is a signer of the sender XRPL multisig account",
      ).default(false),
    )
    .addOption(
      new Option(
        "--from <from>",
        "account to send from (default: active wallet)",
      ),
    )
    .addOption(
      new Option("--to <to>", "destination account").makeOptionMandatory(true),
    )
    .addOption(
      new Option(
        "--token <token>",
        'token to send ("XRP" or "<currency>.<issuer>")',
      ).default("XRP"),
    )
    .addOption(
      new Option(
