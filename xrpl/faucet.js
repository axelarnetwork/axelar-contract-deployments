...
async function faucet(_config, wallet, client, _chain, options) {
  ...
  const isDifferentRecipient =
    wallet.address.toLowerCase() !== recipient.toLowerCase();
  ...
  if (Number(recipientBalanceInXrp) >= Number(options.minBalance)) {
    printWarn(
      `Recipient balance (${recipientBalanceInXrp} XRP) above minimum, skipping faucet request`,
    );
    process.exit(0);
  }

  const amountToClaim = roundUpToNearestXRP(
    Number(amountInDrops) + Number(fee),
  );

  if (amountToClaim > MAX_CLAIMABLE_DROPS) {
    printWarn(
      `Amount too high, maximum is ${(MAX_CLAIMABLE_DROPS - fee) / 1e6} XRP`,
    );
    process.exit(0);
  }

  printInfo(
    `Funding active wallet ${wallet.address} with`,
    `${amountToClaim / 1e6} XRP`,
  );
  await client.fundWallet(wallet, String(amountToClaim / 1e6));

  if (isDifferentRecipient) {
    printInfo(
      `Transferring ${options.amount} XRP from active wallet to recipient`,
      recipient,
    );
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "--recipient <recipient>",
        "recipient to request funds for (default: wallet address)",
      ),
    )
    .addOption(
      new Option(
