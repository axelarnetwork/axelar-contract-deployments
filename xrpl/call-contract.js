    ...
    wallet,
    {
      ...
      memos: [
        { memoType: hex("type"), memoData: hex("call_contract") },
        {
          ...
          memoData: hex(args.destinationAddress.replace("0x", "")),
        },
        {
          memoType: hex("destination_chain"),
          memoData: hex(args.destinationChain),
        },
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "--payload <payload>",
        "payload to call contract at destination address with",
      ).makeOptionMandatory(true),
    )
    .addOption(
      new Option(
        "--gasFeeToken <gasFeeToken>",
        'token to pay gas in ("XRP" or "<currency>.<issuer>")',
      ).makeOptionMandatory(true),
    )
    .addOption(
      new Option(
        "--gasFeeAmount <gasFeeAmount>",
        "amount of the deposited tokens that will be used to pay gas",
      ).makeOptionMandatory(true),
    )
    .action((destinationChain, destinationAddress, options) => {
      mainProcessor(callContract, options, {
        destinationChain,
        destinationAddress,
      });
    });
