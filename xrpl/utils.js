...
class XRPLClient {
  ...
  async tickets(account, limit = 1000, ledgerIndex = "validated") {
    const ticketRes = await this.accountObjects(
      account,
      { type: "ticket" },
      limit,
      ledgerIndex,
  ...
  async signAndSubmitTx(
    ...
    fields = {},
    args = {},
    options = { multisign: false, yes: false },
  ) {
    const tx = await this.buildTx(txType, fields, {
      ...
      fee:
        args.fee ??
        (options.multisign ? String(Number(await this.fee()) * 2) : undefined),
    });

    printInfo(
      `${options.multisign ? "Multi-" : ""}Signing transaction`,
  ...
  async sendPayment(
    signer,
    { destination, amount, memos = [], ...restArgs },
    options = { multisign: false, yes: false },
  ) {
  ...
  async sendSignerListSet(
    signer,
    { quorum, signers, ...restArgs },
    options = { multisign: false, yes: false },
  ) {
  ...
  async sendTicketCreate(
    signer,
    { ticketCount, ...restArgs },
    options = { multisign: false, yes: false },
  ) {
  ...
  async sendAccountSet(
    signer,
    { transferRate, tickSize, domain, flag, ...restArgs },
    options = { multisign: false, yes: false },
  ) {
    return this.signAndSubmitTx(
      signer,
  ...
  async sendTrustSet(
    signer,
    { currency, issuer, value, ...restArgs },
    options = { multisign: false, yes: false },
  ) {
...
async function printWalletInfo(client, wallet, chain) {
  ...
  if (balance === "0") {
    ...
    printInfo(
      "Wallet balance",
      `${xrpl.dropsToXrp(balance)} ${chain.tokenSymbol || ""}`,
    );
  }

  if (sequence === "-1") {
    printWarn(
      "Wallet is not active because it does not meet the base reserve requirement",
    );
    return;
  }
  ...
  printInfo(
    "Wallet IOU balances",
    lines
      .map((line) => `${line.balance} ${line.currency}.${line.account}`)
      .join("  "),
  );
}

async function mainProcessor(
  processor,
  options,
  args,
  save = true,
