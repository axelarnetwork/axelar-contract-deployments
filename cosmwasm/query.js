...
async function rewards(client, config, options) {
  ...
  const rewardsContractAddresses = {
    ...
    voting_verifier:
      config.axelar.contracts.VotingVerifier?.[chainName]?.address,
  };

  for (const [key, address] of Object.entries(rewardsContractAddresses)) {
    try {
      const result = await client.queryContractSmart(
        config.axelar.contracts.Rewards.address,
        {
          rewards_pool: {
            pool_id: {
      ...
      printInfo(
        `Rewards pool for ${key} on ${chainName}`,
        JSON.stringify(result, null, 2),
      );
    } catch (error) {
      printWarn(
        `Failed to fetch rewards pool for ${key} on ${chainName}`,
        `${error.message}`,
      );
    }
  }
...
const programHandler = () => {
  ...
  const rewardCmd = program
    .command("rewards")
    .description(
      "Query rewards pool state for multisig and voting_verifier contracts",
    )
    .action((options) => {
