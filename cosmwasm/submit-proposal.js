...
const predictAddress = async (client, contractConfig, options) => {
  ...
  printInfo(
    `Predicted address for ${chainName ? chainName.concat(" ") : ""}${contractName}. Address`,
    contractAddress,
  );
...
const printProposal = (proposal, proposalType) => {
  printInfo(
    ...
    JSON.stringify(
      decodeProposalAttributes(
        proposalType.toJSON(proposalType.decode(proposal.value)),
      ),
      null,
      2,
    ),
...
const storeCode = async (client, wallet, config, options) => {
  ...
  const proposalId = await callSubmitProposal(
    client,
    wallet,
    config,
    options,
  ...
  contractBaseConfig.storeCodeProposalCodeHash = createHash("sha256")
    .update(readContractCode(options))
    .digest()
    .toString("hex");
};
...
const storeInstantiate = async (client, wallet, config, options) => {
  ...
  const { contractConfig, contractBaseConfig } = getAmplifierContractConfig(
    config,
    options,
  );
  await addDefaultInstantiateAddresses(client, config, options);
  ...
  const initMsg = CONTRACTS[contractName].makeInstantiateMsg(
    config,
    options,
    contractConfig,
  );
...
const instantiate = async (client, wallet, config, options) => {
  ...
  if (instantiate2) {
    ...
    printInfo(
      "Contract address cannot be predicted without using `--instantiate2` flag, address will not be saved in the config",
    );
  }
...
const registerItsChain = async (client, wallet, config, options) => {
  const chains = options.chains.map((chain) => {
    ...
    const { maxUintBits, maxDecimalsWhenTruncating } = getChainTruncationParams(
      config,
      chainConfig,
    );
...
const programHandler = () => {
  ...
  const registerItsChainCmd = program
    ...
    .description(
      "Submit an execute wasm contract proposal to register an InterchainTokenService chain",
    )
    .argument(
      "<chains...>",
      "list of chains to register on InterchainTokenService hub",
    )
  ...
  addAmplifierOptions(paramChangeCmd, {
    paramChangeProposalOptions: true,
    proposalOptions: true,
  });
