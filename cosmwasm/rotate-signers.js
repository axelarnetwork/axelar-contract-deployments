...
const getVerifierSetStatus = async (config, chain, client, verifierStatus) => {
  return client.queryContractSmart(
    ...
    { verifier_set_status: verifierStatus },
  );
};
...
const confirmVerifierRotation = async (
  config,
  [chain, txHash],
  wallet,
  client,
  ...
  const nextVerifierSet = (await getNextVerifierSet(config, chain, client))
    .verifier_set;
  ...
  let rotationPollStatus = await getVerifierSetStatus(
    config,
    chain,
    client,
    nextVerifierSet,
  ...
  while (rotationPollStatus === "in_progress") {
    await sleep(1000);
    rotationPollStatus = await getVerifierSetStatus(
      config,
      chain,
      client,
      nextVerifierSet,
  ...
  if (rotationPollStatus !== "succeeded_on_source_chain") {
    printError(
      "Poll failed for verifier set rotation with message",
      rotationPollStatus,
    );
    process.exit(0);
  }
