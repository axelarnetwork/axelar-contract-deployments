...
const addAmplifierOptions = (program, options) => {
  ...
  program.addOption(
    new Option("-m, --mnemonic <mnemonic>", "mnemonic")
      .makeOptionMandatory(true)
      .env("MNEMONIC"),
  );
  program.addOption(
    new Option("-y, --yes", "skip prompt confirmation").env("YES"),
  );
  ...
  if (options.storeOptions) {
    ...
    program.hook("preAction", async (thisCommand) => {
      ...
      const contractCodePath = await getContractCodePath(
        opts,
        opts.contractName,
      );
  ...
  if (options.codeId) {
    program.addOption(
      new Option(
        "--codeId <codeId>",
        "the code id of the contract previously uploaded",
      ).argParser((value) => {
  ...
  if (options.fetchCodeId) {
    program.addOption(
      new Option(
        "--fetchCodeId",
        "fetch code id from the chain by comparing to the uploaded code hash",
      ),
    );
  }
  ...
  if (options.runAs) {
    program.addOption(
      new Option(
        "-r, --runAs <runAs>",
        "the address that will execute the message. Defaults to governance address",
...
const addContractOptions = (program) => {
  ...
  program.hook("preAction", (command) => {
    ...
    if (!CONTRACTS[contractName].makeInstantiateMsg) {
      throw new Error(
        `makeInstantiateMsg function for contract ${contractName} is not defined`,
      );
    }
...
const addStoreProposalOptions = (program) => {
  program.addOption(
    new Option(
      "--source <source>",
      "a valid HTTPS URI to the contract's source code",
    ),
  );
  program.addOption(
    new Option(
      "--builder <builder>",
      'a valid docker image name with tag, such as "cosmwasm/workspace-optimizer:0.16.0',
    ),
  );
  program.addOption(
    new Option(
...
const addInstantiateOptions = (program) => {
  program.addOption(
    new Option(
      "--admin <address>",
      "when instantiating contract, set an admin address. Defaults to governance module account",
    ).default(governanceAddress),
  );
  program.addOption(
    new Option("-l, --label <label>", "contract instance label"),
  );
};
...
const addInstantiate2Options = (program) => {
  program.addOption(
    new Option(
      "--instantiate2",
      "use instantiate2 for constant address deployment",
    ),
  );
  program.addOption(
    new Option(
      "-s, --salt <salt>",
      "salt for instantiate2. defaults to contract name",
    ).env("SALT"),
  );
};
...
const addInstantiateProposalOptions = (program) => {
  program.addOption(
    new Option("--predictOnly", "output the predicted changes only").env(
      "PREDICT_ONLY",
    ),
  );
};
...
const addMigrateOptions = (program) => {
  program.addOption(
    new Option(
      "--msg <msg>",
      "json encoded migration message. Use '{}' to denote an empty migration message",
    ).makeOptionMandatory(true),
  );
};
...
const addProposalOptions = (program) => {
  program.addOption(
    new Option("-t, --title <title>", "title of proposal").makeOptionMandatory(
      true,
    ),
  );
  program.addOption(
