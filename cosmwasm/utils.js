...
} = require("cosmjs-types/cosmos/params/v1beta1/params");
...
const prepareWallet = async ({ mnemonic }) =>
  await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, { prefix: "axelar" });

const prepareDummyWallet = async () => {
  return await DirectSecp256k1HdWallet.fromMnemonic(DUMMY_MNEMONIC, {
    prefix: "axelar",
  });
};
...
const fromHex = (str) =>
  new Uint8Array(Buffer.from(str.replace("0x", ""), "hex"));

const getSalt = (salt, contractName, chainName) =>
  fromHex(getSaltFromKey(salt || contractName.concat(chainName)));
...
const getAmplifierContractConfig = (config, { contractName, chainName }) => {
  const contractBaseConfig = getAmplifierBaseContractConfig(
    config,
    contractName,
  );
  ...
  if (!contractConfig) {
    throw new Error(
      `Contract ${contractName} (${chainName}) not found in config`,
    );
  }
...
const makeCoordinatorInstantiateMsg = (config, _options, contractConfig) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      "Missing or invalid Coordinator.governanceAddress in axelar info",
    );
  }

  if (!validateAddress(registryAddress)) {
    throw new Error(
      "Missing or invalid ServiceRegistry.address in axelar info",
    );
  }

  return {
    governance_address: governanceAddress,
    service_registry: registryAddress,
  };
};

const makeServiceRegistryInstantiateMsg = (
  _config,
  _options,
  contractConfig,
) => {
  ...
  if (!validateAddress(governanceAccount)) {
    throw new Error(
      "Missing or invalid ServiceRegistry.governanceAccount in axelar info",
    );
  }
...
const makeMultisigInstantiateMsg = (config, _options, contractConfig) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      "Missing or invalid Multisig.governanceAddress in axelar info",
    );
  }
...
const makeRewardsInstantiateMsg = (_config, _options, contractConfig) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      "Missing or invalid Rewards.governanceAddress in axelar info",
    );
  }
...
const makeRouterInstantiateMsg = (config, _options, contractConfig) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      "Missing or invalid Router.governanceAddress in axelar info",
    );
  }

  if (!validateAddress(axelarnetGateway)) {
    throw new Error(
      "Missing or invalid AxelarnetGateway.address in axelar info",
    );
  }
...
const makeXrplVotingVerifierInstantiateMsg = (
  config,
  options,
  contractConfig,
) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].governanceAddress in axelar info`,
    );
  }

  if (!isString(serviceName)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].serviceName in axelar info`,
    );
  }

  if (!isString(sourceGatewayAddress)) {
    throw new Error(
      `Missing or invalid [${chainName}].contracts.AxelarGateway.address in axelar info`,
    );
  }

  if (!isStringArray(votingThreshold)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].votingThreshold in axelar info`,
    );
  }

  if (!isNumber(blockExpiry)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].blockExpiry in axelar info`,
    );
  }

  if (!isNumber(confirmationHeight)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].confirmationHeight in axelar info`,
    );
  }
...
const makeVotingVerifierInstantiateMsg = (config, options, contractConfig) => {
  ...
  if (!validateAddress(governanceAddress)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].governanceAddress in axelar info`,
    );
  }

  if (!isString(serviceName)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].serviceName in axelar info`,
    );
  }

  if (!isString(sourceGatewayAddress)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].sourceGatewayAddress in axelar info`,
    );
  }

  if (!isStringArray(votingThreshold)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].votingThreshold in axelar info`,
    );
  }

  if (!isNumber(blockExpiry)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].blockExpiry in axelar info`,
    );
  }

  if (!isNumber(confirmationHeight)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].confirmationHeight in axelar info`,
    );
  }

  if (!isString(msgIdFormat)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].msgIdFormat in axelar info`,
    );
  }

  if (!isString(addressFormat)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].addressFormat in axelar info`,
    );
  }
...
const makeXrplGatewayInstantiateMsg = (config, options, contractConfig) => {
  ...
  if (!validateAddress(adminAddress)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].adminAddress in axelar info`,
    );
  }
  ...
  if (!validateAddress(itsHubAddress)) {
    throw new Error(
      "Missing or invalid InterchainTokenService.address in axelar info",
    );
  }

  if (!validateAddress(verifierAddress)) {
    throw new Error(
      `Missing or invalid XrplVotingVerifier[${chainName}].address in axelar info`,
    );
  }
...
const makeGatewayInstantiateMsg = (config, options, _contractConfig) => {
  ...
  if (!validateAddress(verifierAddress)) {
    throw new Error(
      `Missing or invalid VotingVerifier[${chainName}].address in axelar info`,
    );
  }
...
const makeXrplMultisigProverInstantiateMsg = async (
  config,
  options,
  contractConfig,
) => {
  ...
  if (!validateAddress(adminAddress)) {
    throw new Error(
      `Missing or invalid XrplMultisigProver[${chainName}].adminAddress in axelar info`,
    );
  }

  if (!validateAddress(governanceAddress)) {
    throw new Error(
      `Missing or invalid XrplMultisigProver[${chainName}].governanceAddress in axelar info`,
    );
  }

  if (!validateAddress(gatewayAddress)) {
    throw new Error(
      `Missing or invalid XrplGateway[${chainName}].address in axelar info`,
    );
  }
  ...
  if (!isStringArray(signingThreshold)) {
    throw new Error(
      `Missing or invalid XrplMultisigProver[${chainName}].signingThreshold in axelar info`,
    );
  }

  if (!isString(serviceName)) {
    throw new Error(
      `Missing or invalid XrplMultisigProver[${chainName}].serviceName in axelar info`,
    );
  }

  if (!isNumber(verifierSetDiffThreshold)) {
    throw new Error(
      `Missing or invalid XrplMultisigProver[${chainName}].verifierSetDiffThreshold in axelar info`,
    );
  }
...
const makeMultisigProverInstantiateMsg = (config, options, contractConfig) => {
  ...
  const separator =
    domainSeparator ||
    calculateDomainSeparator(chainName, routerAddress, axelarChainId);
  contractConfig.domainSeparator = separator;

  if (!validateAddress(adminAddress)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].adminAddress in axelar info`,
    );
  }

  if (!validateAddress(governanceAddress)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].governanceAddress in axelar info`,
    );
  }

  if (!validateAddress(gatewayAddress)) {
    throw new Error(
      `Missing or invalid Gateway[${chainName}].address in axelar info`,
    );
  }
  ...
  if (!isKeccak256Hash(separator)) {
    throw new Error(
      `Invalid MultisigProver[${chainName}].domainSeparator in axelar info`,
    );
  }

  if (!isStringArray(signingThreshold)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].signingThreshold in axelar info`,
    );
  }

  if (!isString(serviceName)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].serviceName in axelar info`,
    );
  }

  if (!isNumber(verifierSetDiffThreshold)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].verifierSetDiffThreshold in axelar info`,
    );
  }

  if (!isString(encoder)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].encoder in axelar info`,
    );
  }

  if (!isString(keyType)) {
    throw new Error(
      `Missing or invalid MultisigProver[${chainName}].keyType in axelar info`,
    );
  }
...
const makeAxelarnetGatewayInstantiateMsg = (
  config,
  _options,
  contractConfig,
) => {
...
const makeInterchainTokenServiceInstantiateMsg = (
  config,
  _options,
  contractConfig,
) => {
...
const fetchCodeIdFromCodeHash = async (client, contractBaseConfig) => {
  ...
  for (let i = codes.length - 1; i >= 0; i--) {
    if (
      codes[i].checksum.toUpperCase() ===
      contractBaseConfig.storeCodeProposalCodeHash.toUpperCase()
    ) {
      codeId = codes[i].id;
      break;
    }
...
const getChainTruncationParams = (config, chainConfig) => {
  ...
  const chainTruncationParams =
    config.axelar.contracts.InterchainTokenService[key];

  let maxUintBits = chainTruncationParams?.maxUintBits;
  let maxDecimalsWhenTruncating =
    chainTruncationParams?.maxDecimalsWhenTruncating;
  ...
  validateParameters({
    isValidNumber: { maxUintBits, maxDecimalsWhenTruncating },
  });
  ...
  const instantiatePermission =
    instantiateAddresses && instantiateAddresses.length > 0
      ? getInstantiatePermission(
          AccessType.ACCESS_TYPE_ANY_OF_ADDRESSES,
          instantiateAddresses,
        )
...
const getExecuteContractParams = (config, options, chainName) => {
  ...
  return {
    ...getSubmitProposalParams(options),
    contract:
      contractConfig[chainConfig?.axelarId]?.address || contractConfig.address,
    msg: Buffer.from(msg),
  };
};
...
const encodeStoreInstantiateProposal = (config, options, msg) => {
  const proposal = StoreAndInstantiateContractProposal.fromPartial(
    getStoreInstantiateParams(config, options, msg),
  );

  return {
    typeUrl: "/cosmwasm.wasm.v1.StoreAndInstantiateContractProposal",
    value: Uint8Array.from(
      StoreAndInstantiateContractProposal.encode(proposal).finish(),
    ),
  };
};
...
const encodeInstantiateProposal = (config, options, msg) => {
  const proposal = InstantiateContractProposal.fromPartial(
    getInstantiateContractParams(config, options, msg),
  );
  ...
  return {
    ...
    value: Uint8Array.from(
      InstantiateContractProposal.encode(proposal).finish(),
    ),
  };
};

const encodeInstantiate2Proposal = (config, options, msg) => {
  const proposal = InstantiateContract2Proposal.fromPartial(
    getInstantiateContract2Params(config, options, msg),
  );

  return {
    typeUrl: "/cosmwasm.wasm.v1.InstantiateContract2Proposal",
    value: Uint8Array.from(
      InstantiateContract2Proposal.encode(proposal).finish(),
    ),
  };
};

const encodeExecuteContractProposal = (config, options, chainName) => {
  const proposal = ExecuteContractProposal.fromPartial(
    getExecuteContractParams(config, options, chainName),
  );
...
const encodeParameterChangeProposal = (options) => {
  const proposal = ParameterChangeProposal.fromPartial(
    getParameterChangeParams(options),
  );
...
const encodeMigrateContractProposal = (config, options) => {
  const proposal = MigrateContractProposal.fromPartial(
    getMigrateContractParams(config, options),
  );
...
const encodeSubmitProposal = (content, config, options, proposer) => {
  ...
  return {
    ...
    value: MsgSubmitProposal.fromPartial({
      content,
      initialDeposit: [
        { denom: `u${tokenSymbol.toLowerCase()}`, amount: deposit },
      ],
      proposer,
    }),
  };
...
const submitProposal = async (client, wallet, config, options, content) => {
  ...
  const submitProposalMsg = encodeSubmitProposal(
    content,
    config,
    options,
    account.address,
