...
import { prepareDummyWallet, prepareClient } from './utils';
...
function get_contract_info(client: typeof CosmWasmClient, contract_address: string): Promise<ContractInfo> {
    return new Promise(async (resolve, _) => {
        let result = await client.queryContractRaw(contract_address, Buffer.from('contract_info'))
        let contract_info: ContractInfo = JSON.parse(Buffer.from(result).toString('ascii'))
...
const programHandler = () => {
    ...
    addAmplifierQueryOptions(program.command("info")
