...
function isValidTimeFormat(timeString) {
  const regex =
    /^\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|1\d|2\d|3[01])T(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d$/;
...
function isValidStellarAddress(addresses) {
  if (typeof addresses === "string") {
    return (
      isValidStellarAccount(addresses) || isValidStellarContract(addresses)
    );
  }
  ...
  if (Array.isArray(addresses)) {
    return addresses.every(
      (address) =>
        isValidStellarAccount(address) || isValidStellarContract(address),
    );
  }
...
const getAmplifierContractOnchainConfig = async (config, chain) => {
  ...
  const value = await client.queryContractRaw(
...
async function getDomainSeparator(config, chain, options) {
  ...
  printInfo(
    `Retrieving domain separator for ${chain.name} from Axelar network`,
  );
  const domainSeparator = hexlify(
    (await getAmplifierContractOnchainConfig(config, chain.axelarId))
      .domain_separator,
  );

  if (domainSeparator !== expectedDomainSeparator) {
    throw new Error(
      `unexpected domain separator (want ${expectedDomainSeparator}, got ${domainSeparator})`,
    );
  }
...
const getMultisigProof = async (config, chain, multisigSessionId) => {
  ...
  const client = await CosmWasmClient.connect(config.axelar.rpc);
  const value = await client.queryContractSmart(
    config.axelar.contracts.MultisigProver[chain].address,
    query,
  );
  return value;
...
const getCurrentVerifierSet = async (config, chain) => {
  const client = await CosmWasmClient.connect(config.axelar.rpc);
  const { id: verifierSetId, verifier_set: verifierSet } =
...
const calculateDomainSeparator = (chain, router, network) =>
  keccak256(Buffer.from(`${chain}${router}${network}`));
...
const itsEdgeContract = (chainConfig) => {
  ...
  if (!itsEdgeContract) {
    printError(
      `Missing InterchainTokenService edge contract for chain: ${chainConfig.name}`,
    );
  }
...
const parseTrustedChains = (config, trustedChains) => {
  return trustedChains.length === 1 && trustedChains[0] === "all"
    ? itsEdgeChains(config)
    : trustedChains;
};
...
function encodeITSDestination(config, destinationChain, destinationAddress) {
  const chainType = getChainConfig(config, destinationChain, {
    skipCheck: true,
  })?.chainType;
