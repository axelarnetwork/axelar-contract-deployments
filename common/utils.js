...
const getContractStorageConfig = async (config, chain) => {
...
async function getDomainSeparator(config, chain, options) {
    ...
    const domainSeparator = hexlify((await getContractStorageConfig(config, chain.axelarId)).domain_separator);
