...
const CHAIN_ENVIRONMENTS = fs
  .readdirSync(CHAIN_CONFIG_PATH)
  .map((chainName) => chainName.split(".")[0]);
...
const addBaseOptions = (program, options = {}) => {
  ...
  program.addOption(
    new Option("--parallel", "run script parallely wrt chains"),
  );
  program.addOption(
    new Option("--saveChainSeparately", "save chain info separately"),
  );
  program.addOption(
    new Option("--gasOptions <gasOptions>", "gas options cli override"),
  );
  ...
  if (!options.ignoreChainNames) {
    program.addOption(
      new Option(
        ...
        "chains to run the script over",
      )
        .makeOptionMandatory(true)
        .env("CHAINS"),
    );
    program.addOption(
      new Option("--skipChains <skipChains>", "chains to skip over"),
    );
    program.addOption(
      new Option(
  ...
  if (!options.ignorePrivateKey) {
    program.addOption(
      new Option("-p, --privateKey <privateKey>", "private key")
        .makeOptionMandatory(true)
        .env("PRIVATE_KEY"),
    );
  }
  ...
  if (options.address) {
    program.addOption(
      new Option("-a, --address <address>", "override address"),
    );
  }
