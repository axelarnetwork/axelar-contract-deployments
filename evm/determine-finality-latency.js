...
if (require.main === module) {
  ...
  program.addOption(
    new Option(
      "--update",
      "update finality setting in the chain config based on result",
    ),
  );
  program.addOption(
    new Option(
      "--confirmations <confirmations>",
      "default wait time to add if finalized tag is not supported",
    ),
  );
  program.addOption(
    new Option(
      "--attempts <attempts>",
      "number of attempts to calculate difference in block number to reach conclusion",
    ).default(200),
  );
  program.addOption(
    new Option(
      "--blockTime <blockTime>",
      "default block confirmations to wait for if finalized tag is not supported",
    ),
  );
  program.addOption(
    new Option(
      "--delay <delay>",
      "delay between calculating consecutive block finality differences",
    ).default(10),
  );
