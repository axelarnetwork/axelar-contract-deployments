  ...
  switch (contractName) {
    case "AxelarServiceGovernance": {
      ...
      if (!isNonEmptyString(governanceChain)) {
        throw new Error(
          `Missing AxelarServiceGovernance.governanceChain in the chain info.`,
        );
      }
      ...
      if (!isNonEmptyString(governanceAddress)) {
        throw new Error(
          `Missing AxelarServiceGovernance.governanceAddress in the chain info.`,
        );
      }
      ...
      if (!isNumber(minimumTimeDelay)) {
        throw new Error(
          `Missing AxelarServiceGovernance.minimumTimeDelay in the chain info.`,
        );
      }
      ...
      if (!isAddress(multisig)) {
        throw new Error(
          `Missing AxelarServiceGovernance.multisig address in the chain info.`,
        );
      }
    ...
    case "InterchainGovernance": {
      ...
      if (!isNonEmptyString(governanceChain)) {
        throw new Error(
          `Missing InterchainGovernance.governanceChain in the chain info.`,
        );
      }
      ...
      if (!isNonEmptyString(governanceAddress)) {
        throw new Error(
          `Missing InterchainGovernance.governanceAddress in the chain info.`,
        );
      }
      ...
      if (!isNumber(minimumTimeDelay)) {
        throw new Error(
          `Missing InterchainGovernance.minimumTimeDelay in the chain info.`,
        );
      }
  ...
  switch (contractName) {
    ...
    case "InterchainGovernance": {
      ...
      if (governanceChain !== contractConfig.governanceChain) {
        printError(
          `Expected governanceChain ${contractConfig.governanceChain} but got ${governanceChain}.`,
        );
      }

      const governanceChainHash = await contract.governanceChainHash();
      const expectedChainHash = keccak256(
        toUtf8Bytes(contractConfig.governanceChain),
      );

      if (governanceChainHash !== expectedChainHash) {
        printError(
          `Expected governanceChainHash ${expectedChainHash} but got ${governanceChainHash}.`,
        );
      }
      ...
      if (governanceAddress !== contractConfig.governanceAddress) {
        printError(
          `Expected governanceAddress ${contractConfig.governanceAddress} but got ${governanceAddress}.`,
        );
      }

      const governanceAddressHash = await contract.governanceAddressHash();
      const expectedAddressHash = keccak256(
        toUtf8Bytes(contractConfig.governanceAddress),
      );

      if (governanceAddressHash !== expectedAddressHash) {
        printError(
          `Expected governanceAddressHash ${expectedAddressHash} but got ${governanceAddressHash}.`,
        );
      }
      ...
      if (!minimumTimeDelay.eq(contractConfig.minimumTimeDelay)) {
        printError(
          `Expected minimumTimeDelay ${contractConfig.minimumTimeDelay} but got ${minimumTimeDelay}.`,
        );
      }
...
async function processCommand(config, chain, options) {
  ...
  const verifyOptions = verify
    ? { env, chain: chain.name, only: verify === "only" }
    : null;
  ...
  if (contractConfig.address && options.skipExisting) {
    printWarn(
      `Skipping ${contractName} deployment on ${chain.name} because it is already deployed.`,
    );
    return;
  }
  ...
  const { deployerContract, salt } = getDeployOptions(
    deployMethod,
    options.salt || contractName,
    chain,
  );
  ...
  if (await isContract(predictedAddress, provider)) {
    printWarn(
      `Contract ${contractName} is already deployed on ${chain.name} at ${predictedAddress}`,
    );
    return;
  }
  ...
  printInfo(
    `${contractName} will be deployed to`,
    predictedAddress,
    chalk.cyan,
  );
  ...
  if (existingAddress !== undefined && predictedAddress !== existingAddress) {
    printWarn(
      `Predicted address ${predictedAddress} does not match existing deployment ${existingAddress} in chain configs.`,
    );
...
if (require.main === module) {
  ...
  program
    ...
    .description("Deploy contracts using create, create2, or create3");
  ...
  program.addOption(
    new Option(
      "--ignoreError",
      "ignore errors during deployment for a given chain",
    ),
  );
