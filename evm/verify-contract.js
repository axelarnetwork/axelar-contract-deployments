...
async function verifyConsensusGateway(
  config,
  chain,
  contractConfig,
  env,
  ...
  const contractFactory = await getContractFactoryFromArtifact(
    contractJson,
    wallet,
  );
  ...
  const { addresses, weights, threshold } = await getEVMAddresses(
    ...
    {
      keyID:
        chain.contracts.AxelarGateway.startingKeyIDs[0] ||
        options.args ||
        `evm-${chain.axelarId.toLowerCase()}-genesis`,
    },
  );
  const authParams = [
...
async function verifyAmplifierGateway(
  chain,
  contractConfig,
  env,
  wallet,
  ...
  const amplifierGateway = contractFactory.attach(
    options.address || contractConfig.address,
  );
  const implementation = await amplifierGateway.implementation();
  const previousSignersRetention = (
    await amplifierGateway.previousSignersRetention()
  ).toNumber();
  const domainSeparator = await amplifierGateway.domainSeparator();
  const minimumRotationDelay = (
    await amplifierGateway.minimumRotationDelay()
  ).toNumber();
...
async function processCommand(config, chain, options) {
  ...
  const contractAddress =
    options.address || chain.contracts[contractName]?.address;
  ...
  switch (contractName) {
    ...
    case "Multisig": {
      await verifyContract(
        ...
        chain.axelarId,
        contractAddress,
        [contractConfig.signers, contractConfig.threshold],
        verifyOptions,
      );
      break;
    }
    ...
    case "BurnableMintableCappedERC20": {
      ...
      const gatewayFactory = await getContractFactoryFromArtifact(
        AxelarGateway,
        wallet,
      );
      const gateway = gatewayFactory.attach(
        chain.contracts.AxelarGateway.address,
      );

      const tokenAddress = await gateway.tokenAddresses(symbol);
      const tokenContract = contractFactory.attach(
      ...
      printInfo(
        `Verifying ${name} (${symbol}) decimals ${decimals} on ${chain.axelarId}...`,
      );
    ...
    case "InterchainTokenService": {
      ...
      const [trustedChains, trustedAddresses] =
        await getTrustedChainsAndAddresses(config, its);
      ...
      await verifyContract(
        ...
        [
          implementation,
          chain.contracts.InterchainTokenService.deployer,
          setupParams,
        ],
        {
          ...verifyOptions,
      ...
      await verifyContract(
        ...
        [
          interchainTokenFactoryImplementation,
          chain.contracts.InterchainTokenFactory.deployer,
          "0x",
        ],
        {
          ...verifyOptions,
    ...
    case "TokenManagerProxy": {
      ...
      validateParameters({
        ...
        isValidCalldata: { minter },
      });

      const InterchainTokenService = getContractJSON("InterchainTokenService");
      const interchainTokenServiceFactory =
        await getContractFactoryFromArtifact(InterchainTokenService, wallet);
      ...
      const [implementationType, tokenAddress] =
        await tokenManagerProxy.getImplementationTypeAndTokenAddress();
...
if (require.main === module) {
  ...
  program
    ...
    .description("Verify selected contracts on specified chains.");
  ...
  program.addOption(
    new Option("--minter <minter>", "interchain token minter address"),
  );
  program.addOption(
    new Option("--tokenId <tokenId>", "interchain token tokenId"),
  );
