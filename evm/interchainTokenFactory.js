...
async function processCommand(config, chain, options) {
  ...
  const interchainTokenFactoryAddress =
    address || contracts.InterchainTokenFactory?.address;
  ...
  const interchainTokenFactory = new Contract(
    interchainTokenFactoryAddress,
    IInterchainTokenFactory.abi,
    wallet,
  );
  ...
  switch (action) {
    ...
    case "interchainTokenDeploySalt": {
      ...
      const interchainTokenDeploySalt =
        await interchainTokenFactory.interchainTokenDeploySalt(
          deployer,
          deploymentSalt,
        );
      printInfo(
    ...
    case "canonicalinterchainTokenDeploySalt": {
      ...
      const canonicalinterchainTokenDeploySalt =
        await interchainTokenFactory.canonicalinterchainTokenDeploySalt(
          tokenAddress,
        );
      printInfo(
        `canonicalinterchainTokenDeploySalt for token address: ${tokenAddress}`,
    ...
    case "interchainTokenId": {
      ...
      const interchainTokenId = await interchainTokenFactory.interchainTokenId(
        deployer,
        deploymentSalt,
      );
      printInfo(
        `InterchainTokenId for deployer ${deployer} and deployment salt: ${deploymentSalt}`,
        interchainTokenId,
      );
    ...
    case "canonicalInterchainTokenId": {
      ...
      const canonicalInterchainTokenId =
        await interchainTokenFactory.canonicalInterchainTokenId(tokenAddress);
      printInfo(
        `canonicalInterchainTokenId for token address: ${tokenAddress}`,
    ...
    case "interchainTokenAddress": {
      ...
      const interchainTokenAddress =
        await interchainTokenFactory.interchainTokenAddress(
          deployer,
          deploymentSalt,
        );
      printInfo(
        `interchainTokenAddress for deployer ${deployer} and deployment salt: ${deploymentSalt}`,
    ...
    case "deployInterchainToken": {
      ...
      const tokenId = await interchainTokenFactory.interchainTokenId(
        wallet.address,
        deploymentSalt,
      );
    ...
    case "registerCanonicalInterchainToken": {
      ...
      const tx = await interchainTokenFactory.registerCanonicalInterchainToken(
        tokenAddress,
        gasOptions,
      );
    ...
    case "deployRemoteCanonicalInterchainToken": {
      ...
      ](tokenAddress, destinationChain, gasValue, {
        value: gasValue,
        ...gasOptions,
      });
    ...
    case "registerCustomToken": {
      ...
      const tokenId = await interchainTokenFactory.linkedTokenId(
        wallet.address,
        deploymentSalt,
      );
      printInfo("tokenId", tokenId);
...
if (require.main === module) {
  ...
  program
    ...
    .description("Script to perform interchain token factory commands");
  ...
  program.addOption(
    new Option("--sender <sender>", "TokenManager deployer address"),
  );
  program.addOption(new Option("--deployer <deployer>", "deployer address"));
  program.addOption(
    new Option("--tokenAddress <tokenAddress>", "token address"),
  );
  ...
  program.addOption(
    new Option("--minter <minter>", "token minter").default(AddressZero),
  );
  program.addOption(
    new Option("--operator <operator>", "token manager operator").default(
      AddressZero,
    ),
  );
  program.addOption(
    new Option("--tokenManagerType <tokenManagerType>", "token manager type"),
  );
  program.addOption(
    new Option("--initialSupply <initialSupply>", "initial supply").default(
      1e9,
    ),
  );
  program.addOption(
    new Option("--destinationChain <destinationChain>", "destination chain"),
  );
  program.addOption(
    new Option(
  ...
  program.addOption(
    new Option("--gasValue <gasValue>", "gas value").default(0),
  );
  program.addOption(
  ...
  program.addOption(
    new Option("--linkParams <linkParams>", "parameters to use for linking"),
  );
