...
async function processCommand(config, _, options, file) {
  ...
  if (transaction.status !== "SUCCESS") {
    printInfo(
      ...
      JSON.stringify(transaction.unsignedTx, null, 2),
    );

    // Send the signed transaction
    try {
      const { response, receipt } = await sendTransaction(
...
if (require.main === module) {
  ...
  program
    ...
    .description("Broadcast all the pending signed transactions of the signer");

  program.addOption(
    new Option(
      "--files [files...]",
      "The file where the signed tx are stored",
