...
async function processCommand(_, chain, options) {
  ...
  if (!offline) {
    if (balance.lte(amount)) {
      printError(
        `Wallet balance ${balance} has insufficient funds for ${amount}.`,
      );
      return;
    }
  }
...
if (require.main === module) {
  ...
  program.addOption(
    new Option(
      ...
      "comma-separated recipients of tokens",
    ).makeOptionMandatory(true),
  );
  program.addOption(
    new Option("-a, --amount <amount>", "amount to transfer (in terms of ETH)"),
  );
  program.addOption(
    new Option(
      "--gasUsage <gasUsage>",
      "amount to transfer based on gas usage and gas price",
    ).default("50000000"),
  );
