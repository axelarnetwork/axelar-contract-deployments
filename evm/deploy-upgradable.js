...
async function getImplementationArgs(contractName, config, options) {
  ...
  switch (contractName) {
    ...
    case "AxelarDepositService": {
      ...
      if (symbol === undefined) {
        throw new Error(
          `Missing AxelarDepositService.wrappedSymbol in the chain info.`,
        );
      } else if (symbol === "") {
        console.log(
          `${config.name} | AxelarDepositService.wrappedSymbol: wrapped token is disabled`,
        );
      }
      ...
      if (!isAddress(refundIssuer)) {
        throw new Error(
          `${config.name} | Missing AxelarDepositService.refundIssuer in the chain info.`,
        );
      }
...
async function processCommand(_, chain, options) {
  ...
  const verifyOptions = verifyEnv
    ? { env: verifyEnv, chain: chain.name }
    : null;

  if (
    deployMethod === "create3" &&
    (contractName === "AxelarGasService" ||
      contractName === "AxelarDepositService")
  ) {
  ...
  const proxyPath =
    artifactPath + contractName + "Proxy.sol/" + contractName + "Proxy.json";
  ...
  const implArgs = await getImplementationArgs(
    contractName,
    contracts,
    options,
  );
  ...
  if (upgrade) {
    if (!contractConfig.address) {
      throw new Error(
        `${chain.name} | Contract ${contractName} is not deployed.`,
      );
    }
    ...
    console.log(
      `${chain.name} | New Implementation for ${contractName} is at ${contractConfig.implementation}`,
    );
    ...
    if (
      predictOnly ||
      prompt(
        `Does derived address match existing deployments? Proceed with deployment on ${chain.name}?`,
        yes,
      )
    ) {
      return;
    ...
    printInfo(
      `${chain.name} | Implementation for ${contractName}`,
      contractConfig.implementation,
    );
    printInfo(
      `${chain.name} | Proxy for ${contractName}`,
      contractConfig.address,
    );
