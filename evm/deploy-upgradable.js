...
async function getAdditionalProxyArgs(contractName, chain, wallet) {
    if (contractName === 'ExpressService' || contractName === 'AxelarDepositService' || contractName === 'AxelarGasService') {
        return [];
    }
...
async function getImplementationArgs(contractName, chain, wallet) {
    ...
    if (contractName === 'ExpressService') {
        const expressOperator = get('ExpressService.expressOperator', chain);
        if (!isAddress(expressOperator)) throw new Error(`Missing ExpressService.expressOperator in the chain info.`);

        let proxyDeployer = get('ExpressService.proxyDeployer', chain);
        ...
        if (!isAddress(proxyDeployer)) {
            ...
            chain.ExpressService.proxyDeployer = proxyDeployer;

            console.log(`${chain.name} | ExpressService: deployed a new ExpressProxyDeployer at ${proxyDeployer}`);
        }

        return [chain.gateway, proxyDeployer, expressOperator];
    }
...
function getInitArgs(contractName, chain) {
    ...
    if (contractName === 'ExpressService') return '0x';
    throw new Error(`${contractName} is not supported.`);
}
