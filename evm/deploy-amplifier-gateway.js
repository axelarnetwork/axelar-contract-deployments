...
async function getSetupParams(config, chain, operator, options) {
  const { signers: signerSets, verifierSetId } = await getWeightedSigners(
    config,
    chain,
    options,
  );
...
async function deploy(config, chain, options) {
  ...
  const gatewayFactory = new ContractFactory(
    AxelarAmplifierGateway.abi,
    AxelarAmplifierGateway.bytecode,
    wallet,
  );
  const { deployerContract, salt } = getDeployOptions(
    options.deployMethod,
    options.salt || "AxelarAmplifierGateway",
    chain,
  );
  ...
  if (existingAddress !== undefined && proxyAddress !== existingAddress) {
    printWarn(
      `Predicted address ${proxyAddress} does not match existing deployment ${existingAddress} in chain configs.`,
    );
    printWarn(
  ...
  if (options.skipExisting && contractConfig.address) {
    ...
    const operator =
      options.operator || contractConfig.operator || wallet.address;
    const { params, verifierSetId } = await getSetupParams(
      config,
      chain,
      operator,
      options,
  ...
  if (
    Number(options.previousSignersRetention) !==
    (await gateway.previousSignersRetention()).toNumber()
  ) {
  ...
  if (
    minimumRotationDelay !== (await gateway.minimumRotationDelay()).toNumber()
  ) {
  ...
  if (!reuseProxy) {
    const { signers: signerSets } = await getWeightedSigners(
      config,
      chain,
      options,
    );
...
async function programHandler() {
  ...
  program
    ...
    .description("Deploy Amplifier Gateway");
  ...
  program.addOption(
    new Option(
      ...
      "minium delay for signer rotations",
    ).default(24 * 60 * 60),
  ); // 1 day
  ...
  program.addOption(
    new Option("--owner <owner>", "owner/governance address").env("OWNER"),
  );
  program.addOption(
    new Option("--operator <operator>", "gateway operator address"),
  );
  program.addOption(
    new Option(
      "--keyID <keyID>",
      "use the specified key ID address instead of the querying the chain",
    ).env("KEY_ID"),
  );
