...
const getWeightedSignaturesProof = async (
  data,
  operators,
  weights,
  threshold,
  ...
  const signatures = await Promise.all(
    signers.map((wallet) => wallet.signMessage(hash)),
  );
...
const getSignedWeightedExecuteInput = async (
  data,
  operators,
  weights,
  threshold,
...
async function processCommand(config, chain, options) {
  ...
  const gatewayAbi =
    contracts.AxelarGateway?.connectionType === "amplifier"
      ? IAxelarAmplifierGateway.abi
      : AxelarGateway.abi;
  ...
  switch (action) {
    ...
    case "operators": {
      ...
      const operators = defaultAbiCoder.encode(
      ...
      if (expectedHash !== operatorHash) {
        printError(
          `Expected operator hash ${expectedHash} but found ${operatorHash}`,
        );
      }
    ...
    case "approveAndExecute": {
      ...
      const commandID = options.commandID.startsWith("0x")
        ? options.commandID
        : id(parseInt(options.commandID).toString());
      ...
      const data = buildCommandBatch(
        ...
        [
          getApproveContractCall(
            chain.axelarId,
            walletAddress,
            options.destination || walletAddress,
            payloadHash,
            id(""),
            0,
          ),
      ...
      const signedData = await getSignedWeightedExecuteInput(
        data,
        [walletAddress],
        [1],
        1,
    ...
    case "approveAndExecute": {
      ...
      if (options.messageId) {
        ...
        commandID = options.commandID.startsWith("0x")
          ? options.commandID
          : id(parseInt(options.commandID).toString());
      }
    ...
    case "transferGovernance": {
      const newGovernance =
        options.destination || chain.contracts.InterchainGovernance?.address;
      ...
      if (
        prompt(
          `Proceed with governance transfer to ${chalk.cyan(newGovernance)}`,
          yes,
        )
      ) {
        return;
    ...
    case "transferMintLimiter": {
      const newMintLimiter =
        options.destination || chain.contracts.Multisig?.address;
      ...
      if (
        prompt(
          `Proceed with mint limiter transfer to ${chalk.cyan(newMintLimiter)}`,
          yes,
        )
      ) {
        return;
    ...
    case "mintLimit": {
      ...
      printInfo(
        `Gateway mintLimit ${options.symbol}`,
        await gateway.tokenMintLimit(options.symbol),
      );
      printInfo(
        `Gateway mint amount ${options.symbol}`,
        await gateway.tokenMintAmount(options.symbol),
      );
      break;
    }
    ...
    case "rotateSigners": {
      ...
      const gateway = new Contract(
        gatewayAddress,
        getContractJSON("AxelarAmplifierGateway").abi,
        wallet,
      );
    ...
    case "submitProof": {
      ...
      const { status } = await getMultisigProof(
        config,
        chain.axelarId,
        multisigSessionId,
      );
...
if (require.main === module) {
  ...
  program.addOption(
    new Option("--payloadHash <payloadHash>", "GMP payload hash"),
  );
  program.addOption(
    new Option("--commandID <commandID>", "execute command ID"),
  );
  program.addOption(
    new Option("--messageId <messageId>", "GMP call message ID"),
  );
  program.addOption(
    new Option("--sourceChain <sourceChain>", "GMP source chain"),
  );
  program.addOption(
    new Option("--sourceAddress <sourceAddress>", "GMP source address"),
  );
  program.addOption(
    new Option("--destination <destination>", "GMP destination address"),
  );
  program.addOption(
    new Option(
      "--destinationChain <destinationChain>",
      "GMP destination chain",
    ),
  );
  program.addOption(
    new Option("--batchID <batchID>", "EVM batch ID").default(""),
  );
  program.addOption(new Option("--symbol <symbol>", "EVM token symbol"));
  program.addOption(
    new Option(
      "--multisigSessionId <multisigSessionId>",
      "Amplifier multisig proof session ID",
    ),
  );
