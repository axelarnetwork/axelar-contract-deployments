...
async function processCommand(config, chain, options) {
  ...
  const operatorsContract = new Contract(
    operatorsAddress,
    getContractJSON("IOperators").abi,
    wallet,
  );
  ...
  switch (action) {
    case "isOperator": {
      ...
      printInfo(
        `Is ${operatorAddress} an operator on ${chain.name}?`,
        `${isOperator}`,
      );
    ...
    case "addOperator": {
      ...
      await operatorsContract
        .addOperator(operatorAddress, gasOptions)
        .then((tx) => tx.wait());
    ...
    case "removeOperator": {
      ...
      await operatorsContract
        .removeOperator(operatorAddress, gasOptions)
        .then((tx) => tx.wait());
    ...
    case "collectFees": {
      ...
      if (tokens.length !== amounts.length) {
        throw new Error(
          "Token addresses and token amounts have a length mismatch.",
        );
      }
      ...
      const gasServiceInterface = new Interface(
        getContractJSON("IAxelarGasService").abi,
      );
      const collectFeesCalldata = gasServiceInterface.encodeFunctionData(
        "collectFees",
        [receiver, tokens, amounts],
      );

      try {
        await operatorsContract
          .executeContract(target, collectFeesCalldata, 0, gasOptions)
          .then((tx) => tx.wait());
      } catch (error) {
        printError(error);
      }
    ...
    case "refund": {
      ...
      const refundCalldata = gasServiceInterface.encodeFunctionData("refund", [
        txHash,
        logIndex,
        receiver,
        token,
      ...
      try {
        await operatorsContract
          .executeContract(target, refundCalldata, 0, gasOptions)
          .then((tx) => tx.wait());
      } catch (error) {
        printError(error);
      }
    ...
    case "updateGasInfo": {
      ...
      const updateGasInfoCalldata = gasServiceInterface.encodeFunctionData(
        "updateGasInfo",
...
if (require.main === module) {
  ...
  program.addOption(
    new Option("--ignoreError", "Ignore errors and proceed to next chain"),
  );
