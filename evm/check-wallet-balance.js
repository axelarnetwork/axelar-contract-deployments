...
async function processCommand(_, chain, options) {
  ...
  const gasPrice = BigNumber.from(
    chain.eip1559 ? staticGasOptions.maxFeePerGas : staticGasOptions.gasPrice,
  );
  const minRequiredBalance = gasLimit * gasPrice * 1.5;
  printError(
    `${chain.name} minimum required Balance`,
    `${minRequiredBalance / 1e18}`,
  );
  ...
  for (const address of addresses) {
    ...
    if (balance < minRequiredBalance) {
      printError(
        `${chain.name} Wallet Balance for ${address} is`,
        `${balance / 1e18}`,
      );
    }
  }
...
if (require.main === module) {
  ...
  program
    ...
    .description(
      "Before offline signing checks if each signer has minimum required wallet balance",
    );
  ...
  program.addOption(
    new Option(
      "--addresses <addresses>",
      "The Array of addresses for which the balance to check",
    ).env("ADDRESSES"),
  );
