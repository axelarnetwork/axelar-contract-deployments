...
const deployCreate2 = async (
  ...
  if (!verifyOptions?.only) {
    ...
    contract = {
      address: await predictContractConstant(
        constAddressDeployerAddress,
        wallet,
        contractJson,
        salt,
...
const deployCreate3 = async (
  ...
  if (!verifyOptions?.only) {
    ...
    contract = {
      address: await getCreate3Address(create3DeployerAddress, wallet, key),
    };
  }
...
function validateParameters(parameters) {
  for (const [validatorFunctionString, paramsObj] of Object.entries(
    parameters,
  )) {
    ...
    for (const paramKey of Object.keys(paramsObj)) {
      ...
      if (!isValid) {
        throw new Error(
          `Input validation failed for ${validatorFunctionString} with parameter ${paramKey}: ${paramValue}`,
        );
      }
    }
  }
...
const getDeployOptions = (deployMethod, salt, chain) => {
  ...
  if (deployMethod === "create2") {
    deployer =
      chain.contracts.ConstAddressDeployer?.address ||
      chain.contracts.Create2Deployer?.address;
  } else {
...
const getDeployedAddress = async (deployer, deployMethod, options = {}) => {
  switch (deployMethod) {
    ...
    case "create2": {
      ...
      salt = keccak256(
        defaultAbiCoder.encode(["address", "bytes32"], [deployer, salt]),
      );
...
const getProxy = async (config, chain) => {
  const address = (
    await httpGet(
      `${config.axelar.lcd}/axelar/evm/v1beta1/gateway_address/${chain}`,
    )
  ).address;
  return address;
};
...
const getEVMBatch = async (config, chain, batchID = "") => {
  const batch = await httpGet(
    `${config.axelar.lcd}/axelar/evm/v1beta1/batched_commands/${chain}/${batchID}`,
  );
  return batch;
};

const getAmplifierVerifiers = async (config, chain) => {
  const { verifierSetId, verifierSet, signers } = await getCurrentVerifierSet(
    config,
    chain,
  );
...
const getEVMAddresses = async (config, chain, options = {}) => {
  ...
  const evmAddresses = options.amplifier
    ...
    : await httpGet(
        `${config.axelar.lcd}/axelar/evm/v1beta1/key_address/${chain}?key_id=${keyID}`,
      );

  const sortedAddresses = evmAddresses.addresses.sort((a, b) =>
    a.address.toLowerCase().localeCompare(b.address.toLowerCase()),
  );

  const addresses = sortedAddresses.map(
    (weightedAddress) => weightedAddress.address,
  );
  const weights = sortedAddresses.map((weightedAddress) =>
    Number(weightedAddress.weight),
  );
...
async function printWalletInfo(wallet, options = {}, chain = {}) {
  ...
  if (!options.offline) {
    ...
    if (balance.isZero()) {
      ...
      printInfo(
        "Wallet balance",
        `${balance / 1e18} ${chain.tokenSymbol || ""}`,
      );
    }

    printInfo(
      "Wallet nonce",
      (await wallet.provider.getTransactionCount(address)).toString(),
    );
  }
...
const deployContract = async (
  ...
  const predictedAddress = await getDeployedAddress(
    wallet.address,
    deployMethod,
    {
  ...
  switch (deployMethod) {
    case "create": {
      const contract = await deployCreate(
        wallet,
        contractJson,
        constructorArgs,
        gasOptions,
...
const mainProcessor = async (
  ...
  const chainsToSkip = (options.skipChains || "")
    .split(",")
  ...
  if (options.chainNames === "all") {
    ...
    chains = chains.filter(
      (chain) =>
        !config.chains[chain].chainType ||
        config.chains[chain].chainType === "evm",
    );
  ...
  if (options.startFromChain) {
    const startIndex = chains.findIndex(
      (chain) => chain === options.startFromChain.toLowerCase(),
    );

    if (startIndex === -1) {
      throw new Error(
        `Chain ${options.startFromChain} is not defined in the info file`,
      );
    }
  ...
  if (options.parallel && chains.length > 1) {
    ...
    if (save) {
      for (const chainName of successfullChains) {
        config.chains[chainName.toLowerCase()] = loadParallelExecutionConfig(
          options.env,
          chainName,
        );
      }
...
async function getGasOptions(
  ...
  if (gasOptionsCli) {
    ...
    gasOptions = copyObject(
      contractConfig?.staticGasOptions ||
        chain?.staticGasOptions ||
        defaultGasOptions,
    );
  } else {
    gasOptions = copyObject(
      contractConfig?.gasOptions || chain?.gasOptions || defaultGasOptions,
    );
  }
...
async function handleGasPriceAdjustment(chain, gasOptions) {
  ...
  if (gasPriceAdjustment && !gasOptions.gasPrice) {
    try {
      ...
      gasOptions.gasPrice = Math.floor(
        (await provider.getGasPrice()) * gasPriceAdjustment,
      );
    } catch (err) {
      throw new Error(
        `Provider failed to retrieve gas price on chain ${chain.name}: ${err}`,
      );
    }
  }
...
function isValidChain(config, chainName) {
  ...
  const validChain = Object.values(chains).some(
    (chainObject) => chainObject.axelarId === chainName,
  );
...
async function getWeightedSigners(config, chain, options) {
  ...
  if (isValidAddress(options.keyID)) {
    ...
    const nonce = hexZeroPad(
      BigNumber.from(addresses.created_at).toHexString(),
      32,
    );

    signers = {
      signers: addresses.addresses.map(({ address, weight }) => ({
        signer: address,
...
const verifyContractByName = (
  ...
  verifyContract(env, chain, contract, args, {
    ...options,
    contractPath: getQualifiedContractName(name),
  });
};

const isConsensusChain = (chain) =>
  chain.contracts.AxelarGateway?.connectionType !== "amplifier";
