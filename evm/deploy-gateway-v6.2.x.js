...
async function checkKeyRotation(config, chain) {
  ...
  try {
    resp = await httpGet(
      `${config.axelar.lcd}/axelar/multisig/v1beta1/next_key_id/${chain}`,
    );
  } catch (err) {
    return;
  }
...
async function getAuthParams(config, chain, options) {
  ...
  if (options.prevKeyIDs) {
    for (const keyID of options.prevKeyIDs.split(",")) {
      const { addresses, weights, threshold } = await getEVMAddresses(
        config,
        chain,
        { ...options, keyID },
      );
      printInfo(
        JSON.stringify({ status: "old", keyID, addresses, weights, threshold }),
      );
      params.push(
        defaultAbiCoder.encode(
          ["address[]", "uint256[]", "uint256"],
          [addresses, weights, threshold],
        ),
      );
      keyIDs.push(keyID);
    }
  ...
  const { addresses, weights, threshold, keyID } = await getEVMAddresses(
    config,
    chain,
    options,
  );
  printInfo(
    JSON.stringify({ status: "latest", keyID, addresses, weights, threshold }),
  );
  params.push(
    defaultAbiCoder.encode(
      ["address[]", "uint256[]", "uint256"],
...
async function deploy(config, chain, options) {
  ...
  const governance =
    options.governance ||
    chain.contracts.InterchainGovernance?.address ||
    wallet.address;
  const mintLimiter =
    options.mintLimiter || chain.contracts.Multisig?.address || wallet.address;
  ...
  const gatewayFactory = new ContractFactory(
    AxelarGateway.abi,
    AxelarGateway.bytecode,
    wallet,
  );
  const authFactory = new ContractFactory(
    AxelarAuthWeighted.abi,
    AxelarAuthWeighted.bytecode,
    wallet,
  );
  ...
  const gatewayProxyFactory = new ContractFactory(
    AxelarGatewayProxy.abi,
    AxelarGatewayProxy.bytecode,
    wallet,
  );
  const { deployerContract } = getDeployOptions(
    options.deployMethod,
    options.salt || "AxelarGateway v6.2",
    chain,
  );
  ...
  if (reuseProxy) {
    proxyAddress =
      ...
      (await getProxy(config, chain.axelarId));
  ...
  const existingAddress =
    config.chains.arbitrum?.contracts?.[contractName]?.address;
  ...
  if (await isContract(proxyAddress, wallet.provider)) {
    printError(
      `Contract already deployed at predicted address "${proxyAddress}"!`,
    );
  }
  ...
  if (
    predictOnly ||
    prompt(
      `Does derived address match existing gateway deployments? Proceed with deployment on ${chain.name}?`,
      yes,
    )
  ) {
    return;
  ...
  if (options.skipExisting && contractConfig.authModule) {
    ...
    const { params, keyIDs } = await getAuthParams(
      config,
      chain.axelarId,
      options,
    );
  ...
  const implementationCodehash = await getBytecodeHash(
    implementation,
    chain.axelarId,
  );
  ...
  if (!(reuseProxy && (reuseHelpers || reuseAuth))) {
    ...
    await auth
      .transferOwnership(gateway.address, { gasLimit: 5e6, ...gasOptions })
  ...
  try {
    ...
    printWarn(
      `WARN: Failed to retrieve governance address. Expected when reusing a gateway <v6 proxy`,
    );
  }
  ...
  try {
    ...
    // this can fail when upgrading from an older version
    printWarn(
      `WARN: Failed to retrieve mint limiter address. Expected when reusing a gateway <v6 proxy`,
    );
  }
  ...
  if (!reuseProxy && authModule !== auth.address) {
    printError(
      `ERROR: Auth module retrieved from gateway ${authModule} doesn't match deployed contract ${auth.address}`,
    );
    error = true;
  }
  ...
  if (authOwner !== gateway.address) {
    printError(
      `ERROR: Auth module owner is set to ${authOwner} instead of proxy address ${gateway.address}`,
    );
    error = true;
  }
...
async function upgrade(_, chain, options) {
  ...
  let governance =
    options.governance || chain.contracts.InterchainGovernance?.address;
  ...
  if (!offline) {
    ...
    const codehash = await getBytecodeHash(
  ...
  const tx = await gateway.populateTransaction.upgrade(
    contractConfig.implementation,
  ...
  const { baseTx, signedTx } = await signTransaction(
    wallet,
    chain,
    tx,
    options,
...
async function programHandler() {
  ...
  program.addOption(
    new Option(
      "--reuseProxy",
      "reuse proxy contract modules for new implementation deployment",
    ),
  );
  program.addOption(
    new Option(
      "--reuseHelpers",
      "reuse helper auth and token deployer contract modules for new implementation deployment",
    ),
  );
  program.addOption(
    new Option(
      "--reuseAuth",
      "reuse auth module contract for new implementation deployment",
    ),
  );
  program.addOption(
    new Option(
      "--ignoreError",
      "Ignore deployment errors and proceed to next chain",
    ),
  );
  program.addOption(
    new Option("--governance <governance>", "governance address").env(
      "GOVERNANCE",
    ),
  );
  program.addOption(
    new Option("--mintLimiter <mintLimiter>", "mint limiter address").env(
      "MINT_LIMITER",
    ),
  );
  ...
  program.addOption(
    new Option(
      ...
      "previous key IDs to be used for auth contract",
    ),
  );
