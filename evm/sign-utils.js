...
const signTransaction = async (wallet, chain, tx, options = {}) => {
  if (!tx.to || !isAddress(tx.to)) {
    throw new Error(
      "Target address is missing/not provided as valid address for the tx in function arguments",
    );
  }
  ...
  if (!options.offline) {
    ...
    if (tx.nonce === undefined) {
      ...
      if (tx.nonce === undefined) {
        throw new Error(
          `Nonce is missing for ${address} on ${chain.name} in nonces.json`,
        );
      }
    }
    ...
    if (!tx.gasLimit) {
      throw new Error(
        "Gas limit is missing/not provided for the tx in function arguments",
      );
    }
    ...
    if (
      !tx.gasPrice &&
      !(
        isValidNumber(tx.maxFeePerGas) &&
        (tx.maxPriorityFeePerGas === undefined ||
          isNumber(tx.maxPriorityFeePerGas))
      )
    ) {
      throw new Error(
        "Gas price (legacy or eip-1559) is missing/not provided for the tx in function arguments",
      );
    }
...
function getSignedTx(filePath) {
  ...
  try {
    ...
    printError(
      `Failed to get all signers data from the file ${filePath}`,
      error,
    );
    throw error;
  }
