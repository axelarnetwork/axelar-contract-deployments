...
async function processCommand(config, chain, options) {
  ...
  const argsAxelarDepositService = JSON.stringify({
    wrappedSymbol,
    refundIssuer,
  });
  ...
  for (let i = 0; i < cmds.length; i++) {
    execSync(
      `${cmds[i]} -n ${options.chainNames} -p ${options.privateKey} ${options.yes ? "-y" : ""}`,
      {
        stdio: "inherit",
      },
    );
...
if (require.main === module) {
  ...
  program
    ...
    .description("Deploy contracts to test deployment on chain");
  program.addOption(
  ...
  program.addOption(
    new Option(
      ...
      "include AxelarDepositService in deployment tests",
    )
