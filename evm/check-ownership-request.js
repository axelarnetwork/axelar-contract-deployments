...
async function processCommand(config, options) {
  ...
  for (let i = 0; i < data.length; ++i) {
    ...
    printInfo(
      `Verifying data at index ${i + 2} for Token address`,
      tokenAddress,
    );

    const destinationChainsRaw = row[destinationChainsRowIndex].split(",");
    const destinationChains = destinationChainsRaw
    ...
    const invalidDestinationChains = await verifyChains(
      config,
      tokenAddress,
      destinationChains,
    );
    const validDestinationChains = destinationChains.filter(
      (chain) => !invalidDestinationChains.includes(chain),
    );

    if (validDestinationChains.length > 0) {
      finalData[i - totalRowsRemoved][destinationChainsRowIndex] =
        validDestinationChains.length === 1
          ? `${validDestinationChains[0]}`
          : `"${validDestinationChains.join(", ")}"`;
    } else {
...
async function getSenderDustTx(dustTx, chains) {
  if (!dustTx.startsWith("https") && !dustTx.startsWith("0x")) {
    throw new Error(
      'Invalid dustTx format. It must start with "https" or "0x".',
    );
  }
...
function updateCSVFile(filePath, columnNames, data) {
  ...
  const csvContent = [columnNames, ...data]
    .map((row) => row.join(","))
    .join("\n");
...
async function createCsvFile(filePath, data) {
  ...
  const columnNames = [
    "Token Address",
    "Chains to claim token ownership on",
    "Telegram Contact details",
  ];
  const selectedColumns = [
    tokenAddressRowIndex,
    destinationChainsRowIndex,
  ...
  const csvContent = [columnNames, ...filteredData]
    .map((row) => row.join(","))
    .join("\n");
  writeCSVData(filePath, csvContent);
}
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option(
        "-f, --file <file>",
        "The csv file path containing details about pending token ownership requests",
      )
