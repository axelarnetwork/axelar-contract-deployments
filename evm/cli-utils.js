...
const addEvmOptions = (program, options = {}) => {
  ...
  if (options.artifactPath) {
    program.addOption(
      new Option("--artifactPath <artifactPath>", "artifact path"),
    );
  }
  ...
  if (options.salt) {
    program.addOption(
      new Option("-s, --salt <salt>", "salt to use for create2 deployment").env(
        "SALT",
      ),
    );
  }
  ...
  if (options.skipExisting) {
    program.addOption(
      new Option(
        "-x, --skipExisting",
        "skip existing if contract was already deployed on chain",
      ).env("SKIP_EXISTING"),
    );
  }
  ...
  if (options.upgrade) {
    program.addOption(
      new Option("-u, --upgrade", "upgrade a deployed contract").env("UPGRADE"),
    );
  }
