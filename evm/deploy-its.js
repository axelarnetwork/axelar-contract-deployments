...
async function deployAll(config, wallet, chain, options) {
  ...
  const interchainTokenService = options.reuseProxy
    ...
    : await getDeployedAddress(wallet.address, proxyDeployMethod, {
        salt: proxySalt,
        deployerContract: getDeployOptions(proxyDeployMethod, proxySalt, chain)
          .deployerContract,
  ...
  if (options.reuseProxy) {
    printInfo(
      "Reusing existing Interchain Token Service proxy",
      interchainTokenService,
    );
  } else {
    printInfo(
      "Interchain Token Service will be deployed to",
      interchainTokenService,
    );
  }
  ...
  const interchainTokenFactory = options.reuseProxy
    ...
    : await getDeployedAddress(wallet.address, proxyDeployMethod, {
        salt: factorySalt,
        deployerContract: getDeployOptions(
          proxyDeployMethod,
          factorySalt,
          chain,
        ).deployerContract,
  ...
  if (!isValidAddress(interchainTokenFactory)) {
    throw new Error(
      `Invalid Interchain Token Factory address: ${interchainTokenFactory}`,
    );
  }

  if (options.reuseProxy) {
    printInfo(
      "Reusing existing Interchain Token Factory proxy",
      interchainTokenFactory,
    );
  } else {
    printInfo(
      "Interchain Token Factory will be deployed to",
      interchainTokenFactory,
    );
  }
  ...
  const itsChains = Object.values(config.chains).filter(
    (chain) =>
      chain.chainType === "evm" &&
      chain.contracts?.InterchainTokenService?.address,
  );
  ...
  const existingAddress =
    config.chains.ethereum?.contracts?.[contractName]?.address;

  if (
    existingAddress !== undefined &&
    interchainTokenService !== existingAddress
  ) {
    printWarn(
    ...
    const existingCodeHash =
      config.chains.ethereum.contracts[contractName].predeployCodehash;
  ...
  const deployments = {
    ...
    interchainTokenFactory: {
      ...
      async deploy() {
        const args = [
          itsFactoryContractConfig.implementation,
          wallet.address,
          "0x",
        ];
  ...
  for (const key in deployments) {
    ...
    if (
      options.reuseProxy &&
      ["InterchainToken", "InterchainProxy"].includes(deployment.contractName)
    ) {
    ...
    if (!(await isContract(contract.address, provider))) {
      throw new Error(
        `Contract ${deployment.name} at ${contract.address} was not deployed on ${chain.name}`,
      );
    }
  }
}
...
async function upgrade(_, chain, options) {
  ...
  if (currImplementation === contractConfig.implementation) {
    ...
    if (
      predictOnly ||
      prompt(`Proceed with ITS upgrade on ${chain.name}?`, options.yes)
    ) {
      return;
    }
  ...
  const InterchainTokenFactory = getContractJSON(
    "InterchainTokenFactory",
    artifactPath,
  );
  const itsFactory = new Contract(
    itsFactoryContractConfig.address,
    InterchainTokenFactory.abi,
    wallet,
  );
  const factoryCodehash = await getBytecodeHash(
    itsFactoryContractConfig.implementation,
    chain.axelarId,
    provider,
  );
  ...
  printInfo(
    `New ITS Factory implementation`,
    itsFactoryContractConfig.implementation,
  );
...
if (require.main === module) {
  ...
  program
    .name("deploy-its")
    .description(
      "Deploy interchain token service and interchain token factory",
    );
  ...
  program.addOption(
    new Option(
      "--reuseProxy",
      "reuse existing proxy (useful for upgrade deployments",
    ),
  );
  program.addOption(
    new Option("--contractName <contractName>", "contract name").default(
      "InterchainTokenService",
    ),
  ...
  program.addOption(
    new Option(
      "-s, --salt <salt>",
      "deployment salt to use for ITS deployment",
    ).env("SALT"),
  );
  program.addOption(
    new Option(
  ...
  program.addOption(
    new Option(
      ...
      "address of the ITS operator/rate limiter",
