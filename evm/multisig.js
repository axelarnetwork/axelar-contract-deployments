...
async function preExecutionChecks(
  ...
  if (action === "withdraw") {
    topic = multisigContract.interface.encodeFunctionData("withdraw", [
      target,
      nativeValue,
    ]);
  } else if (action === "executeMultisigProposal") {
    topic = multisigContract.interface.encodeFunctionData(
      "executeMultisigProposal",
      [target, calldata, nativeValue],
    );
  } else {
    topic = multisigContract.interface.encodeFunctionData("executeContract", [
      target,
      calldata,
      nativeValue,
    ]);
  ...
  if (voteCount.eq(0)) {
    printWarn(
      `The vote count for this topic is zero. This action will create a new multisig proposal.`,
    );
  ...
  if (voteCount.eq(threshold.sub(1))) {
    printWarn(
      `The vote count is one below the threshold. This action will execute the multisig proposal.`,
    );
...
async function processCommand(_, chain, options) {
  ...
  switch (action) {
    ...
    case "setTokenMintLimits": {
      ...
      const multisigCalldata = gateway.interface.encodeFunctionData(
        "setTokenMintLimits",
        [symbolsArray, limitsArray],
      );
      ...
      tx = await multisigContract.populateTransaction.executeContract(
        multisigTarget,
        multisigCalldata,
        0,
        gasOptions,
    ...
    case "withdraw": {
      ...
      tx = await multisigContract.populateTransaction.withdraw(
        recipient,
        amount,
      );
      break;
    ...
    case "executeMultisigProposal": {
      if (!isValidAddress(target)) {
        throw new Error(
          `Invalid target for execute multisig proposal: ${target}`,
        );
      }

      if (!isValidCalldata(calldata)) {
        throw new Error(
          `Invalid calldata for execute multisig proposal: ${calldata}`,
        );
      }
      ...
      if (!isNumber(parseFloat(nativeValue))) {
        throw new Error(
          `Invalid native value for execute multisig proposal: ${nativeValue}`,
        );
      }
      ...
      if (!isValidAddress(governance)) {
        throw new Error(
          `Missing AxelarServiceGovernance address in the chain info.`,
        );
      }
      ...
      tx = await governanceContract.populateTransaction.executeMultisigProposal(
        target,
        calldata,
        nativeValue,
      );
    ...
    case "setFlowLimits": {
      ...
      if (!isValidAddress(multisigTarget)) {
        throw new Error(
          `Missing InterchainTokenService address in the chain info.`,
        );
      }
      ...
      const multisigCalldata = its.interface.encodeFunctionData(
        "setFlowLimits",
        [tokenIdsArray, limitsArray],
      );
      ...
      if (!offline) {
        ...
        if (!hasOperatorRole) {
          throw new Error(
            "Missing Operator role for the used multisig address.",
          );
        }

        // loop over each token
        for (let i = 0; i < tokenIdsArray.length; ++i) {
          const tokenManagerAddress = await its.validTokenManagerAddress(
            tokenIdsArray[i],
          );
...
if (require.main === module) {
  ...
  program.addOption(
    new Option("--symbols <symbols>", "token symbols").makeOptionMandatory(
      false,
    ),
  );
  program.addOption(
    new Option("--limits <limits>", "token limits").makeOptionMandatory(false),
  );
  ...
  program.addOption(
    new Option(
      "--target <target>",
      "execute multisig proposal target",
    ).makeOptionMandatory(false),
  );
  program.addOption(
    new Option(
      "--calldata <calldata>",
      "execute multisig proposal calldata",
    ).makeOptionMandatory(false),
  );
  program.addOption(
    new Option(
      "--nativeValue <nativeValue>",
      "execute multisig proposal nativeValue",
    )
      .makeOptionMandatory(false)
      .default(0),
  );
