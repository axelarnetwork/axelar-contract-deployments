  ...
  switch (targetContractName) {
    case "AxelarGateway": {
      ...
      if (currGovernance !== governance.address) {
        printWarn(
          `Gateway governor ${currGovernance} does not match governance contract: ${governance.address}`,
        );
      }
      ...
      let newMintLimiter =
        options.newMintLimiter || contracts.Multisig?.address || AddressZero;
      ...
      if (newGovernance !== AddressZero || newMintLimiter !== AddressZero) {
        setupParams = defaultAbiCoder.encode(
          ...
          [newGovernance, newMintLimiter, "0x"],
        );
      }
...
async function getProposalCalldata(governance, chain, wallet, options) {
  ...
  switch (action) {
    ...
    case "upgrade": {
      const implementation =
        ...
        (targetContractName === "AxelarGateway"
          ? chain.contracts[targetContractName]?.implementation
          : "");

      if (!isValidAddress(implementation)) {
        throw new Error(
          `Invalid new implementation address: ${implementation}\nDo you need to pass in 'targetContractName'?`,
        );
      }
      ...
      if (currImplementation === implementation) {
        printWarn(
          `Current implementation ${currImplementation} matches new implementation ${implementation}`,
        );
      }

      const newImplementationCodeHash = await getBytecodeHash(
        implementation,
        chain.axelarId,
        provider,
      );
      ...
      const setupParams = await getSetupParams(
        governance,
        targetContractName,
        target,
        chain.contracts,
      ...
      calldata = upgradable.interface.encodeFunctionData("upgrade", [
        implementation,
    ...
    case "transferGovernance": {
      const newGovernance =
        options.newGovernance || chain.contracts.InterchainGovernance?.address;

      if (!isValidAddress(newGovernance)) {
        throw new Error(
          `Invalid new gateway governance address: ${newGovernance}`,
        );
      }
      ...
      calldata = gateway.interface.encodeFunctionData("transferGovernance", [
        newGovernance,
      ]);
      break;
    }
    ...
    case "withdraw": {
      ...
      calldata = governance.interface.encodeFunctionData("withdraw", [
        options.target,
        amount,
      ]);
  ...
  if (!isValidAddress(target)) {
    throw new Error(
      `Target address required for this governance action: ${action}`,
    );
  }
...
async function processCommand(_, chain, options) {
  ...
  let { target, calldata, title, description } = await getProposalCalldata(
    governance,
    chain,
    wallet,
    options,
  ...
  switch (proposalAction) {
    ...
    case "schedule": {
      if (!isValidTimeFormat(date)) {
        throw new Error(
          `Invalid ETA: ${date}. Please pass the eta in the format YYYY-MM-DDTHH:mm:ss`,
        );
      }
      ...
      const minEta =
        currTime + (await governance.minimumTimeLockDelay()).toNumber();
      ...
      printInfo(
        "Time difference between current time and eta",
        etaToDate(eta - currTime),
      );

      const existingProposalEta = await governance.getProposalEta(
        target,
        calldata,
        nativeValue,
      );
      ...
      if (!existingProposalEta.eq(BigNumber.from(0))) {
        throw new Error(
          `Proposal already exists with eta: ${existingProposalEta}.`,
        );
      }
    ...
    case "cancel": {
      ...
      gmpPayload = getGovernanceProposal(
    ...
    case "scheduleMultisig": {
      if (contractName === "InterchainGovernance") {
        throw new Error(
          `Invalid governance action for InterchainGovernance: ${action}`,
        );
      }

      if (!calldata) {
        throw new Error(
          `Calldata required for this governance action: ${action}`,
        );
      }
    ...
    case "submitMultisig": {
      ...
      const eventEmitted = wasEventEmitted(
        receipt,
        governance,
        "ProposalScheduled",
      );
    ...
    case "execute": {
      ...
      const tx = await governance.executeProposal(
        target,
        calldata,
        nativeValue,
        gasOptions,
  ...
  if (commandType !== -1) {
    ...
    const payloadBase64 = Buffer.from(`${gmpPayload}`.slice(2), "hex").toString(
      "base64",
    );
...
if (require.main === module) {
  ...
  program
    .name("governance")
    .description("Script to manage interchain governance actions");
  ...
  program.addOption(
    new Option("--file <file>", "file to write Axelar proposal JSON to"),
  );
  program.addOption(
    new Option("--newGovernance <governance>", "governance address").env(
      "GOVERNANCE",
    ),
  );
  program.addOption(
    new Option("--newMintLimiter <mintLimiter>", "mint limiter address").env(
      "MINT_LIMITER",
    ),
  );
  program.addOption(new Option("--commandId <commandId>", "command id"));
  program.addOption(
    new Option("--target <target>", "governance execution target"),
  );
  program.addOption(new Option("--calldata <calldata>", "calldata"));
  program.addOption(
    new Option("--nativeValue <nativeValue>", "nativeValue").default(0),
  );
  program.addOption(
    new Option("--proposal <proposal>", "governance proposal payload"),
  );
