...
async function getCallData(
  ...
  switch (action) {
    case "withdraw": {
      ...
      const callData = targetContract.interface.encodeFunctionData("withdraw", [
        recipient,
        amount,
      ]);
      return callData;
    ...
    case "transfer": {
      ...
      const callData = targetContract.interface.encodeFunctionData("transfer", [
        recipient,
        amount,
      ]);
      return callData;
    ...
    case "approve": {
      ...
      const callData = targetContract.interface.encodeFunctionData("approve", [
        recipient,
        amount,
      ]);
      return callData;
...
async function executeContract(options, chain, wallet) {
  ...
  if (action === "default") {
    ...
    const ITargetContract = getContractJSON(
      targetContractName,
      targetContractPath,
    );
    ...
    finalCallData = await getCallData(
      action,
      targetContract,
      recipientAddress,
      Number(amount),
...
if (require.main === module) {
  ...
  program.addOption(
    new Option(
      "-c, --callContractName <callContractName>",
      "name of the called contract",
    ).makeOptionMandatory(true),
  );
  program.addOption(
    new Option(
      "--callContractPath <callContractPath>",
      "artifact path for the called contract",
    ),
  );
  program.addOption(
    new Option(
      "--targetContractPath <targetContractPath>",
      "artifact path for the target contract",
    ),
  );
  program.addOption(
    new Option(
      "-t, --targetContractName <targetContractName>",
      "target contract name called by executeContract",
    ).makeOptionMandatory(true),
  );
  program.addOption(
    new Option(
      "-a, --targetContractAddress <targetContractAddress>",
  ...
  program.addOption(
    new Option("--callData <callData>", "The calldata to be sent")
      .env("CALL_DATA")
      .default("0x"),
  );
  program.addOption(
