...
async function getTrustedChainsAndAddresses(config, interchainTokenService) {
  ...
  const itsHubAddress =
    config.axelar?.contracts?.InterchainTokenService?.address;
  ...
  const trustedAddressesValues = await Promise.all(
    allChains.map(
      async (chainName) =>
        await interchainTokenService.trustedAddress(chainName),
    ),
  );
  const trustedChains = allChains.filter(
    (_, index) => trustedAddressesValues[index] !== "",
  );
  const trustedAddresses = trustedAddressesValues.filter(
    (address) => address !== "",
  );
...
function compare(contractValue, configValue, variableName) {
  contractValue = isNonEmptyString(contractValue)
    ? contractValue.toLowerCase()
    : contractValue;
  configValue = isNonEmptyString(configValue)
...
function compareToConfig(contractConfig, contractName, toCheck) {
  for (const [key, value] of Object.entries(toCheck)) {
    if (contractConfig[key]) {
      ...
      printWarn(
        `Warning: The key '${key}' is not found in the contract config for ${contractName}.`,
      );
    }
  }
}
...
async function processCommand(config, chain, action, options) {
  ...
  const interchainTokenServiceAddress =
    address || contracts.InterchainTokenService?.address;

  if (!interchainTokenServiceAddress) {
    printWarn(
      `No InterchainTokenService address found for chain ${chain.name}`,
    );
    return;
  }
  ...
  switch (action) {
    ...
    case "token-manager-address": {
      ...
      const tokenIdBytes32 = hexZeroPad(
        tokenId.startsWith("0x") ? tokenId : "0x" + tokenId,
        32,
      );

      const tokenManagerAddress =
        await interchainTokenService.tokenManagerAddress(tokenIdBytes32);
      printInfo(
        `TokenManager address for tokenId: ${tokenId}`,
        tokenManagerAddress,
      );
      ...
      try {
        ...
        printInfo(
          `TokenManager for tokenId: ${tokenId} exists at address:`,
          tokenManagerAddress,
        );
      } catch (error) {
    ...
    case "interchain-token-address": {
      ...
      const interchainTokenAddress =
        await interchainTokenService.interchainTokenAddress(tokenIdBytes32);
      printInfo(
        `InterchainToken address for tokenId: ${tokenId}`,
      ...
      try {
        ...
        printInfo(
          `Token for tokenId: ${tokenId} exists at address:`,
          interchainTokenAddress,
        );
      } catch (error) {
    ...
    case "interchain-token-id": {
      ...
      const interchainTokenId = await interchainTokenService.interchainTokenId(
        sender,
        deploymentSalt,
      );
      printInfo(
        `InterchainTokenId for sender ${sender} and deployment salt: ${deploymentSalt}`,
        interchainTokenId,
      );
    ...
    case "token-manager-implementation": {
      const tokenManagerImplementation =
        await interchainTokenService.tokenManager();
      printInfo(
        `TokenManager implementation address`,
    ...
    case "flow-limit": {
      ...
      printInfo(
        `Flow limit for TokenManager with tokenId ${tokenId}`,
        flowLimit,
      );
    ...
    case "flow-out-amount": {
      ...
      printInfo(
        `Flow out amount for TokenManager with tokenId ${tokenId}`,
        flowOutAmount,
      );
    ...
    case "flow-in-amount": {
      ...
      printInfo(
        `Flow in amount for TokenManager with tokenId ${tokenId}`,
        flowInAmount,
      );
    ...
    case "contract-call-value": {
      ...
      const isTrustedAddress = await interchainTokenService.isTrustedAddress(
        sourceChain,
        sourceAddress,
      );
      ...
      const [tokenAddress, tokenAmount] =
        await interchainTokenService.contractCallValue(
          sourceChain,
          sourceAddress,
          payload,
        );
      printInfo(
        `Amount of tokens with address ${tokenAddress} that the call is worth:`,
        tokenAmount,
      );
    ...
    case "express-execute": {
      ...
      const tx = await interchainTokenService.expressExecute(
        commandID,
        sourceChain,
        sourceAddress,
        payload,
    ...
    case "interchain-transfer": {
      ...
      if (
        (await interchainTokenService.trustedAddress(destinationChain)) === ""
      ) {
        throw new Error(
          `Destination chain ${destinationChain} is not trusted by ITS`,
        );
      }
      ...
      const implementationType = (
        await tokenManager.implementationType()
      ).toNumber();
      const decimals = await token.decimals();
      const amountInUnits = BigNumber.from(amount).mul(
      ...
      if (balance.lt(amountInUnits)) {
        throw new Error(
          `Insufficient balance for transfer. Balance: ${balance}, amount: ${amountInUnits}`,
        );
      }
      ...
      if (
        ...
        printInfo(
          "Approving ITS for a transfer for token with token manager type",
          implementationType,
        );
        await token
          .approve(interchainTokenService.address, amountInUnits, gasOptions)
          .then((tx) => tx.wait());
      }
    ...
    case "register-token-metadata": {
      ...
      const tx = await interchainTokenService.registerTokenMetadata(
        tokenAddress,
        gasValue,
        { value: gasValue, ...gasOptions },
      );
    ...
    case "set-flow-limits": {
      ...
      const tx = await interchainTokenService.setFlowLimits(
        tokenIdsBytes32,
        flowLimits,
        gasOptions,
      );
    ...
    case "trusted-address": {
      ...
      const trustedAddress =
        await interchainTokenService.trustedAddress(trustedChain);
    ...
    case "set-trusted-chains": {
      ...
      if (owner.toLowerCase() !== walletAddress.toLowerCase()) {
        throw new Error(
          `${action} can only be performed by contract owner: ${owner}`,
        );
      }
      ...
      if (itsChain === "all") {
        ...
        trustedAddresses = trustedChains.map(
          (_) => itsAddress || chain.contracts?.InterchainTokenService?.address,
        );
      } else {
        const trustedChain =
          getChainConfig(config, itsChain.toLowerCase(), { skipCheck: true })
            ?.axelarId || itsChain.toLowerCase();
        const trustedAddress =
          itsAddress ||
          getChainConfig(config, itsChain.toLowerCase())?.contracts
            ?.InterchainTokenService?.address;

        validateParameters({
          isNonEmptyString: { trustedChain, trustedAddress },
        });
      ...
      if (
        prompt(
          `Proceed with setting trusted address for chain ${trustedChains} to ${trustedAddresses}?`,
          yes,
        )
      ) {
        return;
      ...
      for (const [trustedChain, trustedAddress] of trustedChains.map(
        (chain, index) => [chain, trustedAddresses[index]],
      )) {
        const tx = await interchainTokenService.setTrustedAddress(
          trustedChain,
          trustedAddress,
          gasOptions,
        );
    ...
    case "remove-trusted-chains": {
      ...
      if (itsChain === "all") {
        [trustedChains] = await getTrustedChainsAndAddresses(
          config,
          interchainTokenService,
        );
      } else {
      ...
      for (const trustedChain of trustedChains) {
        const tx = await interchainTokenService.removeTrustedAddress(
          trustedChain,
          gasOptions,
        );
        await handleTx(
    ...
    case "set-pause-status": {
      ...
      const tx = await interchainTokenService.setPauseStatus(
        pauseStatus === "true",
        gasOptions,
      );
    ...
    case "checks": {
      ...
      const interchainTokenDeployer =
        await interchainTokenService.interchainTokenDeployer();
      const interchainTokenFactory =
        await interchainTokenService.interchainTokenFactory();

      const interchainTokenFactoryContract = new Contract(
        interchainTokenFactory,
        InterchainTokenFactory.abi,
        wallet,
      );
      const interchainTokenFactoryImplementation =
        await interchainTokenFactoryContract.implementation();

      const interchainTokenDeployerContract = new Contract(
        interchainTokenDeployer,
        IInterchainTokenDeployer.abi,
        wallet,
      );
      ...
      const [trustedChains, trustedAddresses] =
        await getTrustedChainsAndAddresses(config, interchainTokenService);
      ...
      for (let i = 0; i < trustedAddresses.length; i++) {
        ...
        if (
          (isConsensusChain(chain) && isConsensusChain(chainConfig)) ||
          chainConfig.axelarId === config.axelar.axelarId
        ) {
      ...
      const toCheck = {
        tokenManagerDeployer:
          await interchainTokenService.tokenManagerDeployer(),
        interchainTokenDeployer,
        interchainToken,
      ...
      compareToConfig(
        itsFactoryContractConfig,
        itsFactoryContractName,
        toCheckFactory,
      );
    ...
    case "link-token": {
      ...
      try {
        ...
        printInfo(
          `TokenManager for tokenId ${tokenId} exists on the current chain`,
          tokenManagerAddress,
        );

        const sourceTokenAddress =
          await interchainTokenService.registeredTokenAddress(tokenId);
        printInfo(
          `Token address on current chain for tokenId ${tokenId}`,
          sourceTokenAddress,
        );
      } catch (error) {
        printError(
          `TokenManager for tokenId ${tokenId} does not yet exist on the current chain.`,
        );
        return;
      }
      ...
      if (
        prompt(
          `Proceed with linking tokenId ${tokenId} to ${destinationTokenAddress} on chain ${destinationChain}?`,
          yes,
        )
      ) {
        return;
    ...
    case "add-trusted-chains": {
      ...
      const isAmplifier =
        chainConfig?.contracts?.AxelarGateway?.connectionType === "amplifier";
...
async function main(action, args, options) {
  options.args = args;
  await mainProcessor(options, (config, chain, options) =>
...
if (require.main === module) {
  ...
  program
    ...
    .addOption(
      new Option("--rawSalt <rawSalt>", "raw deployment salt").env("RAW_SALT"),
    )
    .addOption(
      new Option("--metadata <metadata>", "token transfer metadata").default(
        "0x",
      ),
    )
    .addOption(new Option("--gasValue <gasValue>", "gas value").default(0))
    .action(
      (destinationChain, tokenId, destinationAddress, amount, options, cmd) => {
        main(
          cmd.name(),
          [destinationChain, tokenId, destinationAddress, amount],
          options,
        );
      },
    );
  ...
  program
    ...
    .action(
      (
        ...
        main(
          cmd.name(),
          [tokenId, destinationChain, destinationTokenAddress, type, operator],
          options,
        );
      },
    );
  ...
  program
    ...
    .argument("<trusted-chain>", "Trusted chain")
    .addOption(
      new Option("--trustedAddress <trustedAddress>", "Trusted Address"),
    )
    .action((trustedChain, options, cmd) => {
      main(cmd.name(), [trustedChain], options);
    });
