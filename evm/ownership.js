...
async function processCommand(_, chain, options) {
  ...
  const ownershipContract = new Contract(
    ownershipAddress,
    IOwnable.abi,
    wallet,
  );
  ...
  switch (action) {
    ...
    case "transferOwnership": {
      ...
      if (owner.toLowerCase() !== wallet.address.toLowerCase()) {
        throw new Error(
          `Caller ${wallet.address} is not the contract owner but ${owner} is.`,
        );
      }
      ...
      try {
        await ownershipContract
          .transferOwnership(newOwner, gasOptions)
          .then((tx) => tx.wait());
      } catch (error) {
    ...
    case "proposeOwnership": {
      ...
      try {
        await ownershipContract
          .proposeOwnership(newOwner, gasOptions)
          .then((tx) => tx.wait());
      } catch (error) {
        throw new Error(error);
      }
