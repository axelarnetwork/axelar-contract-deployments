...
async function processCommand(_, chain, options) {
  ...
  const provider = rpc
    ? getDefaultProvider(rpc)
    : getDefaultProvider(chain.rpc);

  if (
    prompt(
      `Proceed with the nonces update on network ${chalk.green(chain.name)}`,
      yes,
    )
  ) {
    return;
...
if (require.main === module) {
  ...
  program.addOption(
    new Option(
      "--addresses <addresses>",
      "The Array of addresses for which the nonces to update",
    ).env("ADDRESSES"),
  );
