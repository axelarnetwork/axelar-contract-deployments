...
async function processCommand(_, chain, options) {
  ...
  if (
    prompt(
      `Proceed with the static gasOption update on ${chalk.green(chain.name)}`,
      yes,
    )
  ) {
    return;
  ...
  if (chain.eip1559) {
    ...
    gasPriceWei = BigNumber.from(baseFee).add(
  ...
  const minGasLimit =
    (minGasLimits[env] || {})[chain.name.toLowerCase()] || defaultGasLimit;

  if (
    !(chain.staticGasOptions && chain.staticGasOptions.gasLimit !== undefined)
  ) {
    chain.staticGasOptions = { gasLimit: minGasLimit };
  }

  const minGasPrice =
    ((minGasPrices[env] || {})[chain.name.toLowerCase()] || 0) * 1e9;
...
if (require.main === module) {
  ...
  program
    .name("update-static-gas-options")
    .description("Update staticGasOptions");
  ...
  program.addOption(
    new Option(
      "-r, --rpc <rpc>",
      "The rpc url for creating a provider to fetch gasOptions",
    ),
  );
