...
[Release](https://github.com/Trust-Machines/axelar_stacks_bridge/releases/tag/1.1.0)
...
```bash
wget -O axelar_stacks_bridge_v1.1.0.zip https://github.com/Trust-Machines/axelar_stacks_bridge/archive/refs/tags/1.1.0.zip
shasum -a 256 axelar_stacks_bridge_v1.1.0.zip
```

Check that the checksum matches:

```
11bcd2dc3bb241f552e70acda9066a6a27569717507b84b33c817a9492f64f35  axelar_stacks_bridge_v1.1.0.zip
```

Then copy the contracts to the `./stacks/contracts` folder:

```bash
unzip axelar_stacks_bridge_v1.1.0.zip
cp -r axelar_stacks_bridge-1.1.0/contracts/. ./stacks/contracts/
rm axelar_stacks_bridge_v1.1.0.zip
rm -rf axelar_stacks_bridge-1.1.0
```
...
```bash
ts-node stacks/deploy-contract.js --contract gas-impl --version "1.1.0" --basePath ./stacks/contracts
```
...
```bash
ts-node stacks/deploy-contract.js --contract gateway-impl --version "1.1.0" --basePath ./stacks/contracts
```
...
```bash
ts-node stacks/deploy-contract.js --contract interchain-token-factory-impl --version "1.1.0" --basePath ./stacks/contracts
```
...
```bash
ts-node stacks/deploy-contract.js --contract interchain-token-service-impl --version "1.1.0" --basePath ./stacks/contracts
```
...
Initially, the owner of the contracts will be able to upgrade them. Eventually, the Governance contract should be responsible for upgrading contracts, and the owner will transfer ownership to the Governance contract.

This will be done as follows:

1. Setup Governance contract

The Governance contract should be already deployed as per [Deployment: Phase 1](#deployment-phase-1)
...
The owner (initial deployer) of the contracts can change the ownership to the new Governance contract

```bash
ts-node stacks/governance.js set-owner Gateway "[governance address]"
```

```bash
ts-node stacks/governance.js set-owner InterchainTokenFactory "[governance address]"
```

```bash
ts-node stacks/governance.js set-owner InterchainTokenService "[governance address]"
```
