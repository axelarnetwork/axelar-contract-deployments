...
shasum -a 256 axelar_stacks_bridge_v1.0.0.zip
```

Check that the checksum matches:

```
c1643a0c116f9200ee437dbf1a358f4fea51d91163ee1d8151f4d79a29a594fe  axelar_stacks_bridge_v1.0.0.zip
```
...
```bash
ts-node stacks/prepare-contracts.js
```
...
```bash
ts-node stacks/deploy-contract.js --contract clarity-stacks
```

#### gas-storage

```bash
ts-node stacks/deploy-contract.js --contract gas-storage
```

#### traits

```bash
ts-node stacks/deploy-contract.js --contract traits
```

#### gas-impl

```bash
ts-node stacks/deploy-contract.js --contract gas-impl --version "1.0.0"
```

#### gateway-storage

```bash
ts-node stacks/deploy-contract.js --contract gateway-storage
```

#### gas-service

```bash
ts-node stacks/deploy-contract.js --contract gas-service
```

#### gateway-impl

```bash
ts-node stacks/deploy-contract.js --contract gateway-impl --version "1.0.0"
```

#### gateway

```bash
ts-node stacks/deploy-contract.js --contract gateway
```

#### governance

```bash
ts-node stacks/deploy-contract.js --contract governance
```

#### interchain-token-service-storage

```bash
ts-node stacks/deploy-contract.js --contract interchain-token-service-storage
```

#### interchain-token-factory

```bash
ts-node stacks/deploy-contract.js --contract interchain-token-factory
```

#### interchain-token-service

```bash
ts-node stacks/deploy-contract.js --contract interchain-token-service
```

#### interchain-token-factory-impl

```bash
ts-node stacks/deploy-contract.js --contract interchain-token-factory-impl --version "1.0.0"
```

#### verify-onchain

```bash
ts-node stacks/deploy-contract.js --contract verify-onchain
```

#### interchain-token-service-impl

```bash
ts-node stacks/deploy-contract.js --contract interchain-token-service-impl --version "1.0.0"
```
...
```bash
ts-node stacks/deploy-contract.js --contract native-interchain-token
```
...
1. Deploy `sample-sip-010` token to use for testing (or use another name for the token if repeating the checklist)
...
```bash
ts-node stacks/setup-contract.js TokenManager --name TokenManagerSample --token "[address of the token]"
```

3. Register the token manager on Stacks ITS contract

```bash
ts-node stacks/its-example.js register-token-manager TokenManagerSample
```

4. Deploy the token on another chain

```bash
ts-node stacks/its-example.js deploy-remote-canonical-interchain-token TokenManagerSample --destinationChain "[destination chain]" --gasValue "1000000" # 1 STX for cross chain gas
```
...
```bash
ts-node stacks/its-example.js interchain-transfer TokenManagerSample --destinationChain "[destination chain]" --destinationAddress "[destination-address]" --value "10000000" --gasValue "1000000" # send 10 SMPL tokens & pay 1 STX for cross chain gas 
```
...
ts-node evm/its.js interchain-transfer -n [source-chain] $CHAIN [token-id] [decoded-recipient-address] [amount]
```
