...
```bash
npm run script evm/deploy-amplifier-gateway.js -m create --minimumRotationDelay 86400
```
...
```bash
npm run script evm/deploy-upgradable.js -c AxelarGasService -m create2 --args '{"collector": "0x7D
...
```bash
npm run script evm/ownership.js -c AxelarGateway --action transferOwnership --newOwner 0x6f24A47F
