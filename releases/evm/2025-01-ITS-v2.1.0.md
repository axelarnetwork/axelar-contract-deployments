...
node evm/deploy-its.js "v2.1.0 devnet-amplifier" create2 --reuseProxy
...
node evm/deploy-its.js "v2.1.0 consensus devnet-amplifier" create2 --reuseProxy
...
```bash
# Deploy new implementation
node evm/deploy-its.js "v2.1.0" create2 --reuseProxy
...
node evm/its.js register-token-metadata [tokenAddress] 0
```
...
node evm/its.js transfer-mintership [tokenAddress] [tokenManager]
```

- [ ]  Interchain Transfer (both ways)

```bash
node evm/its.js interchain-transfer [tokenId] chain2 [recipient] 1 0x 0
```
