...
> [!NOTE]  
> `tokenManager` address is different per token, which can be found on tx output from `registerCustomToken` action on chain
...
> [!NOTE]  
> `tokenId` can be fetched from `registerCustomToken` tx output.
> This is NOT the same `tokenId` from `deployInterchainToken` action
