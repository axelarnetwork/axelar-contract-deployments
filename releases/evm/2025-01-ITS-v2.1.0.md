...
npm run script evm/deploy-its.js -s "v2.1.0 devnet-amplifier" -m create2 --reuseProxy

# Upgrade proxy
npm run script evm/deploy-its.js --upgrade
```
...
npm run script evm/deploy-its.js -s "v2.1.0 consensus devnet-amplifier" -m create2 --reuseProxy
...
```bash
# Deploy new implementation
npm run script evm/deploy-its.js -s "v2.1.0" -m create2 --reuseProxy
...
```bash
npm run script evm/its.js --action checks -n $CHAIN -y
```
...
npm run script evm/interchainTokenFactory.js --action deployInterchainToken --minter [wallet] --name "test" --symbol "TST" --decimals [decimals] --initialSupply 10000 --salt "salt12345"

# Register token metadata. Ensure GMP call is executed
npm run script evm/its.js --action registerTokenMetadata --tokenAddress [tokenAddress]
```
...
npm run script evm/interchainTokenFactory.js --action registerCustomToken --tokenAddress [tokenAddress] --tokenManagerType 4 --operator [wallet] --salt "salt6789"

# Link to remote token. Ensure GMP call is executed
npm run script evm/interchainTokenFactory.js --action linkToken --destinationChain chain2 --destinationTokenAddress [remote token address] --tokenManagerType 4 --linkParams "0x" --salt "salt6789"
```
...
npm run script evm/its.js --action tokenManagerAddress --tokenId [tokenId]
```
...
npm run script evm/its.js --action transferMintership --tokenAddress [tokenAddress] --minter [tokenManager]
```

- Interchain Transfer (both ways)

```bash
npm run script evm/its.js --action interchainTransfer --destinationChain chain2 --tokenId [tokenId] --destinationAddress [recipient] --amount 1 --gasValue 0
```
