...
2. **Deploy ASG if the chain doesn't have one** (`governanceChain=axelar`, `governanceAddress`=gov module,
   `minimumTimeDelay`, `operator`=emergency EOA):
   ```bash
...
> When committing each chain's records: stage ONLY the canonical fields the deploy wrote
> (`implementation`/`implementationCodehash`/`owner` + the `AxelarServiceGovernance` block). Do NOT commit
> incidental local config mutations (RPC endpoints with API keys, `gasLimit`, `deployer`) — scrub the diff.

## Status — testnet EVM amplifier chains
| Chain | Upgraded to pausable | ASG deployed | owner = ASG | Notes |
|---|---|---|---|---|
| berachain | ✅ `0x39fD…9F49` | ✅ `0x848aEc…4b0D` | ✅ | from pre-release build; re-verify codehash vs v6.2.0; currently **paused**; ASG `minimumTimeDelay=60` is a test value |
| flow | ⬜ | ⬜ | ⬜ | |
| hedera | ⬜ | ⬜ | ⬜ | |
| xrpl-evm | ⬜ | ⬜ | ⬜ | |
| plume | ⬜ | ⬜ | ⬜ | |
