...
__Note:__ While Axelar nodes can be upgraded to `v1.3.2` at upgrade height, Axelar validators are recommended to upgrade to `v1.3.4` to address an issue with `vald` where it sends too many heartbeat txs.

- **Building from source:** If building from source, use `Go 1.25`. The recommended approach is static linking with `make build-static`, which eliminates additional libwasmvm handling. As a fallback, if you need a dynamically linked binary using `make build`, make sure to update libwasmvm from `v1.5.8` to `v2.2.4`.
- **Latest Prebuilt binaries:** Available on the [release page](https://github.com/axelarnetwork/axelar-core/releases/tag/v1.3.4). For Linux, we recommend the statically linked binary [axelard-linux-amd64-v1.3.4-static](https://github.com/axelarnetwork/axelar-core/releases/download/v1.3.4/axelard-linux-amd64-v1.3.4-static) which runs on any Linux distribution without requiring libwasmvm to be installed on your system.
- Prepare to [update ampd to 1.13.2] at the same time
- Ensure your system has at least **64GB of RAM** to handle the upgrade process. If running in environments with memory limits (e.g., Kubernetes), configure limits appropriately to avoid OOM-kills.
...
3. Verify the `vald` broadcaster key exists in your keyring: `axelard keys show broadcaster --keyring-backend file`. If missing, add it with `axelard keys add broadcaster --recover --keyring-backend file`.
4. Update `app.toml`:
    - Add the following parameters at the root level (required for optimal migration performance):
...
6. (For verifiers only) Update the `config.toml` for `ampd`
...
- Under the `tofnd_config` make sure you update the configuration:
  Please remove the current configuration:
    ...
    And replace it with timeout="3s" under [tofnd_config] key:
...
- At the root of the `ampd` `config.toml` please update the [tm_grpc_timeout] config:
    ...
    And replace it with the following at `config.toml` root:

    ```toml
    tm_grpc_timeout="5s"
    ```

7. Restart the validator node, `vald`, and `ampd` with the new binaries.
   When restarting `axelard` do not do a rollback. Node will begin a migration which will take a while to complete.
   Also note that you can expect to see this error during the migration, which is **completely normal** and can be ignored:
    ...
    ```
   **Important:** Do not restart the node during migration, as any restart will cause the migration to begin again. Block production will resume once quorum of validators by stake have completed the upgrade, which can add 30–90 minutes to the overall migration duration.
8. Please also upgrade any other Axelar nodes you operate (RPC nodes etc.), and restart your IBC relayer (e.g. hermes).
...
3. If you see the message `migrating module auth from version 2 to version 3`, it means the upgrade has started and is progressing normally.

4. If you hit the error: `undefined symbol: migrate_with_info`. Switch to using the statically linked binary ([axelard-linux-amd64-v1.3.4-static](https://github.com/axelarnetwork/axelar-core/releases/download/v1.3.4/axelard-linux-amd64-v1.3.4-static)). If you need to use a dynamically linked binary, make sure you have the correct `libwasmvm` version (`2.2.4`)

5. If you hit the error: `ERR failed to get consensus params err="collections: not found: key 'no_key' of type`. No action is necessary - this is expected during preblock so the upgrade plan can be executed and the consensus params set for the first time in the new format.

6. Required RAM is 64GB. If your process runs out of memory during the upgrade you will have to increase your system RAM to at least `64GB` and the upgrade will restart from the beginning and progress will be lost

7. If you see the error `No records were found in keyring`, this means your broadcaster account is missing from the keyring. Add it with `axelard keys add broadcaster --recover --keyring-backend file`

8. If you hit an error saying: `failed to execute message: too many heartbeats received from operator in vald` upgrade to the axelard v1.3.4 version.
