...
- **Building from source:** If building from source, use Go 1.25. The recommended approach is static linking with `make build-static`, which creates a binary that doesn't require libwasmvm. As a fallback, if you need a dynamically linked binary, make sure to update libwasmvm from `v1.5.8` to `v2.2.4`.
