...
- **Archival nodes:** ~12.5 hours migration + catch-up time. Peak memory: ~64GB.
- **Validator/RPC nodes:** ~1 hour migration + catch-up time. Peak memory: ~64GB.

1. Wait for upgrade height to be reached.
2. Stop the validator `axelard` node, `vald`, and `ampd` (if you are running a verifier).
3. Verify the broadcaster key exists in your keyring: `axelard keys list --keyring-backend file`. If missing, add it with `axelard keys add broadcaster --recover --keyring-backend file`.
...
4. If you hit the error: `undefined symbol: migrate_with_info`. Switch to using the statically linked binary ([axelard-linux-amd64-v1.3.2-static](https://github.com/axelarnetwork/axelar-core/releases/download/v1.3.2/axelard-linux-amd64-v1.3.2-static)). If you need to use a dynamically linked binary, make sure you have the correct `libwasm` version (`2.2.4`)
...
7. If you see the error `No records were found in keyring`, this means your broadcaster account is missing from the keyring. Add it with `axelard keys add broadcaster --recover --keyring-backend file`. If after adding the key into the keyring you encounter the same error, add `--keyring-backend file` to the command you are executing when the error is encountered. 
...
- **Archival node:** Migration took **~12.5 hours**, plus additional catch-up time.
    - Peak memory usage observed: **~64 GB**
...
- **Be aware that migration memory usage may reach ~64 GB.**  
