...
- **Prebuilt binaries:** Available on the `v1.3.2` [release page](https://github.com/axelarnetwork/axelar-core/releases/tag/v1.3.2). For Linux, we recommend the statically linked binary [axelard-linux-amd64-v1.3.2-static](https://github.com/axelarnetwork/axelar-core/releases/download/v1.3.2/axelard-linux-amd64-v1.3.2-static) which runs on any Linux distribution without requiring libwasmvm to be installed on your system.
...
5. (For verifiers only) Update the `config.toml` for `ampd`
...
6. Restart the validator node, `vald`, and `ampd` with the new binaries.
...
4. If you hit the error: `undefined symbol: migrate_with_info`. Switch to using the statically linked binary ([axelard-linux-amd64-v1.3.2-static](https://github.com/axelarnetwork/axelar-core/releases/download/v1.3.2/axelard-linux-amd64-v1.3.2-static)). If you need to use a dynamically linked binary, make sure you have the correct `libwasmvm` version (`2.2.4`)
...
8. If you hit an error saying: `failed to execute message: too many heartbeats received from operator in vald`. This just requires waiting for your node to catch up. We recommend starting vald only once the node has fully caught up and is producing blocks
