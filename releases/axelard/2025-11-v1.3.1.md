...
2. If you run in to the error: 
```bash
Upgraded the node to v1.3.1 but stuck on `migrating module auth from version 2 to version 3`
and ampd upgraded v1.13.1`
```
Please wait 10-15 minutes, this error will resolve on its own.
3. If you hit the error `Upgrading IAVL storage for faster queries`. Then please make sure the following is added to your `config.toml` file at the root:
```bash
iavl-cache-size = 0
iavl-disable-fastnode = true
```
You will also need to revert to the previous block. If you hit this case wait time for the upgrade to go through may be close to an hour.
4. If you hit the error: `undefined symbol: migrate_with_info`. Then please make sure you are using the correct `libwasm` version (`2.2.4`)
5. If you hit the error: `ERR failed to get consensus params err="collections: not found: key 'no_key' of type`. No action is necessary so that during preblock the upgrade plan can be executed and the consensus params set for the first time in the new format.
