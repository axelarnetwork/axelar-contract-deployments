...
3. Update axelar-core node config (`config.toml`):

   | Parameter | Old Value (v1.2.4) | New Value (v1.3.1) |
   | --------- | ------------------ | ------------------ |
   | `timeout_commit` | `5s` | `1s` |

4. Update vald config (alternatively, remove these entries entirely to use the new defaults):

   | Section | Parameter | Old Value (v1.2.4) | New Value (v1.3.1) |
   | ------- | --------- | ------------------ | ------------------ |
   | (root) | `max_blocks_behind_latest` | `10` | `50` |
   | `[broadcast]` | `max_retries` | `3` | `5` |
   | `[broadcast]` | `min_sleep_before_retry` | `5s` | `1s` |
   | `[broadcast]` | `max_timeout` | `15s` | `10s` |

5. Restart the node and ampd with the new binaries. Please note that the migration will take a while to complete (~1h on testnet)
