...
4. (For verifiers only) Update the `config.toml` for `ampd`
  - Under the `tofnd_config` make sure you update the configuration:
    ...
    And replace it with the `timeout="3s"`:
...
7. If your process runs out of memory during the upgrade, increase your system RAM to at least `64GB`.
...
#### Causes of Restarts Identified
1. **Container memory limits too low**
   - Nodes configured with **16–24 GB** memory were OOM-killed when migration memory usage peaked at **~42 GB**.
   - These OOM events triggered restarts, forcing migrations to restart from scratch.

2. **Liveness probe configuration**
   - A liveness probe with an `initialDelaySeconds` of **3600 seconds (1 hour)** caused Kubernetes to restart the node once migration exceeded that duration.
   - This also resulted in the migration restarting from the beginning.

#### Action Items for Validators / RPC Operators  
- **Be aware that migration memory usage may reach ~40–42 GB.**  
  Nodes with lower memory limits (e.g., 16–24 GB) may be OOM-killed during this phase.

- **Review liveness probe settings** if using Kubernetes.  
  A probe assuming migrations finish within 1 hour may incorrectly restart the node.
