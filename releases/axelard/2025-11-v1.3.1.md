...
4. (For verifiers only) Update the `config.toml` for `ampd`
  - Under the `tofnd_config` make sure you have remove the current configuration and replace it with the following:
    ```toml
    [tofnd_config]
  ...
  - At the root of the `ampd` `config.toml` please remove the old [tm_grpc_timeout] config and replace it with the following: 
    ```toml
    tm_grpc_timeout="5s"
    ```
