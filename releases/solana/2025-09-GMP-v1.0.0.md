...
| -------------------- | --------------------- | ---------- |
| **Devnet Amplifier** | Completed             | 2025-10-28 |
...
| -------------------- | ----------- | -------------- | --------------------------------------------- |
| **Devnet Amplifier** | Gateway     | **Devnet**     | `gt2EEXhF49mAGhi6Fxsuwwuoz5Dpx7daAXAWE8sLZ77` |
| **Devnet Amplifier** | Gas Service | **Devnet**     | `ga265e933y15ANN43gRD94Gd676sZ7YNbMYyQJQjrc9` |
| **Devnet Amplifier** | Governance  | **Devnet**     | `govrP5KY8epG6rTi456P2XJXGz2cRwd7oXUi2eGAWgW` |
| **Devnet Amplifier** | Multicall   | **Devnet**     | `mcTZhkVD7emqaop8CBRWXSf1HuuLGK2S9Dvt3fm45BK` |
| **Devnet Amplifier** | Memo        | **Devnet**     | `me2G9aTaYPvYjuSxjsMKmbBiYXs4ydUvDwP1SwkUV7F` |
...
1. Compile the Solana programs:
    ...
    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_gas_service_v2
    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_gateway_v2
    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_memo_v2
...
1. Declare additional environment variables in your `.env`:
    ...
    GATEWAY_PROGRAM_PATH="axelar-amplifier-solana/target/deploy/axelar_solana_gateway_v2.so"

    GAS_SERVICE_PROGRAM_KEYPAIR_PATH="<path/to/gas_service_program_keypair.json>"
    GAS_SERVICE_PROGRAM_PATH="axelar-amplifier-solana/target/deploy/axelar_solana_gas_service_v2.so"

    MEMO_PROGRAM_KEYPAIR_PATH="<path/to/memo_program_keypair.json>"
    MEMO_PROGRAM_PATH="axelar-amplifier-solana/target/deploy/axelar_solana_memo_v2.so"
...
1. Deploy and verify axelar_solana_gateway_v2 program:

    ```sh
    anchor deploy -p axelar_solana_gateway_v2 --provider.cluster $CLUSTER --program-keypair $GATEWAY_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH

    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_gateway_v2
    anchor verify -p axelar_solana_gateway_v2 --provider.cluster $CLUSTER $(solana address -k $GATEWAY_PROGRAM_KEYPAIR_PATH) -- --no-default-features --features $ENV
    ```

1. Deploy and verify axelar_solana_gas_service_v2 program:

    ```sh
    anchor deploy -p axelar_solana_gas_service_v2 --provider.cluster $CLUSTER --program-keypair $GAS_SERVICE_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH

    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_gas_service_v2
    anchor verify -p axelar_solana_gas_service_v2 --provider.cluster $CLUSTER $(solana address -k $GAS_SERVICE_PROGRAM_KEYPAIR_PATH) -- --no-default-features --features $ENV
    ```

1. Deploy and verify axelar_solana_memo_v2 program:

    ```sh
    anchor deploy -p axelar_solana_memo_v2 --provider.cluster $CLUSTER --program-keypair $MEMO_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH

    solana-verify build --base-image $BASE_IMAGE --library-name axelar_solana_memo_v2
    anchor verify -p axelar_solana_memo_v2 --provider.cluster $CLUSTER $(solana address -k $MEMO_PROGRAM_KEYPAIR_PATH) -- --no-default-features --features $ENV
    ```
