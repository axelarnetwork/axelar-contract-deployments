...
1. Deploy and verify `axelar_solana_operators` program:

    ```sh
    anchor deploy -p axelar_solana_operators --provider.cluster $CLUSTER --program-keypair $OPERATORS_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH

    anchor verify -p axelar_solana_operators --provider.cluster $CLUSTER $(solana address -k $OPERATORS_PROGRAM_KEYPAIR_PATH) -- --no-default-features --features $ENV
    ```

1. Deploy and verify `axelar_solana_memo` program:

    ```sh
    anchor deploy -p axelar_solana_memo --provider.cluster $CLUSTER --program-keypair $MEMO_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH

    anchor verify -p axelar_solana_memo --provider.cluster $CLUSTER $(solana address -k $MEMO_PROGRAM_KEYPAIR_PATH) -- --no-default-features --features $ENV
    ```
