...
| -------------------- | --------------------- | ---------- | --------------- |
| **Devnet Amplifier** | Completed             | 2025-11-03 | @nbayindirli    |
| **Stagenet**         | Completed             | 2026-03-04 | @rista404       |
...
| -------------------- | -------------- | --------------------------------------------- |
| **Devnet Amplifier** | **Devnet**     | `its9RfZWZJxUmKfEv5SNyr3nWWt36n7nPfJ2Cu1ocGc` |
| **Stagenet**         | **Testnet**    | `itsm3zZhp2oGgEfq7XBu9ojRCYZJnhzecbAEPCrvx2B` |
| **Testnet**          | **Devnet**     | `itsJo4kNJ3mdh3requwbtTTt7vyYTudp1pxhn2KiHMc`  |
| **Mainnet**          | **Mainnet**    | ``                                            |
...
1. Ensure you have access to the keypairs for the following accounts, unless you plan to generate new keypairs (below):
    ...
    | **Stagenet**         | `upaYdfY8H5dKa3wCV2pnfxEVNP1yT6mJycoZo3BJFFK` | `gopuMNntD2aFtuVnwskU7jYpmdBdLXJMBpZWZjYE1sj` |
    | **Testnet**          | `upaHiTygNXvZZzAb9ZZxn63LhJoma35BKgt6rxH5XdS` | `gopdSWQFLKvxPz1SiP2W8CumFBtd2FF1ddDgNVkLmdG` |
...
1. Deploy `solana_axelar_its` program:

    ```sh
    solana/cli deploy \
        --program its \
        --version <VERSION> \
        --program-keypair $ITS_PROGRAM_KEYPAIR_PATH \
...
1. Initialize ITS:
    ...
    ```sh
    solana/cli send -s $OPERATOR_KEYPAIR_PATH its init \
...
1. Register Solana ITS on ITS Hub:
    ...
    - For Devnet-amplifier / Stagenet / Testnet / Mainnet
            ...
            "contracts": {
                "InterchainTokenService": {
                    "$CHAIN": {
                        ...
                        "msgTranslator": "<ItsSolanaTranslator address>"
                    }
                }
            }
        }
        ...
        > **Note:** The `msgTranslator` should be set to the `ItsSolanaTranslator` contract address deployed in the [Solana GMP Amplifier](../cosmwasm/2025-09-Solana-GMP-v1.0.0.md) release. If not specified, it defaults to `ItsAbiTranslator` which is incorrect for Solana.

        Register $CHAIN on ITS Hub (for new chains):
        ...
        Or update existing chain registration (e.g., to add/change the translator):

        ```bash
        ts-node cosmwasm/contract.ts its-hub-update-chains $CHAIN \
            -m $MNEMONIC \
            --governance # omit on devnet-amplifier
        ```
...
1. Switch `PRIVATE_KEY` to Solana keypair.
...
1. Deploy Native Interchain Token:
    ...
    solana/cli send its deploy-interchain-token \
        ...
        --initial-supply <TOKEN_INITIAL_SUPPLY>
...
1. Interchain Token Transfer for Native Interchain Token:
    ...
    solana/cli send its interchain-transfer \
        --source-account <SOLANA_MINT_TOKEN_MANAGER_ADDRESS> \
        --token-id <TOKEN_ID> \
        --destination-chain <DESTINATION_CHAIN> \
    ...
    **Note:** To get the `SOLANA_MINT_TOKEN_MANAGER_ADDRESS`, follow these steps:
    ...
    **Step 2:** For the account _that was created by ITS_, retrieve its account info:
    ...
    **Step 3:** Look for the output that shows:
    - `Address: <TOKEN_ACCOUNT_ADDRESS>` ← This is your SOLANA_MINT_TOKEN_MANAGER_ADDRESS
    - `Mint: <MINT_ADDRESS>` ← This should match your token's mint
    - `Balance: <AMOUNT>` ← This shows how many tokens you have

    **Important:** The `SOLANA_MINT_TOKEN_MANAGER_ADDRESS` is the **token account address** (where your tokens are stored), NOT the mint address (the token type). The `spl-token accounts` command sometimes shows mint addresses, so always verify with `spl-token account-info`.
...
1. Register Canonical Token:
    ...
    - Switch `PRIVATE_KEY` to EVM private key.

    Register the destination token on the destination chain (e.g., for EVM):

    ```sh
    ts-node evm/interchainTokenFactory.js register-canonical-interchain-token \
        --tokenAddress <DESTINATION_TOKEN_ADDRESS> \
        --chainNames <DESTINATION_CHAIN>
    ```

    - Switch `PRIVATE_KEY` to Solana keypair.

    Link the Solana token to the existing token on the destination chain:
...
1. Register Custom Token:
    ...
    ```sh
    solana/cli send its register-custom-token \
        --mint <SOLANA_MINT_ADDRESS> \
        --token-manager-type <TOKEN_MANAGER_TYPE> \
        --operator <OPERATOR_PDA> \
        --salt <SALT>
    ```
...
1. Switch `PRIVATE_KEY` to EVM private key.
...
1. Deploy Native Interchain Token:
    ...
    ts-node evm/interchainTokenFactory.js deploy-interchain-token \
        ...
        --minter <TOKEN_MINTER> \
        --chainNames <SOURCE_CHAIN> \
        --salt <SALT>

    ts-node evm/interchainTokenFactory.js deploy-remote-interchain-token $CHAIN \
        --chainNames <SOURCE_CHAIN> \
        --salt <SALT>
    ```
...
1. Interchain Token Transfer for Native Interchain Token:
    ...
    ```sh
    ts-node evm/its.js interchain-transfer --destinationChain $CHAIN --tokenId <TOKEN_ID> --destinationAddress <SOLANA_MINT_TOKEN_MANAGER_ADDRESS> --amount <TOKEN_AMOUNT> --chainNames <SOURCE_CHAIN>
    ```
...
1. Deploy Remote Canonical Token:
    ...
    ```sh
    ts-node evm/interchainTokenFactory.js register-canonical-interchain-token \
        --tokenAddress <EXISTING_EVM_TOKEN_ADDRESS> \
        --chainNames <SOURCE_CHAIN>

    ts-node evm/interchainTokenFactory.js deploy-remote-canonical-interchain-token
        <EXISTING_EVM_TOKEN_ADDRESS>
        $CHAIN \
        --chainNames <SOURCE_CHAIN>
    ```
...
1. Interchain Token Transfer for Canonical Token:
    ...
    ```sh
    ts-node evm/its.js interchain-transfer --destinationChain $CHAIN --tokenId <TOKEN_ID> --destinationAddress <SOLANA_MINT_TOKEN_MANAGER_ADDRESS> --amount <TOKEN_AMOUNT> --gasValue <GAS_VALUE> --chainNames <SOURCE_CHAIN>
    ```

1. Link Custom Token:

    Register an existing token on EVM with custom configuration:
    ...
    Register the destination token on Solana:
    ...
    Link the EVM custom token to the existing token on Solana:

    ```sh
    ts-node evm/interchainTokenFactory.js link-token \
        --destinationChain $CHAIN \
        --destinationTokenAddress <SOLANA_MINT_ADDRESS> \
        --tokenManagerType <TOKEN_MANAGER_TYPE> \
        -s <SALT> \
        -n <SOURCE_CHAIN>
    ```
    ...
    ```sh
    ts-node evm/its.js interchain-transfer --destinationChain $CHAIN --tokenId <TOKEN_ID> --destinationAddress <SOLANA_MINT_TOKEN_MANAGER_ADDRESS> --amount <AMOUNT> --gasValue <GAS_VALUE> --chainNames <SOURCE_CHAIN>
    ```
