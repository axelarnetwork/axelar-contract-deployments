...
1. Deploy and verify `solana_axelar_its` program:
    ...
    ```sh
    anchor deploy -p solana_axelar_its --provider.cluster $CLUSTER --program-keypair $ITS_PROGRAM_KEYPAIR_PATH --provider.wallet $UPGRADE_AUTHORITY_KEYPAIR_PATH -- --upgrade-authority $UPGRADE_AUTHORITY_KEYPAIR_PATH
