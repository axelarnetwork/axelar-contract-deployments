...
```bash
npm run stellar:deploy -- InterchainTokenService --version 1.0.0
```

2. Re-deploy Axelar Example with ITS support

```bash
npm run stellar:deploy -- AxelarExample --wasm-path ../axelar-amplifier-stellar/target/wasm32-unknown-unknown/release/stellar_example.optimized.wasm
```
