...
node cosmwasm/submit-proposal.js \
    ...
    -t "Register $CHAIN on ITSHub" \
    -d "Register $CHAIN on ITSHub" \
...
# Change `PRIVATE_KEY and `ENV` in `.env` to EVM
node evm/its.js --action interchainTransfer -n [source-chain] --destinationChain $CHAIN --destinationAddress [encoded-stellar-recipient-address] --tokenId [token-id] --amount [amount]
```
...
# Change `PRIVATE_KEY and `ENV` in `.env` to EVM
node evm/its.js --action interchainTransfer -n [source-chain] --destinationChain $CHAIN --destinationAddress [destination-address] --tokenId [token-id] --amount [amount]
```
