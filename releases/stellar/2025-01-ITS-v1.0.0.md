...
```bash
ts-node evm/interchainTokenFactory.js register-canonical-interchain-token [token_address] --chainNames [source_chain]

ts-node evm/interchainTokenFactory.js deploy-remote-canonical-interchain-token [token_address] $CHAIN --chainNames [source_chain]
```
