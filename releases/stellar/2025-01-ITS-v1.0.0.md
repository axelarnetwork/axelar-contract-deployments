...
node stellar/its.js deploy-remote-interchain-token 0x1234 [destination-chain] --gas-token-address [gas-token-address] --gas-amount [gas-amount]
```

2. Interchain Token Transfer for Native Interchain Token

```bash
node stellar/its.js interchain-transfer [token-id] [destination-chain] [destination-address] [amount] --gas-token-address [gas-token-address] --gas-amount [gas-amount]
```
...
node stellar/its.js deploy-remote-canonical-token [token-address] [destination-chain] --gas-token-address [gas-token-address] --gas-amount [gas-amount]
```
