...
```bash
ts-node evm/interchainTokenFactory.js deploy-interchain-token --name "test" --symbol "TEST" --decimals 18 --initialSupply <initialSupply> --minter <minter> --chainNames [source-chain] --env <env> --salt "salt"
...
```bash
ts-node evm/interchainTokenFactory.js register-canonical-interchain-token --tokenAddress [token-address] --chainNames [source-chain] --env <env>

ts-node evm/interchainTokenFactory.js deploy-remote-canonical-interchain-token --tokenAddress [token-address] --destinationChain $CHAIN --chainNames [source-chain] --env <env>
```
