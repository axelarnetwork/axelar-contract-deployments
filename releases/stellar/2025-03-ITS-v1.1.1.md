...
```bash
ts-node evm/interchainTokenFactory.js deploy-interchain-token --name [name] --symbol [symbol] --decimals [decimals] --initialSupply [initial_supply] --minter [minter]  --chainNames [source_chain] --salt [some salt]

# Adjust `--gasValue` or add gas directly from axelarscan for mainnet
ts-node evm/interchainTokenFactory.js deploy-remote-interchain-token --destinationChain $CHAIN --chainNames [source_chain] --salt [same salt as above]
```
