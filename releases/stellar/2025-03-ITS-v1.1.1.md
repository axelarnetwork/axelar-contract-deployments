...
```bash
ts-node evm/interchainTokenFactory.js deploy-interchain-token --name [source-chain] --symbol "TEST" --decimals 18 --initialSupply <initialSupply> --minter <minter>  --chainNames [source-chain] --env <env> --salt "salt"

# Adjust `--gasValue` or add gas directly from axelarscan for mainnet
ts-node evm/interchainTokenFactory.js deploy-remote-interchain-token --destinationChain $CHAIN --chainNames [source-chain]  --env <env> --salt "salt"
```
