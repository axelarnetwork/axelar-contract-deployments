...
```bash
npm run script stellar/deploy-contract.js upgrade InterchainTokenService --version 1.1.2 --migration-data '{"newTokenManagerWasmHash":"9883cdb6740d685007a88dbd5a1269
...
```bash
npm run script stellar/its.js migrate-tokens "A/cM/Lqj2/Fx22VBAjjgW1jDrnywIZy3Bo1Z1Wr5b7Q=" "dGQ0vIU2L8yyO8EEthUPrvCy0v+WpQ8tElUwREprmQ8=" "XqRV/aAChDa0fS7nCVsHUDo9e
...
```bash
npm run script stellar/its.js migrate-tokens "zlmGy2Rk0M/FhXFVKbDXxzQxjq33qxrvWtExvamdenI=" "pGiAIMevFALTHyQpjk+P1+e9nlgSqIEjHi9nOq0UxcE=" "sF8KrUqC9SrWClGQyl3oDK2Qf
...
```bash
npm run script stellar/its.js deploy-interchain-token TEST132 test132 18 0x891012 100000000

https://stellar.expert/explorer/public/tx/243163516243623936
```

```bash
npm run script stellar/its.js deploy-remote-interchain-token 0x891012 flow --gas-amount 50000000
...
```bash
npm run script evm/interchainTokenFactory.js --action deployInterchainToken -n avalanche --destinationChain stellar --salt "testC" --name "testC" --symbol "TESTC" --decimals 18

https://snowtrace.io/tx/0xde5796810bd40a35528c00f0ad67d5d36819470104e5f35c3d7af4e7fe4d6d19
```

```bash
npm run script evm/interchainTokenFactory.js --action deployRemoteInterchainToken -n flow --destinationChain stellar --salt "test55" --gasValue 500000000000000000
