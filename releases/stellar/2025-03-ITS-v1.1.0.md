...
```bash
npm run stellar:upload -- TokenManager --version 1.1.0
```
...
```bash
npm run stellar:upload -- InterchainToken --version 1.1.0
```
...
```bash
npm run stellar:upgrade -- InterchainTokenService --version 1.1.0 --migration-data '{"newTokenManagerWasmHash":"<bytes>","newInterchainTokenWasmHash":"<bytes>"}'
```
...
```bash
npm run stellar:upgrade -- Upgrader --version 1.1.0
```
