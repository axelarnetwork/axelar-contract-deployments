...
ts-node stellar/deploy-contract deploy InterchainTokenService --version 1.1.2 --interchain-token-version 1.1.2 --token-manager-version 1.1.2
...
```bash
node evm/interchainTokenFactory.js deploy-interchain-token -n avalanche --salt "salt" --name "test" --symbol "TEST" --decimals 18 --initialSupply 10000 --minter [your_wallet_address]

# Adjust `--gasValue` or add gas directly from axelarscan for mainnet
node evm/interchainTokenFactory.js deploy-remote-interchain-token $CHAIN -n [source-chain] --salt "salt" --gasValue 100000000000
```

2. Interchain Token Transfer for Native Interchain Token

```bash
node evm/its.js interchain-transfer -n [source-chain] --destinationChain $CHAIN --destinationAddress [encoded-recipient] --tokenId [token-id] --amount [amount]
```

3. Deploy Remote Canonical Token

```bash
node evm/interchainTokenFactory.js register-canonical-interchain-token [token-address] -n [source-chain]

node evm/interchainTokenFactory.js deploy-remote-canonical-interchain-token [token-address] $CHAIN -n [source-chain] --gasValue 100000000000
```

4. Interchain Token Transfer for Canonical Token

```bash
node evm/its.js interchain-transfer -n [source-chain] --destinationChain $CHAIN --destinationAddress [encoded-recipient] --tokenId [token-id] --amount [amount] --gasValue auto
```
...
node evm/interchainTokenFactory.js deploy-interchain-token --minter 0xba76c6980428A0b10CFC5d8ccb61949677A61233 --name $NAME --symbol $SYMBOL --decimals 18 --initialSupply 10000000 --salt $SALT -n $EVM_CHAIN
...
node evm/its.js register-token-metadata $EVM_TOKEN_ADDRESS -n $EVM_CHAIN --gasValue 1000000000000000000

# Register Custom Token on EVM
MINT_BURN_TYPE=4
node evm/interchainTokenFactory.js register-custom-token --tokenAddress $EVM_TOKEN_ADDRESS --tokenManagerType $MINT_BURN_TYPE --operator 0xba76c6980428A0b10CFC5d8ccb61949677A61233 --salt $SALT -n $EVM_CHAIN -y

# Link Token
MINT_BURN_FROM_TYPE=1
node evm/interchainTokenFactory.js link-token --destinationChain $STELLAR_CHAIN --destinationTokenAddress $STELLAR_TOKEN_ADDRESS --tokenManagerType $MINT_BURN_FROM_TYPE --linkParams "0x" --salt $SALT -n $EVM_CHAIN --gasValue 10000000000000000000 -y

# Set TOKEN_ID from the result
TOKEN_ID=

# Get token manager address on EVM
node evm/its.js token-manager-address $TOKEN_ID -n $EVM_CHAIN -y

# Set EVM_TOKEN_MANAGER from the result
EVM_TOKEN_MANAGER=

# Transfer mintership to the token manager
node evm/its.js transfer-mintership $EVM_TOKEN_ADDRESS $EVM_TOKEN_MANAGER -n $EVM_CHAIN -y
...
node evm/its.js interchain-transfer --destinationChain $STELLAR_CHAIN --tokenId $TOKEN_ID --destinationAddress $USER --amount 1 -n $EVM_CHAIN
