...
4. Deploy Example with dummy ITS address

```bash
node stellar/deploy-contract.js deploy example --version v1.0.0 --use-dummy-its-address
```

5. After deploying stellar contracts, ensure that you deploy [Stellar Amplifier contracts](../cosmwasm/2025-01-Stellar-GMP-v1.0.0.md).

6. Rotate genesis verifier set on Stellar Gateway

```bash
node stellar/gateway.js rotate
```

7. Get the list of operators used by the relayer and register on the Operators contract.
...
```bash
node evm/gateway.js -n [destination-chain] --action isMessageApproved --commandID [command-id] --sourceChain $CHAIN --sourceAddress [source-address] --destination [destination-address] --payloadHash 0x1ac7d1b81b7ba1025b36ccb86723da6ee5a87259f1c2fd5abe69d3200b512ec8
```
