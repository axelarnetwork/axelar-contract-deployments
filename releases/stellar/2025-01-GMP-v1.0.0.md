...
```bash
node stellar/deploy-contract.js deploy axelar_gateway --version 1.0.0 --minimum-rotation-delay [minimum-rotation-delay] --previous-signers-retention 15
```
...
```bash
node stellar/deploy-contract.js deploy example --version 1.0.0 --use-dummy-its-address
```
