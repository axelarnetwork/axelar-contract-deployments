...
ts-node cosmwasm/query token-config $EVM_CHAIN $EVM_TOKEN_ID
...
# Link Token
ts-node stellar/its link-token $SALT $EVM_CHAIN $STELLAR_TOKEN_ID LOCK_UNLOCK --gas-amount 10000000

# Verify token metadata is registered on ITS Hub
ts-node cosmwasm/query token-config stellar $STELLAR_TOKEN_ID
