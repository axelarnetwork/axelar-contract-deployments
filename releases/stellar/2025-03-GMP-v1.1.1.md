...
```bash
npm run script stellar/deploy-contract.js upgrade AxelarGateway --version 1.1.1 --migration-data '<[["<sourceChainN>", "<messageIdN>"]]>'
```
...
```bash
npm run script stellar/deploy-contract.js upgrade AxelarOperators --version 1.1.1 --migration-data '["<operatorN>"]'
```
...
```bash
npm run script stellar/operators.js is-operator operatorN
```
