...
docker run -d \
  ...
  axelarnet/axelar-ampd:v1.14.2 \
  --config /home/axelard/.ampd/config.toml
```
...
docker run -d \
  ...
  --restart unless-stopped \
  -v ~/.ampd/<chain-name>:/home/axelard/<chain-name> \
  axelarnet/axelar-ampd-<handler-type>-handler:v0.1.1 \
  --config-dir /home/axelard/<chain-name>
```
...
docker run -d \
  --name flow-handler \
  --restart unless-stopped \
  -v ~/.ampd/flow:/home/axelard/flow \
  axelarnet/axelar-ampd-evm-handler:v0.1.1 \
  --config-dir /home/axelard/flow
```

The config files you create following the instructions above should be placed in `~/.ampd/` (for ampd) and `~/.ampd/<chain-name>/` (for each handler) on your host machine, and they will be mounted into the containers.
