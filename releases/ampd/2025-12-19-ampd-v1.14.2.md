...
- [Solana handlers](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/solana-handler-v0.1.1)
...
└── solana/
    ├── config.toml                 # Base config for Solana handler
    └── solana-handler-config.toml  # Solana-specific config
```
...
Create a file named `solana-handler-config.toml` in your configuration directory:

```toml
rpc_url = "[YOUR_SOLANA_CHAIN_RPC_URL]"
```
