...
[Release v1.14.2](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/ampd-v1.14.2) (2025-12-19)

[Changelog](https://github.com/axelarnetwork/axelar-amplifier/blob/main/CHANGELOG.md#v1142)

## Handler Releases

- [EVM handlers](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/evm-handler-v0.1.1)
- [Sui handlers](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/sui-handler-v0.1.1)
- [Stellar handlers](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/stellar-handler-v0.1.1)
- [XRPL handlers](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/xrpl-handler-v0.1.1)
...
- **Handler Support**: Includes handler migrations and support for Stellar, Sui, and XRPL chains
...
This release introduces the new modular handler architecture for Axelar verifiers. Previously, all chain-specific verification logic was bundled into a single `ampd` daemon. With this release, handlers are now deployed as independent services.
...
> **⚠️ CRITICAL:** `ampd` v1.14.2 and modular handlers v0.1.1 must be deployed **in parallel**. If you upgrade `ampd` without deploying the corresponding handlers, or vice versa, handlers will stop working. Both components must be upgraded together.

### Step 1: Update AMPD Configuration
...
### Step 3: Configure Modular Handlers
...
└── xrpl/
    ├── config.toml                 # Base config for XRPL handler
...
The ampd handlers support setting up monitoring servers to integrate with Prometheus similarly to how ampd does it. To enable this, add the following section to your `config.toml` files and replace the example values with your actual values.
...
Regardless of the chain name, create a file named `evm-handler-config.toml` in your configuration directory:

```toml
rpc_url = "[YOUR_EVM_CHAIN_RPC_URL]"
finalization = "RPCFinalizedBlock" # Optional: "RPCFinalizedBlock" is the default when not specified. Use "ConfirmationHeight" if needed.
```
...
### Step 4: Deploy Handler Binaries

Download and deploy the handler binaries for each chain you support. Pre-built binaries and source code can be found at:
...
If you are building from source, run each of the following commands from the corresponding release source code directory to build the binaries:

```bash
cargo build --release --bin evm-handler
```

```bash
cargo build --release --bin sui-handler
```

```bash
cargo build --release --bin stellar-handler
```

```bash
cargo build --release --bin xrpl-handler
```

The binaries will be built in the `target/release` directory.

Once `ampd` is up and running with the gRPC server enabled, start the handler services for each chain you support:
...
The configuration instructions above work the same way for Docker deployments. You'll mount your config directory as a volume when running the containers.

#### Running AMPD with Docker
...
docker run -d \
  ...
  -v ~/.ampd:/root/.ampd \
  axelarnetwork/ampd:ampd-v1.14.2 \
  --config-dir /root/.ampd
```

#### Running Handlers with Docker

For each chain handler, run:

```bash
docker run -d \
  --name <chain-name>-handler \
  --restart unless-stopped \
  -v ~/.ampd/<chain-name>:/root/.ampd/<chain-name> \
  axelarnetwork/<handler-type>-handler:<handler-type>-handler-v0.1.1 \
  --config-dir /root/.ampd/<chain-name>
```

**Note:**

- Replace `<handler-type>` with the handler type: `evm`, `sui`, `stellar`, or `xrpl`
- Replace `<chain-name>` with the actual chain name (e.g., `ethereum`, `avalanche`, `flow`, `sui`, `stellar`, `xrpl`)

**Example:** For an EVM handler running on the "flow" chain:

```bash
docker run -d \
  --name flow-handler \
  --restart unless-stopped \
  -v ~/.ampd/flow:/root/.ampd/flow \
  axelarnetwork/evm-handler:evm-handler-v0.1.1 \
  --config-dir /root/.ampd/flow
```

The config files you create following the instructions above should be placed in `~/.ampd/` (for ampd) and `~/.ampd/<chain-name>/` (for each handler) on your host machine, and they will be mounted into the containers at the same paths.
...
1. Run the below command to output the version:
...
2. Check `ampd` and handler logs to ensure they are running correctly.

3. For each chain, monitor voting and signing activity for your verifier on Axelarscan to verify it's operating correctly.
