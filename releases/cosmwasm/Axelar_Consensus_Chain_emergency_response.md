...
## 7. Key Differences from Amplifier Chains

| Aspect | Consensus Chain | Amplifier Chain |
|--------|-----------------|-----------------|
| **Implementation** | Cosmos SDK native modules | CosmWasm smart contracts |
| **Chain control** | `DeactivateChain` / `ActivateChain` | `freeze_chain` / `unfreeze_chain` |
| **Routing control** | ❌ Not available | `disable_routing` / `enable_routing` |
| **Signing control** | ❌ Not available | `disable_signing` / `enable_signing` |
| **Execution control** | ❌ Not available | `disable_execution` / `enable_execution` |
| **Permission system** | `ROLE_ACCESS_CONTROL` / `ROLE_CHAIN_MANAGEMENT` | Admin EOA / Governance |
| **Command format** | `axelard tx <module> <action>` | `axelard tx wasm execute <addr> '{...}'` |
| **Emergency granularity** | Chain-level only | Chain/Routing/Signing/Execution levels |

---

## 8. Environment-Specific Information

| Environment | Chain ID | RPC URL | Config Location |
|-------------|----------|---------|-----------------|
| mainnet | `axelar-dojo-1` | TBD | TBD |
| testnet | `axelar-testnet-lisbon-3` | TBD | TBD |

**Note:** Update this section with actual environment-specific values before deployment.

---

## Comments / Open Items

1. **Consensus chain has fewer emergency controls than Amplifier**
   - No `disable_signing` / `enable_signing`
   - No `disable_routing` / `enable_routing`
   - Primary emergency action is `DeactivateChain`

2. **For severe incidents, validator coordination may be required**
   - Consensus chain operations are ultimately controlled by validator consensus
   - In extreme cases, validators may need to halt the entire chain

3. **Permission key management is critical**
   - `ROLE_ACCESS_CONTROL` key should be stored securely (HSM recommended)
   - Consider multi-sig setup for governance operations

4. **Action Items:**
   - [ ] Document exact addresses for each environment
   - [ ] Set up monitoring alerts for chain state changes
   - [ ] Create automated scripts for common emergency operations
   - [ ] Establish secure key storage procedures
