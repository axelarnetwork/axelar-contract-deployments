...
```bash
ts-node cosmwasm/contract.ts store-code -c InterchainTokenService -t "Upload InterchainTokenService contract v1.3.0" -d "Upload InterchainTokenService contract v1.3.0" --instantiateAddresses $INIT_ADDRESSES --version 1.3.0 --governance
```

2. Upload ITS ABI Translator contract

```bash
ts-node cosmwasm/contract.ts store-code -c ItsAbiTranslator -t "Upload ItsAbiTranslator contract v2.0.0" -d "Upload ItsAbiTranslator contract v2.0.0" --instantiateAddresses $INIT_ADDRESSES --version 2.0.0 --governance
```
