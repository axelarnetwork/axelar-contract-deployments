...
7. Update `ampd` with the Stacks chain configuration. Verifiers should use their own [Stacks Blockchain API](https://github.com/hirosystems/stacks-blockchain-api), which includes both a Stacks node, Bitcoin node & API for use as the `rpc_url`
   in production.
