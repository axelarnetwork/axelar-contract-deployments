...
- Confirm `ItsStacksTranslator(v1.0.0)`, `StacksVotingVerifier(v1.0.0)`, `Gateway(v1.1.1)` and `StacksMultisigProver(v1.0.0)` contracts are already stored in `$ENV.json`:
...
```bash
node ./cosmwasm/deploy-contract.js instantiate -c ItsStacksTranslator --fetchCodeId --instantiate2 --admin $CONTRACT_ADMIN
```
