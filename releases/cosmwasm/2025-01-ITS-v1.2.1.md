...
```bash
ts-node cosmwasm/contract.ts store-code -c InterchainTokenService -t "Upload InterchainTokenService contract v1.2.1" -d "Upload InterchainTokenService contract v1.2.1" --instantiateAddresses $INIT_ADDRESSES --version 1.2.1 --governance
```
...
```bash
ts-node evm/interchainTokenFactory.js deploy-interchain-token --name [name] --symbol [symbol] --decimals [decimals] --initialSupply [initialSupply] --minter [wallet]  --chainNames [chain_name] --salt [some salt]
```
