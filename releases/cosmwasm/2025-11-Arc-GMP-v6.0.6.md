...
- [VotingVerifier v2.0.0](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/voting-verifier-v2.0.0)
...
These are the instructions for deploying Amplifier contracts for the Arc connection.

### Pre-requisites

1. Predict the [External Gateway](../evm/2025-11-Arc-GMP-v6.0.6.md) address, as `VotingVerifier` needs the `sourceGatewayAddress` which is the External Gateway address.
...
```bash
VotingVerifier (v2.0.0) -> "storeCodeProposalCodeHash": "11877b06b4021d2ec15a02338bcaa97fe932aaec03dcd21e09a2de005a59779a"
...
1. Set environment variables
    ...
    | Network              | `PROVER_ADMIN`                                  | `REWARD_AMOUNT`     | `EPOCH_DURATION` |
    | -------------------- | ----------------------------------------------- | ------------------- | ---------------- |
    | **Devnet-amplifier** | `axelar1zlr7e5qf3sz7yf890rkh9tcnu87234k6k7ytd9` | `1000000uamplifier` | `500`            |
    | **Stagenet**         | `axelar1l7vz4m5g92kvga050vk9ycjynywdlk4zhs07dv` | `1000000uaxl`       | `3000`           |
    | **Testnet**          | `axelar17qafmnc4hrfa96cq37wg5l68sxh354pj6eky35` | `1000000uaxl`       | `3000`           |
    | **Mainnet**          | `axelar1pczf792wf3p3xssk4dmwfxrh6hcqnrjp70danj` | `1000000uaxl`       | `74225`          |
    ...
    - Note: all the following governance proposals should be submitted at one time so deployment doesn't get held up while waiting for voting. [ITS proposal](../evm/2025-11-Arc-ITS-v2.2.0.md) should also be submitted at this time if possible.

1. Create reward pools for Voting Verifier and Multisig

    | Network              | `participation_threshold` | `rewards_per_epoch` |
    | -------------------- | ------------------------- | ------------------- |
    | **Devnet-amplifier** | `[\"7\", \"10\"]`         | `100`               |
    | **Stagenet**         | `[\"7\", \"10\"]`         | `100`               |
    | **Testnet**          | `[\"7\", \"10\"]`         | `100`               |
    | **Mainnet**          | `[\"8\", \"10\"]`         | `TBD`               |
    - for `devnet-amplifier` use direct execution

    ```bash
    # Execute voting verifier pool creation
    ...
    # Execute multisig pool creation
    axelard tx wasm execute $REWARDS \
    '{
        "create_pool": {
        "params": {
    ...
    - for `stagenet`, `testnet`, and `mainnet` submit proposal

    ```bash
    ts-node cosmwasm/submit-proposal.js execute \
    -c Rewards \
    -t "Create reward pools for $CHAIN" \
    -d "Create reward pools for $CHAIN voting verifier and multisig" \
    --msg "[
        {
        \"create_pool\": {
            \"params\": {
...
1. Register ITS edge contract on ITS Hub
    ...
    Proceed with this step only if ITS deployment on Arc is confirmed. Add the following to `contracts` in the Arc config within `ENV.json`:
...
1. Confirm proposals have passed
    ...
    - "Instantiate contracts for Arc"
    - "Create reward pools for Arc"
    - (optional) "Register Arc on ITS Hub"
...
1. Update `ampd` with the Arc chain configuration. Verifiers should use their own Arc RPC node for the `http_url` in production.
...
1. Update `ampd` with the Arc chain configuration.
...
The [GMP checklist for Arc](../evm/2025-11-Arc-GMP-v6.0.6.md) will test GMP calls.
