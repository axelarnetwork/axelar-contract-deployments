...
```bash
mkdir wasm
wget https://static.axelar.network/releases/cosmwasm/interchain-token-service/1.1.0/interchain-token-service.wasm --directory-prefix=wasm/
```
...
CHECKSUM=$(cat checksums.txt | grep interchain-token-service.wasm | awk '{print $1}')
shasum -a 256 wasm/interchain-token-service.wasm | grep $CHECKSUM
```

3. Expected output, make sure this matches before proceeding

```
174688fff71f479dca62066a9db5bb417e8b38db2d066650bf20e7e2b623f854  wasm/interchain-token-service.wasm
```
