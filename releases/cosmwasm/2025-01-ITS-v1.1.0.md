...
```bash
ts-node cosmwasm/contract.ts store-code -c InterchainTokenService -t "Upload InterchainTokenService contract v1.1.0" -d "Upload InterchainTokenService contract v1.1.0" --instantiateAddresses $INIT_ADDRESSES -a ./wasm --governance
```
