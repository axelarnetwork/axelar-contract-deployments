...
```bash
axelard q wasm contract-state smart <router-address> '{"chain_info": "<chain-name>"}' --output json | jq .
...
```bash
axelard q wasm contract-state smart <multisig-addr> '{"is_caller_authorized": {"contract_address": "<xrpl-multisig-prover-addr>", "chain_name": "<chain-name>"}}' --output json | jq .
