...
- Create an `.env` config. `CHAIN` should be set to `xrpl` for mainnet.
...
| Network              | `serviceName` | `votingThreshold` | `signingThreshold` | `confirmationHeight` | `xrplFee` | `ticketCountThreshold` |
| -------------------- | ------------- | ----------------- | ------------------ | -------------------- | --------- | ---------------------- |
...
```bash
XrplVotingVerifier(v1.0.0) -> "storeCodeProposalCodeHash": "0x123"
XrplGateway(v1.0.0) -> "storeCodeProposalCodeHash": "0xabc"
XrplMultisigProver(v1.0.0) -> "storeCodeProposalCodeHash": "0xdef"
```
...
axelard q wasm contract-state smart $REWARDS "{\"rewards_pool\":{\"pool_id\":{\"chain_name\":\"$CHAIN\", \"contract\":\"$XRPL_VOTING_VERIFIER\"}}}" --output json | jq .
```
