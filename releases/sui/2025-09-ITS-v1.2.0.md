...
```bash
ts-node sui/its link-coin [coin-symbol] [coin-description] [coin-decimals] [destination-chain] [destination-token-address] --tokenManagerMode ['lock_unlock' | 'mint_burn'] --destinationOperator [tokenManagerOnEVMChain]
```
