...
# Register custom coin on EVM chain
ts-node evm/interchainTokenFactory.js --action registerCustomToken --tokenAddress $EVM_TOKEN_ADDRESS --tokenManagerType $LOCK_UNLOCK --operator $TOKEN_MANAGER_ADDRESS --salt $SALT -n $EVM_CHAIN
```
...
ts-node evm/interchainTokenFactory.js --action linkToken --destinationChain sui --destinationTokenAddress $SUI_TOKEN_ADDRESS --tokenManagerType LOCK_UNLOCK --linkParams "0x" --salt $SALT -n $EVM_CHAIN --gasValue 5000
```
