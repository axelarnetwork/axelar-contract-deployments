...
Wait for "Register Token Metadata" step to be completed before proceeding with steps below it during command execution.

```bash
ts-node sui/its link-coin [coin-symbol] [coin-description] [coin-decimals] [destination-chain] [destination-token-address] [gasFee] --tokenManagerMode ['lock_unlock' | 'mint_burn'] --destinationOperator [tokenManagerOnEVMChain]
```
