...
- Run the following for any EVM chain, update env with the EVM private key:

```bash
# Create a token on 1 chain. Substitute the `wallet` below with the deployer key
ts-node evm/interchainTokenFactory --action deployInterchainToken --minter [wallet] --name "test" --symbol "TST" --decimals [decimals] --initialSupply 10000 --salt "salt12345"

# Register token metadata. Ensure GMP call is executed
ts-node evm/its.js register-token-metadata [tokenAddress]
```

- Fetch tokenManager address on EVM chain

```bash
# Record tokenManager address from output for transferMintership
ts-node evm/its.js token-manager-address [tokenId]

# Register evm custom token. Replace tokenAddress and tokenManager with received values.
MINT_BURN_TYPE=4 
ts-node evm/interchainTokenFactory --action registerCustomToken --tokenAddress [tokenAddress] --tokenManagerType $MINT_BURN_TYPE --operator [tokenManager] --salt "salt12345"

# Transfer mintership for each token to the token manager
ts-node evm/its.js transfer-mintership [tokenAddress] [tokenManager]
```

- Run on Sui chain, update env with sui private key:

```bash
# Create a token and register its metadata on Axelar Hub
# (replace destinationChain with evm chain name)
ts-node sui/its register-coin-metadata TST [destinationChain] --coinName "test" --coinDecimals 9

# Example
# ts-node sui/its register-coin-metadata TEST avalanche --coinName "Test Coin" --coinDecimals 9

# Link the sui token to the destination token 
# destinationChain = chain
# destinationAddress = evm tokenAddress
# destinationOperator = evm tokenManager
ts-node sui/its link-coin TST [destinationChain] [destinationAddress] --tokenManagerMode ['lock_unlock' | 'mint_burn']

# Example
# ts-node sui/its link-coin TEST avalanche 0xD9445695778773a7FFDA4DcD1cCAD90bA6d016E7 --tokenManagerMode mint_burn
```
