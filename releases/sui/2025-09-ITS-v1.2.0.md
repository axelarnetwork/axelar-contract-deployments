...
# Deploy token on Sui
ts-node sui/its register-custom-coin $SYMBOL $NAME $DECIMALS --mintAmount 10000

# Register token metadata on Axelar Hub
ts-node sui/its register-coin-metadata $SYMBOL
```

2. Source chain (Sui) registers custom token and links it (this will deploy a token manager on destination chain)
...
3. (**MINT_BURN only**) Transfer mintership on EVM chain to its Token Manager
