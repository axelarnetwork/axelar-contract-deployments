...
```bash
# Create Sui token and register its metadata on Axelar Hub
...
# Link the sui token to its corresponding (destination) token 
# (replace destinationChain destinationAddress (evm token address) and set tokenManagMode
