...
- Link a token on EVM (LOCK_UNLOCK) with a Sui coin (LOCK_UNLOCK and MINT_BURN both)
...
# Deploy a token with 9 decimals on EVM
...
ts-node sui/its register-coin-metadata $SYMBOL $EVM_CHAIN --coinName $NAME --coinDecimals $SUI_DECIMALS

# Example
# ts-node sui/its register-coin-metadata TEST avalanche --coinName "Test Coin" --coinDecimals 9

# Register a custom coin on sui and link it to the destination token
# destinationChain = chain
# destinationAddress = evm tokenAddress
# destinationOperator = evm tokenManager
ts-node sui/its link-coin $SYMBOL $EVM_CHAIN $EVM_TOKEN_ADDRESS --tokenManagerMode $TOKEN_MANAGER_MODE
