...
ts-node sui/its-example deploy-token TEST_L "Test Token" 9 --origin

# Register token with manager type: mint / burn
ts-node sui/its-example deploy-token TEST_M "Test Token" 9 --tokenManagerMode "mint_burn" --mintAmount 100 --origin
```
