...
EVM_CHAIN=avalanche
TOKEN_MANAGER_MODE=mint_burn # Source chain
DESTINATION_TOKEN_MANAGER_MODE=lock_unlock # Destination chain
...
# DESTINATION CHAIN
# Deploy token on EVM
ts-node evm/interchainTokenFactory --action deployInterchainToken --minter 0xba76c6980428A0b10CFC5d8ccb61949677A61233 --name $NAME --symbol $SYMBOL --decimals 9 --initialSupply 100000000000 --salt $EVM_TEMP_SALT -n $EVM_CHAIN
...
ts-node sui/its register-custom-coin [symbol] [name] [decimals] --treasuryCap
...
ts-node sui/its link-coin [symbol] [EVM_CHAIN] [EVM_TOKEN_ADDRESS] --tokenManagerMode mint_burn  --destinationTokenManagerMode lock_unlock --registered
...
4. Interchain Transfer (both ways)

```bash
ts-node evm/its interchain-transfer sui $EVM_TOKEN_ID 0x4cdab023fcddcf492891b9d72ce7b523c5b465883d02fedd1bbe81f4598cf40b 10 --gasValue 1500000000 -n $EVM_CHAIN

ts-node sui/its interchain-transfer [coinPackageId] [coinPackageName] [coinModName] [coinObjectId] [tokenId] [destinationChain] [destinationAddress] [amount]
# ts-node sui/its interchain-transfer 0x265ce251c3a65f0ddfe0d90a62b758662209813b26adb7b76f260c148bc92350 tstc TSTC 0x62b91d9f23658bd01c5ad6942ac0d11d2ef9de9f588f7bb28c040c38f12a2be7 0x1b1fa2634fa1bd00c63259910c7410a9426af705c60874cc295ad2f8904715ae avalanche 0x13f8C723AeB8CA762c652c553a11a11483846d8B 25
```

#### Case 2: Sui as destination chain

1. Deploy tokens to both chains and register their metadata on Axelar Hub
...
ts-node sui/its register-coin-metadata $SYMBOL --coinName $NAME --coinDecimals $DECIMALS

# Record the package id saved to the chain config
SUI_TOKEN_ADDRESS=
```

2. Register the token on the EVM chain

```bash
# Register custom coin on EVM chain, record new token ID
ts-node evm/interchainTokenFactory.js --action registerCustomToken --tokenAddress $EVM_TOKEN_ADDRESS --tokenManagerType $MINT_BURN --operator 0xba76c6980428A0b10CFC5d8ccb61949677A61233 --salt 0x1234ffff -n $EVM_CHAIN

TOKEN_ID=

#Record this token Address
ts-node evm/its.js token-manager-address $TOKEN_ID -n $EVM_CHAIN
...
3. Link the EVM token to the destination chain (Sui)

```bash
# SOURCE CHAIN 
# Link EVM token to desitination token (Sui token)
# Hexlified token type available in Sui token config or the type value should work directly too
SUI_TOKEN_ADDRESS=0x353137313839383736636665636431353063643066663437366333343338303538643666333462323132636431623633623138313838333965383232643161613a3a746573743132333435663a3a54455354313233343546
ts-node evm/interchainTokenFactory.js --action linkToken --destinationChain sui --destinationTokenAddress $SUI_TOKEN_ADDRESS --tokenManagerType $LOCK_UNLOCK --linkParams "0x" --salt 0x1234ffff -n $EVM_CHAIN --gasValue 1000000000000000000
```

4. (**MINT_BURN / MINT_BURN_FROM only**) Transfer mintership on EVM chain to its Token Manager

```bash
# Transfer mintership for evm token to the token manager
ts-node evm/its.js transfer-mintership $EVM_TOKEN_ADDRESS $TOKEN_MANAGER_ADDRESS -n $EVM_CHAIN
```

5. Interchain Transfer (both ways)
