...
impl ArchivedExecutableProposal {
    ...
    fn execute(
        ...
        // Only sign the CPI if the target program is the governance program itself.
        // This emulates the onlySelf modifier from [original implementation](https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/blob/main/contracts/governance/InterchainGovernance.sol#L69).
        let signing_seeds: &[&[&[u8]]] = &[&[seed_prefixes::GOVERNANCE_CONFIG, &[config_pda_bump]]];
        let signer_seeds = if target_address == crate::ID {
            signing_seeds
        } else {
            &[]
        };
