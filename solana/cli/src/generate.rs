...
pub fn generate_unsigned_solana_transaction(
    ...
    let fee_payer_pubkey = Pubkey::from_str(&args.fee_payer)?;
    ...
    match (&args.nonce_account, &args.nonce_authority) {
        (Some(na_str), Some(nauth_str)) => {
            ...
            let nonce_account_pubkey = Pubkey::from_str(na_str)?;
            let nonce_authority_pubkey = Pubkey::from_str(nauth_str)?;

            blockhash_for_message = fetch_nonce_data_and_verify(
                &config.url,
                &nonce_account_pubkey,
            ...
            println!(
                ...
                nonce_account_pubkey, blockhash_for_message
            );

            params.nonce_account = Some(nonce_account_pubkey.to_string());
            params.nonce_authority = Some(nonce_authority_pubkey.to_string());
    ...
    if config.network_type == NetworkType::Devnet {
