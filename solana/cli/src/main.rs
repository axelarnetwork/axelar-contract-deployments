...
enum InstructionSubcommand {
    ...
    GasService(GasServiceCommands),

    #[clap(long_about = "Commands for InterchainTokenService program", subcommand)]
    InterchainTokenService(InterchainTokenServiceCommands),
...
#[derive(Parser, Debug)]
struct InitGasServiceCommandArgs {}

#[derive(Parser, Debug)]
struct InitInterchainTokenServiceCommandArgs {}

#[derive(Parser, Debug)]
struct InitGovernanceCommandArgs {}

#[derive(Parser, Debug)]
struct AddTrustedChainCommandArgs {
    /// Chain ID of the chain to add
    #[clap(long)]
    chain_id: String,
}

#[derive(Parser, Debug)]
struct RemoveTrustedChainCommandArgs {
...
#[derive(Subcommand, Debug)]
enum GasServiceCommands {
    Init(InitGasServiceCommandArgs),
}

#[derive(Subcommand, Debug)]
enum InterchainTokenServiceCommands {
    Init(InitInterchainTokenServiceCommandArgs),
    AddTrustedChain(AddTrustedChainCommandArgs),
    RemoveTrustedChain(RemoveTrustedChainCommandArgs),
}

#[derive(Subcommand, Debug)]
enum GovernanceCommands {
    Init(InitGovernanceCommandArgs),
}
...
async fn build_instruction(
    ...
    let serializable_ix = match instruction {
        InstructionSubcommand::Gateway(command) => {
            gateway::build_gateway_instruction(fee_payer, command, config).await?
        }
        InstructionSubcommand::GasService(gas_service_command) => match gas_service_command {
            GasServiceCommands::Init(_) => todo!(),
        },
        InstructionSubcommand::InterchainTokenService(interchain_token_service_command) => {
            match interchain_token_service_command {
                InterchainTokenServiceCommands::Init(_) => todo!(),
                InterchainTokenServiceCommands::AddTrustedChain(_) => todo!(),
                InterchainTokenServiceCommands::RemoveTrustedChain(_) => todo!(),
            }
        }
        InstructionSubcommand::Governance(governance_command) => match governance_command {
            GovernanceCommands::Init(_) => todo!(),
        },
    };
