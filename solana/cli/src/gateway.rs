...
pub(crate) async fn build_gateway_instruction(
    fee_payer: &str,
    command: Commands,
    config: &Config,
...
async fn init(fee_payer: &str, init_args: InitArgs, config: &Config) -> eyre::Result<Instruction> {
    ...
    let payer = Pubkey::from_str(fee_payer)?;
    let upgrade_authority = payer;
    let operator = Pubkey::from_str(&init_args.operator)?;
...
async fn call_contract(
    ...
    let (signing_pda, signing_pda_bump) =
        axelar_solana_gateway::get_call_contract_signing_pda(payer);
