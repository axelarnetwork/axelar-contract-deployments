...
use solana_sdk::{
    commitment_config::CommitmentConfig, compute_budget::ComputeBudgetInstruction, instruction::InstructionError,
...
use crate::utils::{print_transaction_result, create_compute_budget_instructions, DEFAULT_COMPUTE_UNITS, DEFAULT_PRIORITY_FEE};
    ...
    for serializable_tx in serializable_txs {
                ...
                Ok(sim_result) => {
                    if let Some(units) = sim_result.value.units_consumed {
                        ...
                        if units > 150_000 {
                            ...
                            println!("Added compute budget: {} units with {} micro-lamports priority fee", 
                                DEFAULT_COMPUTE_UNITS, DEFAULT_PRIORITY_FEE);
                        }
                    }
                },
                Err(err) => {
                    println!("Simulation failed: {:?}, proceeding with regular transaction", err);
        ...
        match rpc_client.send_and_confirm_transaction(&transaction) {
            ...
            Err(err) => {
                ...
                if should_continue {
                    println!("Transaction error: GatewayError (code: {:?}), but continuing with next transaction as it's recoverable",
                        err.kind());
                    continue;
                } else {
