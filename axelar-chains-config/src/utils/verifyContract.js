...
const verifyContract = (env, chain, contract, args, options = {}) => {
  ...
  const filePath = options.dir
    ? `${options.dir}/temp-arguments.js`
    : "temp-arguments.js";

  const contractArg = options.contractPath
  ...
  try {
    ...
    if (error.message.toLowerCase().includes("already verified")) {
      console.log(
        `Contract ${contract} is already verified on ${normalizedChainName}.`,
      );
    } else {
      throw new Error(
        `An error occurred while trying to verify ${contract} on ${normalizedChainName}:\n${error}`,
      );
    }
  }
};
