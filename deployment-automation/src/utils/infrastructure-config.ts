...
export async function updateInfrastructureConfig(): Promise<void> {
  try {
    ...
    try {
      ...
      // Find an existing MultisigSigner to get the contract address
      let multisigSignerContract = '';
      const existingMultisigSigner = handlers.find(h => h.type === 'MultisigSigner');
      
      if (existingMultisigSigner && existingMultisigSigner.cosmwasm_contract) {
        multisigSignerContract = existingMultisigSigner.cosmwasm_contract;
        displayMessage(MessageType.INFO, `Found existing MultisigSigner contract: ${multisigSignerContract}`);
      } else {
        displayMessage(MessageType.WARNING, "No existing MultisigSigner found to copy contract address from");
      }
      ...
      if (!hasMsgVerifier || !hasSetVerifier || !hasMultisigSigner) {
        ...
        if (!hasMultisigSigner && multisigSignerContract) {
          newEntries.push(
            `${handlersIndentation}- type: MultisigSigner`,
            `${handlersIndentation}  cosmwasm_contract: ${multisigSignerContract}`,
            `${handlersIndentation}  chain_name: ${chainName}`
          );
          displayMessage(MessageType.SUCCESS, `Added MultisigSigner handler for ${chainName}`);
        }
        ...
        displayMessage(MessageType.INFO, `All handlers for ${chainName} already exist, no changes needed`);
      }
